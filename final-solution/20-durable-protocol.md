...
These are distinct roles, not interchangeable integers. History identities/stamps belong to `bumbledb-log`; `SchemaId` and ordinary application entity values are imported from the core. The main engine sees its canonical values and opaque transaction attachment, never the log's protocol types.
...
**Exactly one authoritative fact payload:** the decision stores the concrete canonical command, whose application payload is the imported core `ChangeSet` (the sealed native checked delta), not a second log operation list or scalar encoding. All application identifiers already have their final bytes. The log codec frames identity/precondition/result metadata around the core's canonical logical value/change bytes, never LMDB index bytes or an independently tagged row codec. `outcome_evidence` contains the terminal tag, changed summary, observed precondition state, or bounded rejection evidence; a bounded declared result is copied into the public receipt. Initial prepare and replay decode the envelope, invoke the same core change decoder/admission, and never run a host callback or sequence-dependent value resolver. Replay verifies the command digest and checks the recorded outcome against evaluation at the exact predecessor. Diagnostic selection/order is codec-defined, not dependent on available RAM or a caller's timeout; insufficient work budget returns progress/failure rather than different decision evidence. After checkpointing, the receipt table retains the bounded ID/digest/outcome/result needed for lookup, not every old command body. Retired receipts can subsequently be removed under the explicit policy.
...
The published snapshot satisfies the core's `QueryReader` interface and accepts the exact core query templates, typed parameters and execution policy, returning the exact core `CompleteResult`. Only acquisition/freshness and added history stamps belong to the log. There is no log query dialect, result conversion layer, fresh scalar type or public writable `Db` escape. Core-local generation witnesses and log `StateStamp` remain intentionally distinct authorities even though the read/query operations are shared.
...
| `PROTO-17` | Core embedded and LocalHistory are separately tested using the same sealed core ChangeSet/query/result types; local history atomically persists facts/receipt/head attachment across process kill. Hosted envelope replay uses the core canonical change decoder; no independent log row codec or recorder survives |
