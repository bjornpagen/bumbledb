...
Every call constructs a lazy effect. Acquisitions run again if the effect is run again; queries rerun against their still-live snapshot; core apply can perform another local transaction. There is no hidden memoization of database calls. Insert/delete effects are normally **re-runnable while their draft is building**: each sequential execution reads the supplied iterable and charges its work again. Repeated identical facts deduplicate at final set normalization, not by memoizing an Effect. Mutable input can yield different facts and a one-shot iterator can be exhausted; constructing an effect neither snapshots nor makes its input replayable. The SDK never automatically retries ingestion or replays user iterators. Concurrent/reentrant construction still refuses and spends/drains the draft; there is no implicit queue. Finish consumes the draft, so a second finish or later ingestion refuses through the existing spent/closed capability state. No per-Effect execution latch or separate spent-operation state is needed. Sealed changes/commands are immutable and reusable while open: retrying submit is allowed and refers to the identical command, not a rebuilt intent. Do not put a whole build/open/generate workflow under `Effect.retry`.
...
2. Cold laziness; mutation between effect construction and execution; successful acceptance then mutation; sequential insert/delete effect reruns (stable arrays, changed arrays and exhausted iterators), cumulative budget charging, concurrent/reentrant refusal, and finish/use-after-finish refusal; one-shot result-stream consumption; escaped scopes and foreign runtime handles. No unchecked typed-cast escape or automatic iterator replay.
