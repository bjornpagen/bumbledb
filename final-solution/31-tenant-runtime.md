...
3. Signal cancellation to cancellable work. If publication was dispatched, preserve its reference/evidence. A live caller may receive a receipt or OutcomeUnknown; an interrupted fiber may receive neither. Its Cause is not evidence of rollback. Close does not roll back remote history.
...
The native close transition reports completion or `CloseIncomplete` with outstanding work/resources. Rust may pass its explicit deadline/policy; TypeScript `close()` uses the NativeRuntime layer's finite cleanup policy and returns chapter 35's CloseReport Effect. If that envelope expires, the owner remains `Closing`; another close joins it. The registry cannot count those bytes or file descriptors as reclaimed. It may not open a successor into the same directory while the old owner remains live.
...
All TypeScript resource owners are acquired in Effect Scope, including drafts, changes/commands and completed results. No Symbol.dispose/asyncDispose or Promise lifetime twin remains. Early close/release returns Effect<CloseReport>; finalizers run that same close and surface incomplete/failed drain as a structured CloseFailure defect, retaining native Closing resources and accounting. Chapter 35 fixes the complete policy, including interrupted acquisition and known receipts followed by cleanup failure. Small owned metadata needs no finalizer. Rust keeps native RAII and its explicit bounded close report.
...
Node's **Effect-only** data API submits native work to a fixed-capacity Rust executor shared by the runtime. A small queue and per-owner admission count prevent unbounded futures retaining command/query payloads. Database-free ingestion/finalization/hash work uses the same bounded runtime and accounting, not an unbounded queue hidden outside tenant limits. Network and blocking filesystem work use appropriately bounded execution lanes; there is not a separate async runtime per tenant. A prepared LMDB writer stays on its owning worker across the bounded remote attempt; it cannot migrate through an arbitrary async executor. No JavaScript application callback, synchronous or asynchronous, executes inside a native transaction.
...
Native query work, integrity judgment, replay, checkpoint encoding, change normalization/finalization and canonical hashing do not run synchronously on the Node event loop. Ordinary core `ChangeSet.insert`/`delete` ingestion is asynchronous and yields between charged, bounded host copy/check chunks; it is required for 1.0, not deferred bulk ingestion or a separate protocol. Single-cell/row, chunk and total input limits prevent one enormous value from defeating the bound. A caller keeps input stable throughout ingestion effect execution; successful completion establishes ownership. Constructing an effect performs no copying or admission. Log sealing asynchronously retains/hashes that exact native change without a second JS row walk/copy. Parameter/row-codec ingestion and outbound result conversion obey the same chunk/accounting discipline; a completed million-row query is not materialized into one enormous JS array.

Pure small schema/query/scalar/intent constructors, inert layer/effect descriptions and already-owned metadata access stay synchronous; they perform no hidden database work. Effect.sync/try wrappers around blocking native work do not satisfy this contract. Getters and iterator steps supplied by the application still execute in JS and cannot be preempted if they never return; limits cover library work and completed host steps, not an arbitrary user program. No database writer gate is held during ingestion. Draft failure/cancellation stops admission and drains/releases native buffers before completed cancellation/failure, or explicitly reports incomplete drain with retained native accounting, even when a wrapper remains reachable; teardown capacity must remain available under queue saturation. Close/release cannot prematurely reclaim an in-flight worker's inputs.
...
5. Acquire a borrow, execute a bounded query/command, and release it through its Effect scope. Expose bounded result pages, not a scan of every note into one response. Define application response codes for decided rejection, precondition failure, overload and unknown publication.
...
One bounded Effect `inspect(work)` snapshot exposes owner state, identity, published stamps, queued/active operation counts, unknown-command count/oldest age, progress toward a requested frontier, current root/pins, last maintenance error, accounted memory/cache/scratch/disk, mapping extent, and open handle counts. A metadata property must not perform a hidden synchronous inspect or filesystem measurement. Structured events may be passed to a caller-supplied sink outside critical sections.
...
## Effect integration stays thin

The core NativeRuntime service/layer owns one bounded addon runtime. The log uses it directly; TenantCache.make acquires the one native registry as a scoped value that an app service can own with Layer.effect. No layer-per-tenant executor, JS TTL cache, automatic RequestResolver or ambient transaction service is added. Completed result delivery uses a core page Stream over a private consuming cursor. Effect owns fiber/scope lifetimes; native capability generations, worker affinity, byte/disk reservations and kernel locks remain native obligations. Chapter 35 owns A/E/R, version-matched Effect APIs and the shared V8 performance rules.

## Mandatory runtime and hosting release gates

| Gate | Required scenario and assertion |
| --- | --- |
| RUN-01 Borrow isolation | Concurrent same-tenant acquires produce distinct borrows. Double release, stale release after reopen, nested Effect scopes, escaped child snapshot/session, abandoned caller and retained writer cannot affect another borrow/slot. An active operation lease blocks eviction after its borrow releases. |
...
| RUN-10 Cancellation/fairness | Cancel queue, input copy/check, change finalization/envelope hash, AWS request/body, replay, query, scratch insert, result transfer and maintenance. Saturate normal queues while a failed draft/close drains. Measure bounded engine safe-point latency and event-loop delay with large admitted cells/batches/codecs; another ready tenant progresses under a hot tenant. No sync data/disposal alternative bypasses these bounds. |
