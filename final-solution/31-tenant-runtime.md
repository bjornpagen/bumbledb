...
There are three necessary pieces: an owner that actually closes native resources, an optional local cache registry returning independent borrows, and a bounded worker adapter for async hosts. Rust implements them once. TypeScript adds ergonomic lifetimes and structured value conversion, not another protocol implementation. The core remains public Rust/TypeScript/C; log history and its migration runner are public TypeScript only. Internal Rust owners/protocol types are implementation details, not a separately packaged Rust/C log SDK.
...
An idle **managed Node snapshot token or core C snapshot token** is revocable when its owner closes. A running native operation cannot have its data freed underneath it: it holds an operation lease until a cancellation safe point and teardown complete. Managed public result values are owned; methods do not hand out arbitrary references into a mapping that an unrelated close could unmap. The optional core C borrowed-view interface is restricted to the explicit result lifetime described in chapter 32. No C log history interface is required.
...
Local disk remains a real constraint. If a Lambda `/tmp` cannot hold a checkpoint materialization plus needed scratch, report `InsufficientLocalDisk` with required/available estimates before attempting the large write. The host may use a larger local volume or another deployment; no automatic placement service is added. This is a particular worker's feasibility result, not an engine database-size cap.
...
The engine supports larger-than-RAM execution through disk-native access and temporary-LMDB scratch. A memory-constrained worker can be slower, but its answers cannot change. Whole-database **in-memory** hydration, full result collection and full-relation resident images are not mandatory intermediate steps. A cold S3 materialization still requires the selected full canonical checkpoint on local disk; this is not a remote demand-paged database design.
...
The example's package versions, Node runtime, native ABI and Linux libc target must all name the released artifact it actually deploys. Deployment tests inspect the attached role and function-URL/API authentication policy. No AWS test with missing credentials counts as passed.

## Migration and deletion are finite log operations, not a platform

The core provides admitted-state construction and coherent snapshot copy primitives. The TypeScript log package exposes named-source pin, read-only export, validated import to a new incarnation, verification, and explicit source write closure/cutover primitives. Chapter 33 adds checked repository migrations and one explicit TypeScript runner. It executes deterministic transforms over bounded copied pages outside native transactions, while internal Rust remains the durable freeze/import/publication authority. A documented application runbook sequences these with downtime; no automatic multi-tenant migration scheduler or dual-write engine is required for 1.0. Application open, request handlers, React hooks and worker cache acquisition never run migrations implicitly.
...
| RUN-03 Native reclamation | Thousands of open/query/read/close cycles with GC disabled and inert Node core/log wrappers and core C tokens retained. Engine owners, locks, FDs and mappings return to baseline; same-path reopen succeeds; allocated disk is actually reclaimable. Separately held legal Rust snapshot guards cause explicit incomplete close until dropped, never unsafe revocation. |
...
| RUN-14 Invocation lifecycle | Remaining-time exhaustion, post-open operational failure, duty timeout/kill, warm invocation reuse, concurrent requests and abrupt process exit. All work is accounted/joined or leaves recoverable unknown state. |
| RUN-15 Migration | Source write closure, named pin, rejected transform, interrupted import/verification and application cutover rollback. Old cache/token/command cannot reach new incarnation; no core dependency on log migration types. |
