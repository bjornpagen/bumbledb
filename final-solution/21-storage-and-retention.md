...
The remote representation is intentionally modest: a linear decision tail and **streamed complete checkpoints**, not an incremental remote B-tree or a general object DAG. The cost is occasional O(database size) checkpoint work. It must be bounded in RAM, measurable, and completable during ongoing writes.
...
An object key is qualified by **epoch, kind, digest, and expected length**. Readers verify the length, domain-separated digest, grammar version, identity, and nested references before interpretation. Checksums are not substitute authorization. The same digest at another epoch is a distinct storage name.
...
There is one configurable finite tail envelope in count **and bytes**. Initial policy values are 4,096 decisions and 64 MiB; these are tunable recovery/latency bounds, not correctness constants or tenant-size limits. Start checkpointing well before either bound. When admission would exceed the envelope, return `MaintenanceRequired`/backpressure until a checkpoint advances. This includes no-op/rejection decisions and large receipt results. It is preferable to an unbounded replay promise or a silent accepted write that makes recovery impossible within host resources.
