...
Keep LMDB. Keep final-state relational admission. Keep genuine set answers. Remove representation accidents that force applications to understand internal allocation, dictionaries, partial output, or a particular memory-size regime.

The core knows **facts, laws, queries, canonical values, transactions, and ordinary LMDB snapshots**. It does not know S3, HEAD, commands, receipt epochs, migration histories, backup retention, or business-generated identities. Those belong to `bumbledb-log`, including its local-history adapter. A dependency boundary, not a naming convention, enforces this split.

Public language scope is deliberate: the **core** retains Rust, TypeScript, and C APIs; **the log product is TypeScript-only in 1.0**, backed by one internal Rust implementation. Rust log types and worker transitions below describe internal implementation boundaries, not a promised public Rust/C log SDK. The local-history convenience mentioned here is likewise part of that TypeScript log surface.
...
7. Value creation does not silently make recursion an unbounded programming language. The supported recursive fragment remains finite active-domain, linear recursion; arithmetic-producing expressions stay outside its recursive feedback path.
...
`CheckedDelta` owns its bytes, schema identity, and layout. Its constructors establish widths, canonical bool/float values, UTF-8, interval bounds, and the absence of dangling internal references. It does not claim the proposed facts satisfy the schema laws; that is the next boundary.
...
Retain discrete `Interval<U64>` and `Interval<I64>`, half-open and nonempty, with the existing uniform two-word representation. Their **represented temporal points exclude the maximum integer**, which is reserved as the ray endpoint. Ordinary integer fact fields still have their full integer domain. State that distinction in public type documentation rather than suggesting every integer can be contained in an interval.
...
An explicit `Finite | Unbounded` endpoint could recover the last integer as a temporal point, but adds a representation arm and changes every temporal kernel/proof. The audit found a construction-boundary defect, not evidence that applications need that last point. **Do not make that expansion in 1.0 merely because breaking changes are allowed.** Preserve the compact interval model and its openly named boundary; revisit it only for a demonstrated application requirement.

Float intervals, approximate float capacity weights, locale collation, nullable/three-valued SQL semantics, decimal, and arbitrary user-defined execution kernels are **not** smuggled into 1.0. Floating values are first-class facts/keys/joins/arithmetic/aggregates; temporal/capacity measures remain exact nonnegative integral measures. These are language boundaries, not a claim that floats cannot be stored or queried.
...
The current `fact_hash` explicitly makes hash equality fact equality. 1.0 removes that logical axiom. Hashes select candidate buckets; full canonical tuple bytes decide equality. A membership index can use `(relation, digest, local-row-id)` and obtain candidate row values before exact comparison. The same rule applies to hashed large determinants, grouping, and deduplication.
...
Canonical logical export orders by relation and tuple digest, then full canonical bytes within a digest collision bucket. Physical row IDs never enter its logical identity. For a deliberately adversarial enormous collision bucket, a repeated bounded-memory minimum scan is an acceptable slow fallback; no unbounded in-memory collision list is required. A future faster collision sorter must preserve this behavior. Cryptographic snapshot integrity still assumes the chosen digest's collision resistance; **logical tuple equality does not**.
...
Fresh allocation moves out of the core. There is no application-visible `reserve<u64>` whose crash safety depends on `Drop`. The log command builder owns `FreshRef` placeholders; a winning decision resolves them to its proposed 28-byte entity identity `(IncarnationId128, DecisionSeq64, Ordinal32)`. The core sees a nominal `Bytes<28>` value and an ordinary key. The receipt returns stable IDs only after publication. Applications needing an offline external identifier supply one explicitly. Ordinary local core users likewise supply their facts' identifiers; the log's local-history adapter supplies the same fresh convenience without S3.
...
| ENG-004/007; REP-004/006; SDK-015 | Remove core fresh reservations; log-owned placeholders/receipt identity | `E-NO-RESERVE`: downstream old reserve API absent; log gate kills process at every placeholder/resolution/publication boundary; no prepublication ID escape or callback replay |
| ENG-005 | Judge a proposed multimap before unique index installation | `E-ADMIT`: original two refused fresh-key rows sharing email plus permutations report both statement IDs; compare naive and disk candidate judgments |
