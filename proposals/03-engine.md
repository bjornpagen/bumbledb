...
Error renames (`error.rs`), census-scanned: `01-language.md` table. The roster names are `ValidationError`; `MeasureOfRay` / `ResultBytesOverflow` live on runtime `Error` and stay there. `FixpointBudgetExceeded` is **renamed `DerivedBudgetExceeded`** — the resource abort vs `evalQuery`, no longer fixpoint-scoped: one derived-tuples ledger prices every derived sink (each interior's emits, then the rec's table), judged after each interior and between rec rounds; the rounds axis is rec-only. Payload drops `stratum`: `{ rounds, tuples }` (`rounds: 0` on a preamble trip). One rec SCC, no strata. The old name would lie on an interiors-only trip — no fixpoint ran.
...
struct PreparedQuery {
    ...
    // ... scratch, schema pin, rendered, budget (tuples axis always judged;
    //     rounds axis inert when rec is None — rounds never advance)
}
...
3. Execute: for each interior, `run_rules` into its projection sink, transpose to `TransientImage` (`synthesize`/`refill` from seen-set rows — today's interior-predicate image), bind that image for later interiors and main `Interior` occurrences (`Colt::reset`, bypass `memo.bind`, recycle `spare_buffers` — the Idb arm in `run_join`, renamed Interior). After each interior finishes, add its seen-set len to the derived-tuples ledger and judge the budget — one `usize` read per interior, today's per-round check relocated; tripping is `DerivedBudgetExceeded { rounds: 0, tuples }`. Then `run_rules` main into the head-owned sink. Ray probes: each interior is an ordinary query — probe after that interior finishes; main probes after main. No deferred-Idb carve-out.
...
- Stop: empty Δ. That is Lean `reachStep acc ⊆ acc` / `T(acc) ⊆ acc` (`02-lean.md`). Budget: values stay (2¹⁶ rounds, 10⁷ tuples); spellings follow the module rename — `DEFAULT_REACH_ROUNDS`, `DEFAULT_DERIVED_TUPLES`. The tuples axis is **derived-wide** (`01-language.md` § No `MAX_CTES`): interior emits enter the ledger before round 0 does, so the rec inherits whatever the preamble spent. Size is the wall, not round count. `set_fixpoint_budget` is renamed `set_derived_budget(rounds, tuples)` on `PreparedQuery`; the rounds axis is inert when `rec` is `None` (rounds never advance), the tuples axis judges every query — no rec-`None` special case, hosts copy-paste. Granularity is honest and unchanged in class: the check runs between units of work (after an interior, between rounds), the same coarseness as today's per-round check — it is a typed ceiling, not a hard memory cap within one unit.
...
| `querygen/shapes_recursive.rs` | Rewrite to emit `Query` (interiors + optional Rec + identity main). Drop mutual and nonlinear variants. Keep linear, main-negation of finished rec, main fold, empty-Δ, budget-trip, primer-shaped `reach(x,x)`. **Add an interiors-heavy family**: deep DAG chains (interior reading interior), anti-joins of earlier interiors, many-interiors (> 16), and a preamble ledger-trip (`DerivedBudgetExceeded { rounds: 0 }`) — the interior preamble and the ledger are new paths and get generator coverage, not only unit locks |
...
`ir/validate/tests/program.rs` → `interior.rs` / `rec.rs`. Mutual / SCC / `UnresolvedPredicateSignature` / `UnknownOutputPredicate` tests become refusals or deletions (`04`). `a_degenerate_program_executes_as_its_query` → `a_plain_query_executes_as_today`. `prepare_executes_recursion_under_the_driver` → reach driver. `a_tight_fixpoint_budget_trips_with_the_typed_error` → `a_tight_derived_budget_trips_under_reach` (rec; `DerivedBudgetExceeded`, `rounds > 0`). A new lock: `a_tight_tuple_budget_trips_on_an_interiors_only_query` — `DerivedBudgetExceeded { rounds: 0, .. }`, no reach driver built; a tight **rounds** budget alone must *not* trip it. A new lock: `an_interiors_only_query_does_not_enter_reach` — assert `!matches!(body, PreparedBody::Reach(_))` (interiors-only is `Rules` or `Empty`). Budget methods remain on every `PreparedQuery`: the tuples axis judges every query, the rounds axis is inert when `rec` is `None` (hosts copy-paste). A new lock: many interiors (more than 16) still validate if each list respects `MAX_RULES` — the `MAX_CTES` corpse must not return as `TooManyCtes`.
