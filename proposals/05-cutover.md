...
`query!`, TS, C++, napi marshal, C++ foreign view, `translate_query`, naive eval, conformance corpus builder (`reach-*.json`), querygen `Query` shapes including the interiors-heavy family (`03`), cookbook tests, primer-shaped `reach(x,x)` lock. `04`. Fingerprint file regenerated. C++ recipe parity. **Now** the generator may overwrite `reach-*.json` — it emits the Lean Reach shape, not `predicates`/`output`.

Sugar caps stay sugar (`max_query_rules = 4` — a cap on rules). Named interiors are uncapped at **every** layer: no `max_interiors` in C++ or TS, variadic builders, sugar mirrors the engine. Engine: `MAX_RULES = 16`, rec pools `MAX_RULES` across `base`+`rec`, **no interior-count cap**.

### 5. Leftover Program — grep-clean

```
rg 'Program|PredId|PredicateDef|AtomSource::Idb|AtomSource::idb|validate_program|render_program|evalProgram|degenerate_embedding|MAX_PREDICATES|MAX_CTES|TooManyCtes|ProgramRef|FixpointProgram|ReachProgram|bdb_program|strata\.rs|\.idb\(|STRATUM|VALIDATE_STRATIFY|StratumStats|DeltaRows|AggregationInRecCte|RecCte|CteId|WithDef|Query\.with\b|FixpointBudgetExceeded|set_fixpoint_budget|DEFAULT_FIXPOINT' \
  --glob '!proposals/**'
```
...
- Interior tables materializing outside the derived-tuples ledger (the budget is not rec-gated)
...
2. **Language.** `query!` all-bare still compiles. Named head without `interior`/`recursive` is a compile error. Cookbook 24–25 native forms use `recursive`. The primer-shaped `reach(x,x)` lock passes (the Primer repo's own recut is a filed issue on their cadence, not a gate). TS `program`/`rec`/`output` gone. C++ `bdb::program`/`rec<>`/`output` gone. C ABI `bdb_program` gone.
3. **Engine.** `prepare(&Query)` only. Interiors-only prepared body is `PreparedBody::Rules` or `Empty`, never `Reach`. Rec path: `ReachDriver`, one `DeltaVariant` per rec arm, watermark Δ, `TransientImage`, existing rule loop. **One engine:** interiors, rec rounds, and main all execute through the same `run_rules` / `run_join` / Free Join pipeline; `run_reach` owns a loop, a watermark, and a budget — no join code of its own. Naive / SQLite / Lean stay oracles, not engines. **One ledger:** `DerivedBudgetExceeded` (né `FixpointBudgetExceeded`) prices every derived table — interior emits and rec rounds on one derived-tuples budget, rounds axis rec-only — incompleteness vs `evalQuery`, not vs Lean fuel; a cross-product interior trips it at `rounds: 0` instead of OOMing. **No `MAX_CTES`.** Rec pools `MAX_RULES`. More than 16 interiors validate. No `strata.rs`. `Db::render_program` gone. `obs::STRATUM` gone. No 16-slot interior span array.
...
- **Named interior of a finished rec** (inlining equivalent — up to the caps: unfolding a k-rule view into m main rules is k·m conjunctive rules against main's `MAX_RULES` / DNF pool, so a wide today-legal view over a rec can have no this-cut image). This cut inlines into main. *Trigger:* two main-shaped queries over one rec that want to share a named projection without a second prepare, **or** an inline image that exceeds the main pool — still not a second SCC.
- **Nonlinear rec** (`P(x,z) ← P(x,y), P(y,z)`). Semi-naive still agrees; it is a worse TC algorithm at 10⁷ / 10 ms (k FJ plans × |Acc|). *Trigger:* a measured L-scale query where the linear encoding is unnatural **and** `|Δ ⋈ Acc|` still fits 10 ms / `DEFAULT_DERIVED_TUPLES`. Not “SQLite grew a second reference.”
...
- Inventing `MAX_CTES` / `MAX_INTERIORS` as a product cap — at any layer, sugar included
...
- Making `DerivedBudgetExceeded` a Lean error or putting fuel back in `reachDen`
