...
| D3 | Success-path error contexts: the per-cell `format!` in `schema_value` (marshal.rs:199), per-cell in `fact_columns` (dies with D2), per-row in `one_fact_row`, fact-lane `req_at` contexts | `ts/crate/src/marshal.rs` | V3 | error text built on the error arm only — byte-identical messages | `alloc-counter` window: zero allocation per success cell in the marshal span; error-message golden tests unchanged |
| D4 | Per-call sealed-roster re-derivation (`sealed_fields`'s fresh `Vec<(Box<str>, ValueType)>`) | `ts/crate/src/marshal.rs::sealed_fields` | V4 | the roster resident on `Sealed`, borrowed per call | alloc window: zero roster allocation per insert call |
| D5 | The engine's second parse: `ParsedRow`, its `Box<[ParsedCell]>` per row, `parse_dyn_collection` | `crates/bumbledb/src/api/db/encode_dyn.rs`, `mutation_core.rs` | V1 | the accepted collection's borrowed cell views; ONE parse implementation, in its one constructor (20) | symbols absent; alloc window: zero per-row allocation between constructor and `encode_fact` |
