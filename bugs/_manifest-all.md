...
Deleted on verification: 110, 118 ([`_rebuttals-ffi.md`](_rebuttals-ffi.md)); 305, 306 ([`_rebuttals-general.md`](_rebuttals-general.md)). New: 308.
...
| 108 | medium | confirmed | 108-slice-in-count-overflow.md | `slice_in` / `from_raw_parts` does not reject `count*size` overflow or misalignment. |
| 109 | medium | confirmed | 109-napi-take-handle-refcell-panic.md | `take_handle` uses panicking `borrow_mut`; re-entrant close unwinds into napi. |
...
| 116 | medium | confirmed | 116-tx-ref-mut-from-ref-aliasing.md | `transaction(&self) -> &mut WriteTx` plus non-atomic `alive`; cross-thread callback use is UB. |
...
| 210 | medium | confirmed | 210-measure-of-ray-not-the-only-runtime-error.md | 70-api omits `MeasureOfRay`; “one runtime type error” slogan hides write-path `CapacityRayMeasure`. |
...
| 221 | medium | confirmed | 221-negated-complement-fold-unmodeled.md | Grounding overview cites Lean theorems for evaluation; Lean leaves `fold_negated` unmodeled. |
| 224 | medium | confirmed | 224-membership-lowering-excludes-negated.md | Named `membership_lowering_preserves` requires membership-free negation; engine runs `AntiProbe`. |
...
| 308 | medium | confirmed | 308-citation-decode-panics-on-short-fact.md | `fact_by_row` skips `check_width`; citation decode panics on a short cited `F` value. |
| 107 | low | confirmed | 107-unguarded-extern-panic-wall.md | Several externs skip `catch_unwind`; panic into `-fno-exceptions` C++ is UB. (No current panic site.) |
| 113 | low | confirmed | 113-cpp-exception-through-rust-callback.md | A throwing C++ callback unwinds through Rust; `catch_unwind` cannot catch it. (In-tree `-fno-exceptions`.) |
...
| 225 | low | confirmed | 225-origin-and-result-bytes-overflow.md | `OriginCapacity` / `ResultBytesOverflow` abort queries Lean still denotes; 70-api/40-execution understate. |
...
**By severity:** critical 1, high 10, medium 25, low 11, info 2. **Total 49.**

**By area (id range):** FFI 17 (100–109, 111–117), spec 26 (200–225), general 6 (301–304, 307–308).

**By confidence:** confirmed 49.

**By auditor area field:** ffi 17, spec-docs-rust 26, correctness 4, persistence 1, other 1.
