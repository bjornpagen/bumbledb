 ...
 *                                      with EXPLICITLY supplied Uuids
 ...
 */
import { bool, contained, Uuid, uuid, key, on, relation, Scalar, schema, str, u64 } from "@bjornpagen/bumbledb"
import { backfill, migrationIntent, renameField, seed } from "@bjornpagen/bumbledb-log/schema"
import { Result } from "effect"

function idOf(hex: string): Uuid {
	const parsed = Uuid.parse(hex)
	if (Result.isFailure(parsed)) {
		throw new Error(`evolution-stages: invalid Uuid literal ${hex}`)
	}
...
// Seed identity is EXPLICIT application-owned Uuid data: a migration can
// never call an ID generator per row.

export const Tag2 = relation("Tag", { id: uuid, name: str })
export const Note2 = Note1
export const App2 = schema("App", { Note: Note2, Tag: Tag2 }, [key(Note2, ["id"]), key(Tag2, ["id"])])

export const tagSeeds = [
	{ id: idOf("00000000-0000-0000-0000-000000000001"), name: "inbox" },
	{ id: idOf("00000000-0000-0000-0000-000000000002"), name: "archive" }
] as const
...
export const Note3 = relation("Note", { id: uuid, text: str, pinned: bool })
...
export const Outbox4 = relation("Outbox", { id: uuid, note: uuid, kind: str })
