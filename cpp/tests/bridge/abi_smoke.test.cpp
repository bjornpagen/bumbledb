...
auto consume_error(abi::bdb_error* error) -> std::string {
    ...
    if (abi::bdb_error_get_message(error, &message)
        == abi::bdb_status::BDB_STATUS_OK) {
...
auto read_fingerprint(std::string_view store_path)
    -> std::expected<std::array<std::uint8_t, 64>, std::string> {
    ...
    auto const fields = std::array{
        ...
        abi::bdb_field_spec{
            ...
            .value_type = scalar_type(
                abi::bdb_value_type_kind::BDB_VALUE_TYPE_KIND_STRING
            ),
            .newtype = absent_view(),
            .fresh = false,
        },
    ...
    auto const created = abi::bdb_db_ephemeral(
        view_of(path_bytes), &spec, &database, &error
    );
    if (created != abi::bdb_status::BDB_STATUS_OK) {
        return std::unexpected(
            std::format("bdb_db_ephemeral failed: {}", consume_error(error))
        );
    }
    ...
    if (read != abi::bdb_status::BDB_STATUS_OK) {
        ...
        return std::unexpected(
            std::format("bdb_db_fingerprint failed: {}", detail)
        );
    }
...
auto make_store_dir() -> std::expected<std::filesystem::path, std::string> {
    ...
    auto const dir = root
        / std::format("bumbledb-abi-smoke-{:08x}{:08x}", device(), device());
...
auto run_cases() -> std::vector<CaseResult> {
    ...
    if (!fingerprint) {
        return {CaseResult{
            .name = std::format(
                "create/fingerprint/destroy succeed ({})", fingerprint.error()
            ),
            .passed = false,
        }};
    }
    return {
        ...
        CaseResult{
            ...
            .passed = std::ranges::any_of(
                *fingerprint,
                [](std::uint8_t character) { return character != '0'; }
            ),
        },
    };
}
