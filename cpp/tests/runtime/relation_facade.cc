...
auto run_cases() -> std::array<CaseResult, 4> {
    return std::array{
        CaseResult{
            ...
            .passed = Service.id.ordinal == 0 && Service.name.ordinal == 1
                && Outage.service.ordinal == 0
                && Outage.window.ordinal == 1,
        },
        CaseResult{
            .name = "name hooks render relation and field names",
            .passed = Service.id.relation() == "Service"
                && Service.id.field() == "id"
                && Outage.window.relation() == "Outage"
                && Outage.window.field() == "window",
        },
        CaseResult{
            .name = "fresh flag marks exactly the annotated u64",
            .passed = Service.id.fresh && !Service.name.fresh
                && !Outage.service.fresh && !Outage.window.fresh,
        },
        CaseResult{
            .name = "kinds classify per the closed value vocabulary",
            .passed = Service.id.kind == bdb::value_kind::u64
                && Service.name.kind == bdb::value_kind::string
                && Outage.window.kind == bdb::value_kind::interval_i64
                && Kinds.digest.fixed_len == 16,
        },
    };
}
