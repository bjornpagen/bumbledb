...
//
// The schema is built through the PRE-SCHEMA LANE (bumbledb.foreign.raii's
...
//
// GCC-only: imports the umbrella module, whose graph reaches meta/.
import std;
import bumbledb;
...
auto make_uptime_spec() -> abi::owned_schema_spec {
        ...
        .fields =
            {
                abi::owned_field{
                    .name = "id",
                    .value_type = abi::scalar_type(
                        abi::bdb_value_type_kind::BDB_VALUE_TYPE_KIND_U64),
                    .newtype = "Service.id",
                    .fresh = true,
                },
                abi::owned_field{
        ...
        .fields =
            {
                ...
                abi::owned_field{
                    .name = "window",
                    .value_type = abi::interval_type(
                        abi::bdb_interval_element::
    ...
    statements.push_back(abi::owned_containment{
        .source = abi::owned_side{
            .relation = "Outage", .projection = {"service"}},
        .target = abi::owned_side{
            .relation = "Service", .projection = {"id"}},
        .bidirectional = false,
    });
    ...
    return abi::owned_schema_spec{
        std::move(relations), std::move(statements)};
}
...
auto make_store_dir(std::string_view label)
    ...
    if (code) {
        return std::unexpected(
            std::format("temp_directory_path: {}", code.message()));
    }
    auto device = std::random_device{};
    auto const dir = root
        / std::format("bumbledb-runtime-{}-{:08x}{:08x}", label, device(),
            device());
    ...
    if (code) {
        return std::unexpected(
            std::format("create_directories: {}", code.message()));
    }
    return dir;
}
...
auto insert_outage_body(std::uint64_t service_id,
    bdb::interval<std::int64_t> window) {
    return [service_id, window](bdb::WriteTx& tx) -> UnitResult {
        return tx
            .insert(Outage,
                OutageRow{.service = service_id, .window = window})
            .transform([](bool) -> UnitDecision { return bdb::commit(); });
    };
}
...
auto fingerprint_case(bdb::Db const& db) -> CaseResult {
    ...
    return CaseResult{
        ...
        .passed = fingerprint.has_value() && fingerprint->size() == 64
...
auto insert_service_case(bdb::Db& db, std::vector<CaseResult>& results)
    ...
    auto written = db.write([](bdb::WriteTx& tx) -> IdResult {
        return tx.alloc(Service.id)
            .and_then([&tx](std::uint64_t id) -> IdResult {
                return tx
                    .insert(Service,
                        ServiceRow{.id = id, .name = std::string{"search"}})
                    .transform([id](bool changed) -> IdDecision {
                        return changed
                            ? IdDecision{bdb::commit(id)}
                            : IdDecision{bdb::abandon(std::monostate{})};
                    });
            });
    });
...
auto read_contains_case(bdb::Db const& db, std::uint64_t service_id)
    -> CaseResult {
    auto const seen = db.read(
        [service_id](bdb::Snapshot& snap) -> std::expected<bool, bdb::Error> {
            return snap.contains(Service,
                ServiceRow{.id = service_id, .name = std::string{"search"}});
        });
    return CaseResult{
...
auto scan_case(bdb::Db const& db, std::uint64_t service_id) -> CaseResult {
    ...
    auto const scanned = db.read(
        [service_id](
            bdb::Snapshot& snap) -> std::expected<ScanFacts, bdb::Error> {
            return snap.scan(Service).transform([&](bdb::RowSet rows) {
                ...
                return ScanFacts{
                    .rows = rows.len(),
                    .arity = rows.arity(0),
                    .id_matches = id_cell.has_value()
                        && std::holds_alternative<std::uint64_t>(*id_cell)
                        && std::get<std::uint64_t>(*id_cell) == service_id,
                    .name_matches = name_cell.has_value()
                        && std::holds_alternative<std::string_view>(
                            *name_cell)
                        && std::get<std::string_view>(*name_cell)
                            == "search",
                };
            });
        });
    return CaseResult{
        ...
        .passed = scanned.has_value() && scanned->rows == 1
            && scanned->arity == 2 && scanned->id_matches
            && scanned->name_matches,
    };
}
...
auto write_from_case(bdb::Db& db, std::uint64_t service_id) -> CaseResult {
    ...
    auto const nested = db.read(
        [&](bdb::Snapshot& snap) -> std::expected<bool, bdb::Error> {
            return db
                .write_from(snap, insert_outage_body(service_id, window))
                .transform([](auto outcome) {
                    return std::holds_alternative<UnitCommitted>(outcome);
                });
        });
    auto const stored = db.read(
        [&](bdb::Snapshot& snap) -> std::expected<bool, bdb::Error> {
            return snap.contains(Outage,
                OutageRow{.service = service_id, .window = window});
        });
    return CaseResult{
        .name = "write_from inside read commits an interval-bearing row (§18)",
        .passed = nested.has_value() && *nested && stored.has_value()
            && *stored,
    };
}
...
auto generation_moved_case(bdb::Db& db, std::uint64_t service_id)
    -> CaseResult {
    ...
    auto const observed = db.read(
        [&](bdb::Snapshot& snap) -> std::expected<bool, bdb::Error> {
            return db
                .write_from(
                    snap, insert_outage_body(service_id, first_window))
                .and_then([&](auto first) -> std::expected<bool, bdb::Error> {
                    if (!std::holds_alternative<UnitCommitted>(first)) {
                        return false;
                    }
                    auto second = db.write_from(
                        snap, insert_outage_body(service_id, second_window));
                    if (second.has_value()) {
                        return false;
                    }
                    auto const payload = second.error().generation_moved();
                    return second.error().kind()
                        == bdb::ErrorKind::GenerationMoved
                        && payload.has_value()
        ...
        return tx.alloc(Service.id)
            .and_then([&](std::uint64_t id) -> std::expected<bool, bdb::Error> {
                ...
                return tx.insert(Service,
                    ServiceRow{.id = id, .name = std::string{"ghost"}});
            })
            .and_then([&tx](bool) -> UnitResult {
                // One value against two sealed fields: FactShape.
                return tx.insert(Service, ShortServiceRow{.id = 1})
                    .transform(
                        [](bool) -> UnitDecision { return bdb::commit(); });
            });
    });
    auto const fact_shape = !aborted.has_value()
        && aborted.error().kind() == bdb::ErrorKind::FactShape;
    auto const ghost_absent = ghost_id.has_value()
        && db.read([&](bdb::Snapshot& snap) -> std::expected<bool, bdb::Error> {
                return snap
                    .contains(Service,
                        ServiceRow{
                            .id = *ghost_id, .name = std::string{"ghost"}})
                    .transform([](bool present) { return !present; });
            }).value_or(false);
    return {
        CaseResult{
...
auto abandon_case(bdb::Db& db) -> std::vector<CaseResult> {
    ...
    auto outcome = db.write(
        [&maybe_id](bdb::WriteTx& tx) -> std::expected<Decision, bdb::Error> {
            return tx.alloc(Service.id)
                .and_then(
                    [&](std::uint64_t id) -> std::expected<bool, bdb::Error> {
                        maybe_id = id;
                        return tx.insert(Service,
                            ServiceRow{
                                .id = id, .name = std::string{"maybe"}});
                    })
                .transform([](bool) -> Decision {
                    return bdb::abandon(std::string{"not today"});
                });
        });
    auto const carried = outcome.has_value()
        && std::holds_alternative<bdb::Abandoned<std::string>>(*outcome)
        && std::get<bdb::Abandoned<std::string>>(*outcome).value
            == "not today";
    auto const absent = maybe_id.has_value()
        && db.read([&](bdb::Snapshot& snap) -> std::expected<bool, bdb::Error> {
                return snap
                    .contains(Service,
                        ServiceRow{
                            .id = *maybe_id, .name = std::string{"maybe"}})
                    .transform([](bool present) { return !present; });
            }).value_or(false);
    return {
        CaseResult{
...
auto commit_rejection_case(bdb::Db& db) -> CaseResult {
    ...
    auto rejected =
        db.write(insert_outage_body(orphan_service, window));
    auto const shaped = !rejected.has_value()
        && rejected.error().kind() == bdb::ErrorKind::CommitRejected;
    if (!shaped) {
        return CaseResult{
    ...
    auto const complete = !violations.empty()
        && std::ranges::any_of(violations, [](bdb::Violation const& cited) {
               return cited.kind == bdb::StatementKind::Containment
                   && !cited.spelling.empty();
           })
...
auto reentrant_write_case(bdb::Db& db) -> CaseResult {
    ...
    auto outer = db.write([&](bdb::WriteTx&) -> UnitResult {
        auto inner = db.write(
            [](bdb::WriteTx&) -> UnitResult { return bdb::commit(); });
        if (!inner.has_value()) {
    ...
    return CaseResult{
        ...
        .passed = outer.has_value() && inner_kind.has_value()
            && *inner_kind == bdb::ErrorKind::EnvironmentLocked,
    };
}
...
auto tx_lanes_case(bdb::Db& db) -> CaseResult {
    ...
    auto outcome = db.write([&](bdb::WriteTx& tx) -> UnitResult {
        return tx.alloc(Service.id)
            .and_then([&](std::uint64_t id) -> std::expected<bool, bdb::Error> {
                auto const row =
                    ServiceRow{.id = id, .name = std::string{"temp"}};
                return tx.insert(Service, row)
                    .and_then([&](bool) { return tx.contains(Service, row); })
                    .and_then([&](bool seen) {
    ...
    return CaseResult{
        ...
        .passed = outcome.has_value() && seen_after_insert
            && removal_changed && !seen_after_remove,
    };
}
...
auto move_case(bdb::Db db) -> CaseResult {
    ...
    return CaseResult{
        ...
        .passed = !db.alive() && target.alive()
            && target.fingerprint().has_value(),
    };
}
...
auto durable_case(abi::owned_schema_spec const& spec)
    ...
    auto stored_id = std::optional<std::uint64_t>{};
    {
        ...
        if (!created.has_value()) {
            return {CaseResult{
                .name = std::format(
                    "durable create failed: {}", created.error().message()),
                .passed = false,
            }};
        }
    ...
    auto const survived = reopened.has_value()
        && reopened
            ->read([&](bdb::Snapshot& snap) -> std::expected<bool, bdb::Error> {
                return snap.contains(Service,
                    ServiceRow{
                        .id = *stored_id, .name = std::string{"search"}});
            })
            .value_or(false);
    auto code = std::error_code{};
    std::filesystem::remove_all(*dir, code);
...
auto run_cases() -> std::vector<CaseResult> {
    ...
    if (!opened.has_value()) {
        results.push_back(CaseResult{
            .name = std::format(
                "ephemeral create failed: {}", opened.error().message()),
            .passed = false,
        });
        return results;
    }
