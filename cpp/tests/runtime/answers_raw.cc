...
auto check_move_leaves_the_source_inert() -> CaseResult {
    ...
    auto const source_inert = !source.alive()
        && source.len() == 0 && source.arity() == 0
        && !source.cell(bdb::Cell{.row = 0, .column = 0}).has_value();
    return CaseResult{
...
auto main() -> int {
    auto const results = std::array{
        check_fresh_carrier_is_empty(),
        check_cell_is_bounds_checked(),
        check_clear_keeps_the_carrier_usable(),
        check_move_leaves_the_source_inert(),
        check_move_assign_releases_and_adopts(),
    };
