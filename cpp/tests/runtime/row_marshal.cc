...
[[nodiscard]] auto check_outage_lowers_to_u64_interval_i64() -> CaseResult {
	...
	return CaseResult{
	    ...
	    .passed = service_cell.kind == abi::bdb_value_kind::BDB_VALUE_KIND_U64 && service_cell.u64_value == 7 &&
	              window_cell.kind == abi::bdb_value_kind::BDB_VALUE_KIND_INTERVAL_I64 && window_cell.interval_i64_start == -5 &&
...
[[nodiscard]] auto check_service_string_cell_borrows_the_row() -> CaseResult {
	...
	auto const borrows_row = name_cell.string_value.data == std::bit_cast<std::uint8_t const*>(service.name.data());
	return CaseResult{
	    .name = "ServiceRow lowers to [U64, String] borrowing row storage",
	    .passed = id_cell.kind == abi::bdb_value_kind::BDB_VALUE_KIND_U64 && id_cell.u64_value == 1 &&
	              name_cell.kind == abi::bdb_value_kind::BDB_VALUE_KIND_STRING && borrowed == service.name && borrows_row,
	};
}
...
[[nodiscard]] auto check_bool_and_bytes_cells() -> CaseResult {
	...
	auto const borrows_row = tag_cell.bytes_value.data == std::bit_cast<std::uint8_t const*>(tagged.tag.data());
	return CaseResult{
	    .name = "TaggedRow lowers to [Bool, FixedBytes(4)] borrowing row",
	    .passed = live_cell.kind == abi::bdb_value_kind::BDB_VALUE_KIND_BOOL && live_cell.bool_value &&
	              tag_cell.kind == abi::bdb_value_kind::BDB_VALUE_KIND_FIXED_BYTES && tag_cell.bytes_value.len == 4 && borrows_row,
	};
}
