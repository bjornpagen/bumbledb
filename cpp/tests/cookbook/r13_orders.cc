...
inline constexpr auto State =
    bdb::closed<"State", "Cart", "Placed", "Shipped">();
...
inline constexpr auto Orders = bdb::schema<"Orders">(
    State,
    Order,
    Placement,
    Shipment,
    ...
    // together.
    bdb::mirrors(
        bdb::on(Shipment.order),
        bdb::on(bdb::where(Order, {.state = State.Shipped}), Order.id)
    )
);

// The handle literal in the match record + the join into the arm.
inline constexpr auto Shipped =
    bdb::query(Orders).rule([](auto r) consteval {
        auto o = r.vars(Order);
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r13-{:08x}{:08x}", device(),
            device());
...
auto seed(bdb::Db& db) -> std::optional<SeedIds> {
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        ...
        auto const mint = [&](std::uint64_t& out)
            ...
            return tx.alloc(Order.id).transform(
                [&](std::uint64_t minted) {
                    out = minted;
                    return true;
                });
        ...
        auto rows_land = mint(ids.cart)
            .and_then([&](bool) { return mint(ids.shipped); })
            .and_then([&](bool) {
                return tx.insert(Order,
                    OrderRow{.id = ids.cart, .state = State.Cart});
            })
            .and_then([&](bool) {
                return tx.insert(Order,
                    OrderRow{.id = ids.shipped, .state = State.Shipped});
            })
            .and_then([&](bool) {
                return tx.insert(Placement,
                    PlacementRow{.order = ids.shipped, .at = 5});
            })
            .and_then([&](bool) {
                return tx.insert(Shipment,
                    ShipmentRow{.order = ids.shipped,
                        .carrier = std::string{"ups"},
                        .at = 9});
            });
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r13") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r13 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r13 store directory", .passed = false});
        return;
    }
    ...
    if (!db.has_value()) {
        ...
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Orders", .passed = false});
        return;
    }
    ...
    auto unevidenced = db->write([&](bdb::WriteTx& tx)
            -> std::expected<
                bdb::WriteDecision<std::monostate, std::monostate>,
                bdb::Error> {
        auto landed = tx.alloc(Order.id).and_then(
            [&](std::uint64_t order) {
                return tx.insert(Order,
                    OrderRow{.id = order, .state = State.Shipped});
            });
        if (!landed.has_value()) {
    ...
    results.push_back(CaseResult{
        .name = "a Shipped order without its Shipment is commit-rejected "
                "(totality)",
        .passed = !unevidenced.has_value()
            && unevidenced.error().kind()
    ...
    auto premature = db->write([&](bdb::WriteTx& tx)
            -> std::expected<
                bdb::WriteDecision<std::monostate, std::monostate>,
                bdb::Error> {
        auto landed = tx.insert(Shipment,
            ShipmentRow{.order = ids->cart,
                .carrier = std::string{"dhl"},
                .at = 11});
        if (!landed.has_value()) {
            return std::unexpected{std::move(landed).error()};
        }
    ...
    results.push_back(CaseResult{
        .name = "a Shipment referencing a Cart order is commit-rejected "
                "(validity)",
        .passed = !premature.has_value()
            && premature.error().kind() == bdb::ErrorKind::CommitRejected
            && !premature.error().violations().empty(),
    });
    ...
    results.push_back(CaseResult{
        ...
        .passed = answers.has_value() && answers->size() == 1
            && answers->rows().front().id == ids->shipped
