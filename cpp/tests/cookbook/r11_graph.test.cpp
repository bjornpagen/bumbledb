...
inline constexpr auto Graph = bdb::schema<"Graph">(
    ...
    // Repo.
    bdb::contained(
        bdb::on(Follows.follower),
        bdb::on(Person.id)
    ),
    bdb::contained(
        bdb::on(Follows.followee),
        bdb::on(Person.id)
    ),
    ...
    bdb::contained(
        bdb::on(Maintains.person),
        bdb::on(Person.id)
    ),
    bdb::contained(
        bdb::on(Maintains.repo),
        bdb::on(Repo.id)
    ),
...
inline constexpr auto Mutual =
    bdb::query(Graph).rule([](auto r) consteval {
        ...
        return r
            ...
            .find({}, bdb::as<"a">(vars.follower),
                bdb::as<"b">(vars.followee));
    });
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r11-{:08x}{:08x}", device(),
            device());
...
auto seed(bdb::Db& db) -> std::optional<SeedIds> {
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        ...
        auto rows_land = mint(Person.id, ids.alice)
            .and_then([&](bool) { return mint(Person.id, ids.bob); })
            .and_then([&](bool) { return mint(Person.id, ids.carol); })
            .and_then([&](bool) { return mint(Repo.id, ids.repo); })
            .and_then([&](bool) {
            ...
            .and_then([&](bool) {
                return tx.insert(Person,
                    PersonRow{.id = ids.bob, .name = std::string{"bob"}});
            })
            .and_then([&](bool) {
                return tx.insert(Person,
                    PersonRow{
                        .id = ids.carol, .name = std::string{"carol"}});
            })
            .and_then([&](bool) {
                return tx.insert(Repo,
                    RepoRow{.id = ids.repo, .name = std::string{"bdb"}});
            })
            .and_then([&](bool) {
                return tx.insert(Follows,
                    FollowsRow{
                        .follower = ids.alice, .followee = ids.bob});
            })
            .and_then([&](bool) {
                return tx.insert(Follows,
                    FollowsRow{
                        .follower = ids.bob, .followee = ids.alice});
            })
            .and_then([&](bool) {
                return tx.insert(Follows,
                    FollowsRow{
                        .follower = ids.alice, .followee = ids.carol});
            })
            .and_then([&](bool) {
                return tx.insert(Maintains,
                    MaintainsRow{.person = ids.alice, .repo = ids.repo});
            });
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r11") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r11 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r11 store directory", .passed = false});
        return;
    }
    ...
    if (!db.has_value()) {
        ...
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Graph", .passed = false});
        return;
    }
    ...
    auto dangling = db->write([&](bdb::WriteTx& tx)
            -> std::expected<
                bdb::WriteDecision<std::monostate, std::monostate>,
                bdb::Error> {
        auto landed = tx.insert(Follows,
            FollowsRow{
                .follower = ids->alice, .followee = 999'999});
        if (!landed.has_value()) {
    ...
    results.push_back(CaseResult{
        ...
        .passed = !dangling.has_value()
            && dangling.error().kind() == bdb::ErrorKind::CommitRejected
            && !dangling.error().violations().empty(),
    });
    ...
    results.push_back(CaseResult{
        ...
        .passed = pairs.has_value() && pairs->size() == 1
            && pairs->rows().front().a == low
            && pairs->rows().front().b == high,
    });
