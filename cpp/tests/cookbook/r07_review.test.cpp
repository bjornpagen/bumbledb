...
inline constexpr auto Kind = bdb::closed<"Kind", KindPayload>(
    bdb::member<"DirectPass">(KindPayload{.mastered = true, .rank = 30}),
...
inline constexpr auto Certificate =
    bdb::relation<"Certificate", CertificateRow>;

inline constexpr auto Review = bdb::schema<"Review">(
    Kind,
    Attempt,
    Certificate,
    ...
    bdb::key(Certificate.attempt),
    bdb::contained(
        bdb::on(Certificate.attempt),
        bdb::on(Attempt.id)
    ),
    ...
    bdb::contained(
        bdb::on(Certificate.kind),
        bdb::on(bdb::where(Kind, {.mastered = true}), Kind.id)
    )
);
...
inline constexpr auto MasteredAttempts =
    bdb::query(Review).rule([](auto r) consteval {
...
auto label(bdb::ref_to<Kind.id> kind) -> std::string {
    switch (kind.row) {
    case Kind.DirectPass.index:
        return std::format(
            "mastered, rank {}", Kind.axioms.DirectPass.rank);
    case Kind.JudgedPass.index:
        return std::format(
            "mastered, rank {}", Kind.axioms.JudgedPass.rank);
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r07-{:08x}{:08x}", device(),
            device());
...
auto seed(bdb::Db& db) -> std::optional<SeedIds> {
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        ...
        auto rows_land =
            tx.insert(Attempt,
                  AttemptRow{.id = *direct, .kind = Kind.DirectPass})
                .and_then([&](bool) {
                    return tx.insert(Attempt,
                        AttemptRow{.id = *judged, .kind = Kind.JudgedPass});
                })
                .and_then([&](bool) {
                    return tx.insert(Attempt,
                        AttemptRow{.id = *failed, .kind = Kind.Failed});
                })
                .and_then([&](bool) {
                    return tx.insert(Certificate,
                        CertificateRow{
                            .attempt = *direct, .kind = Kind.DirectPass});
                });
        if (!rows_land.has_value()) {
            return std::unexpected{std::move(rows_land).error()};
        }
        return bdb::commit(SeedIds{
            .direct = *direct, .judged = *judged, .failed = *failed});
    });
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r07") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r07 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r07 store directory", .passed = false});
        return;
    }
    ...
    if (!db.has_value()) {
        ...
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Review", .passed = false});
        return;
    }
    ...
    results.push_back(CaseResult{
        ...
        .passed = label(Kind.DirectPass) == "mastered, rank 30"
            && label(Kind.JudgedPass) == "mastered, rank 20"
    ...
    auto answered = db->read([&](bdb::Snapshot& snap)
            -> std::expected<std::vector<std::uint64_t>, bdb::Error> {
        return snap.execute(*mastered, {})
            .transform([](bdb::Answers<MasteredAttempts> answers) {
