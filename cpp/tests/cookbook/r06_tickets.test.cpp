...
inline constexpr auto Actionable =
    bdb::query(Tickets).rule([](auto r) consteval {
        auto vars = r.vars(Ticket);
        return r
            .match(Ticket,
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r06-{:08x}{:08x}", device(),
            device());
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r06") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r06 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r06 store directory", .passed = false});
        return;
    }
    ...
    if (!db.has_value()) {
        ...
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Tickets", .passed = false});
        return;
    }
    ...
    results.push_back(CaseResult{
        ...
        .passed = urgent_ids.has_value()
            && *urgent_ids == std::vector{ids->urgent},
    });
    ...
    results.push_back(CaseResult{
        ...
        .passed = actionable_ids.has_value()
            && *actionable_ids == expected_actionable,
    });
