...
inline constexpr auto Playlists = bdb::schema<"Playlists">(
    ...
    bdb::contained(
        bdb::on(Extent.playlist),
        bdb::on(Playlist.id)
    ),
    bdb::contained(
        bdb::on(Slot.playlist),
        bdb::on(Playlist.id)
    ),
    ...
    bdb::mirrors(
        bdb::on(Extent.playlist, Extent.span),
        bdb::on(Slot.playlist, Slot.slot)
    )
);

// Positional access is membership — "what plays at position ?pos".
inline constexpr auto PlayingAt =
    bdb::query(Playlists).rule([](auto r) consteval {
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r09-{:08x}{:08x}", device(),
            device());
...
auto seed(bdb::Db& db) -> std::optional<std::uint64_t> {
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        ...
        auto rows_land = tx.alloc(Playlist.id)
            .and_then([&](std::uint64_t minted) {
                list = minted;
                return tx.insert(Playlist,
                    PlaylistRow{
                        .id = list, .name = std::string{"road trip"}});
            })
            .and_then([&](bool) {
                return tx.insert(Extent,
                    ExtentRow{.playlist = list,
                        .span = bdb::interval<std::uint64_t>::literal(
                            0, 2)});
            })
            .and_then([&](bool) {
                return tx.insert(Slot,
                    SlotRow{.playlist = list,
                        .slot = bdb::interval<std::uint64_t, 1>::literal(
                            0, 1),
                        .track = std::string{"a"}});
            })
            .and_then([&](bool) {
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r09") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r09 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r09 store directory", .passed = false});
        return;
    }
    ...
    if (!db.has_value()) {
        ...
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Playlists", .passed = false});
        return;
    }
    ...
    auto untiled = db->write([&](bdb::WriteTx& tx)
            -> std::expected<
                bdb::WriteDecision<std::monostate, std::monostate>,
                bdb::Error> {
        auto landed = tx.insert(Slot,
            SlotRow{.playlist = *list,
                .slot = bdb::interval<std::uint64_t, 1>::literal(2, 3),
                .track = std::string{"c"}});
        if (!landed.has_value()) {
    ...
    results.push_back(CaseResult{
        ...
        .passed = !untiled.has_value()
            && untiled.error().kind() == bdb::ErrorKind::CommitRejected
            && !untiled.error().violations().empty(),
    });
    ...
    auto at_one = db->execute(*playing_at,
        {.list = *list, .pos = std::uint64_t{1}});
    results.push_back(CaseResult{
        .name = "playingAt(pos: 1) answers {b}",
        .passed = at_one.has_value() && at_one->size() == 1
            && at_one->rows().front().track == "b",
    });
    auto at_zero = db->execute(*playing_at,
        {.list = *list, .pos = std::uint64_t{0}});
    results.push_back(CaseResult{
        .name = "playingAt(pos: 0) answers {a}",
        .passed = at_zero.has_value() && at_zero->size() == 1
            && at_zero->rows().front().track == "a",
    });
    auto past_end = db->execute(*playing_at,
        {.list = *list, .pos = std::uint64_t{5}});
    results.push_back(CaseResult{
    ...
    auto in_play_order = std::vector<Entry>{
        Entry{.slot = bdb::interval<std::uint64_t>::literal(1, 2),
            .track = std::string{"b"}},
        Entry{.slot = bdb::interval<std::uint64_t>::literal(0, 1),
            .track = std::string{"a"}},
    };
    std::ranges::sort(in_play_order,
        bdb::by(&Entry::slot, &Entry::track));
    ...
    results.push_back(CaseResult{
        ...
        .passed = in_play_order.front().track == "a"
            && in_play_order.back().track == "b"
            && reversed.front().track == "b",
    });
