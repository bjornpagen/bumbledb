...
inline constexpr auto State =
    bdb::closed<"State", "Queued", "Running", "Done">();
...
inline constexpr auto Jobs = bdb::schema<"Jobs">(
    State,
    Job,
    Lease,
    ...
    bdb::mirrors(
        bdb::on(Lease.job),
        bdb::on(bdb::where(Job, {.state = State.Running}), Job.id)
    )
);

// update-where's premise — "still Queued" is the witness:
inline constexpr auto StillQueued =
    bdb::query(Jobs).rule([](auto r) consteval {
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r20-{:08x}{:08x}", device(),
            device());
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r20") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r20 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r20 store directory", .passed = false});
        return;
    }
    ...
    if (!db.has_value()) {
        ...
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Jobs", .passed = false});
        return;
    }
    ...
    // own payload out.
    auto outcome = db->write_witnessed(
        [&](bdb::Snapshot& snap, bdb::WriteTx& tx)
            -> std::expected<
                bdb::WriteDecision<std::monostate, std::string>,
                bdb::Error> {
            ...
            for (auto const& row : queued->rows()) {
                auto moved = tx.remove(Job,
                        JobRow{.id = row.id,
                            .state = State.Queued,
                            .payload = std::string{row.payload}})
                    .and_then([&](bool) {
    ...
    auto abandoned = outcome.has_value()
