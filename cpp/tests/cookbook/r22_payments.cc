...
inline constexpr auto Payments = bdb::schema<"Payments">(
    Kind,
    Payment,
    Card,
    Ach,
    ...
    bdb::key(Card.payment),
    bdb::key(Ach.payment),
    bdb::mirrors(
        bdb::on(bdb::where(Payment, {.kind = Kind.Card}), Payment.id),
        bdb::on(Card.payment)
    ),
    bdb::mirrors(
        bdb::on(bdb::where(Payment, {.kind = Kind.Ach}), Payment.id),
        bdb::on(Ach.payment)
    )
);
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r22-{:08x}{:08x}", device(),
            device());
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r22") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r22 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r22 store directory", .passed = false});
        return;
    }
    ...
    if (!db.has_value()) {
        ...
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Payments", .passed = false});
        return;
    }
