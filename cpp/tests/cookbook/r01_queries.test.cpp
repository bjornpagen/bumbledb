...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r01q-{:08x}{:08x}", device(),
            device());
...
auto seed(bdb::Db& db) -> std::optional<SeedIds> {
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        ...
        auto rows_land =
            tx.insert(Service,
                  ServiceRow{.id = *search, .name = std::string{"search"}})
                .and_then([&](bool) {
                    return tx.insert(Service,
                        ServiceRow{.id = *api, .name = std::string{"api"}});
                })
                .and_then([&](bool) {
                    return tx.insert(Outage,
                        OutageRow{.service = *search,
                            .window = bdb::interval<std::int64_t>::literal(
                                0, 100)});
                })
                .and_then([&](bool) {
                    return tx.insert(Outage,
...
auto down_services(bdb::Db& db, bdb::Prepared<DownAt>& prepared,
    std::int64_t at) -> std::optional<std::vector<std::uint64_t>> {
    auto result = db.read([&](bdb::Snapshot& snap)
            -> std::expected<std::vector<std::uint64_t>, bdb::Error> {
        return snap.execute(prepared, {.t = at})
            .transform([](bdb::Answers<DownAt> answers) {
...
auto run_cases(std::vector<CaseResult>& results) -> void {
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r01 queries store directory", .passed = false});
        return;
    }
    auto db = bdb::Db::ephemeral(dir->native(), Uptime);
    if (!db.has_value()) {
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Uptime", .passed = false});
        return;
    }
    ...
    results.push_back(CaseResult{
        .name = "downAt / overlapping / downtime all prepare through the "
                "engine validator",
        .passed = down_at.has_value() && overlapping.has_value()
            && downtime.has_value(),
    });
    if (!down_at.has_value() || !overlapping.has_value()
        || !downtime.has_value()) {
        return;
    }
    ...
    results.push_back(CaseResult{
        ...
        .passed = at_160.has_value()
            && *at_160 == std::vector{ids->search},
    });
    ...
    auto overlap_rows = db->read([&](bdb::Snapshot& snap)
            -> std::expected<
                std::vector<std::pair<std::uint64_t,
                    bdb::interval<std::int64_t>>>,
                bdb::Error> {
        return snap.execute(*overlapping, {.incident = incident})
            .transform([](bdb::Answers<Overlapping> answers) {
                auto rows = std::vector<std::pair<std::uint64_t,
                    bdb::interval<std::int64_t>>>{};
                for (auto const& row : answers.rows()) {
                    rows.emplace_back(row.service, row.window);
                }
                std::ranges::sort(rows, {},
                    [](auto const& row) { return row.first; });
                return rows;
            });
    });
    results.push_back(CaseResult{
        .name = "overlapping([110,140)) answers {(api, [50,120))}",
        .passed = overlap_rows.has_value() && overlap_rows->size() == 1
            && (*overlap_rows)[0].first == ids->api
            && (*overlap_rows)[0].second
                == bdb::interval<std::int64_t>::literal(50, 120),
    });

    // downtime: sum(duration(window)) per service.
    auto downtime_rows = db->read([&](bdb::Snapshot& snap)
            -> std::expected<
                std::vector<std::pair<std::uint64_t, std::uint64_t>>,
                bdb::Error> {
        return snap.execute(*downtime, {})
            .transform([](bdb::Answers<Downtime> answers) {
                auto rows =
                    std::vector<std::pair<std::uint64_t, std::uint64_t>>{};
                for (auto const& row : answers.rows()) {
    ...
    auto expected_downtime = std::vector<
        std::pair<std::uint64_t, std::uint64_t>>{
        {ids->search, 150}, {ids->api, 70}};
    std::ranges::sort(expected_downtime, {},
        [](auto const& row) { return row.first; });
    results.push_back(CaseResult{
        .name = "downtime answers {(search, 150), (api, 70)}",
        .passed =
            downtime_rows.has_value() && *downtime_rows == expected_downtime,
    });

    // §23: execute_into refills ONE caller-owned carrier — capacity
    // reused, previous answers replaced.
    auto reused = bdb::Answers<DownAt>{};
    auto const reuse_sizes = db->read([&](bdb::Snapshot& snap)
            -> std::expected<std::pair<std::size_t, std::size_t>,
                bdb::Error> {
        return snap.execute_into(*down_at, {.t = std::int64_t{60}}, reused)
            .transform([&] { return reused.size(); })
            .and_then([&](std::size_t first)
                    -> std::expected<std::pair<std::size_t, std::size_t>,
                        bdb::Error> {
                return snap
                    .execute_into(
                        *down_at, {.t = std::int64_t{130}}, reused)
                    .transform([&] {
    ...
    results.push_back(CaseResult{
        ...
        .passed = reuse_sizes.has_value() && reuse_sizes->first == 2
            && reuse_sizes->second == 0,
    });
    ...
    auto long_services = long_outages.has_value()
        ? db->read([&](bdb::Snapshot& snap)
                -> std::expected<std::vector<std::uint64_t>, bdb::Error> {
              return snap.execute(*long_outages, {})
                  .transform([](bdb::Answers<LongOutages> answers) {
                      auto services = std::vector<std::uint64_t>{};
                      for (auto const& row : answers.rows()) {
                          services.push_back(row.service);
                      }
        ...
        : std::expected<std::vector<std::uint64_t>, bdb::Error>{
              std::unexpected{std::move(long_outages).error()}};
    results.push_back(CaseResult{
        .name = "longOutages (duration >= 100) answers {search}",
        .passed = long_services.has_value()
            && *long_services == std::vector{ids->search},
    });
    ...
    auto down_names = named.has_value()
        ? db->read([&](bdb::Snapshot& snap)
                -> std::expected<std::vector<std::string>, bdb::Error> {
              return snap.execute(*named, {.t = std::int64_t{60}})
                  .transform([](bdb::Answers<NamedDownAt> answers) {
        ...
        : std::expected<std::vector<std::string>, bdb::Error>{
              std::unexpected{std::move(named).error()}};
    results.push_back(CaseResult{
        .name = "namedDownAt(60) answers {api, search} through borrowed "
                "string views",
        .passed = down_names.has_value()
            && *down_names == std::vector<std::string>{"api", "search"},
    });
