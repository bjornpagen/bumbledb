...
inline constexpr auto Pricing =
    bdb::schema<"Pricing">(Policy, Version,
...
inline constexpr auto Successions = bdb::query(Pricing).rule([](auto r) consteval {
	...
	return r
	    ...
	    .where(bdb::allen_in(version.valid, bdb::allen::meets, policy.live))
	    .find({}, bdb::as<"a">(version.valid), bdb::as<"b">(policy.live));
});
...
[[nodiscard]] auto make_store_dir() -> std::optional<std::filesystem::path> {
	...
	auto const dir = root / std::format("bumbledb-cookbook-r15-{:08x}{:08x}", device(), device());
...
[[nodiscard]] auto seed(bdb::Db& db) -> std::optional<std::uint64_t> {
	...
	auto written = db.write([&](bdb::WriteTx& tx) -> Result {
		...
		auto rows_land =
		    tx.insert(Policy, PolicyRow{.id = *policy, .live = bdb::interval<std::int64_t>::literal(0, 20)})
		        .and_then([&](bool) {
			        return tx.insert(Version,
			                         VersionRow{.policy = *policy, .rate_bps = 100, .valid = bdb::interval<std::int64_t>::literal(0, 10)});
		        })
		        .and_then([&](bool) {
			        return tx.insert(Version,
			                         VersionRow{.policy = *policy, .rate_bps = 200, .valid = bdb::interval<std::int64_t>::literal(10, 20)});
		        })
		        .and_then([&](bool) {
			        return tx.insert(Version,
			                         VersionRow{.policy = *policy, .rate_bps = 300, .valid = bdb::interval<std::int64_t>::literal(20, 25)});
		        });
...
auto run_cases(std::string_view fixtures_path, std::vector<CaseResult>& results) -> void {
	...
	auto const golden = fixtures.has_value() ? golden_of(*fixtures, "r15") : std::nullopt;
	if (!golden.has_value()) {
		results.push_back(CaseResult{.name = "fixtures file carries an r15 line", .passed = false});
		return;
	}

	auto const dir = make_store_dir();
	if (!dir.has_value()) {
		results.push_back(CaseResult{.name = "r15 store directory", .passed = false});
		return;
	}
	auto db = bdb::Db::ephemeral(dir->native(), Pricing);
	if (!db.has_value()) {
		std::println("  Db::ephemeral: {}", db.error().message());
		results.push_back(CaseResult{.name = "Db::ephemeral admits Pricing", .passed = false});
		return;
	}
	...
	auto pairs = db->execute(*successions, {}).transform([](bdb::Answers<Successions> answers) {
		...
		std::ranges::sort(out, {}, [](auto const& row) {
			return row.first.lo();
		});
		return out;
	});
