...
inline constexpr auto KeyedRead = bdb::schema<"KeyedRead">(
    Grp,
    Program,
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r30-{:08x}{:08x}", device(),
            device());
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        ...
        auto rows_land = tx.alloc(Grp.id)
            .and_then([&](std::uint64_t minted) {
                ids.grp = minted;
                return tx.insert(Grp,
                    GrpRow{.id = minted, .label = std::string{"algebra"}});
            })
            .and_then([&](bool) { return tx.alloc(Program.id); })
            .and_then([&](std::uint64_t minted) {
                ids.program = minted;
                return tx.insert(Program,
                    ProgramRow{.id = minted,
                        .grp = ids.grp,
                        .title = std::string{"linear equations"}});
            });
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r30") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r30 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r30 store directory", .passed = false});
        return;
    }
    auto db = bdb::Db::ephemeral(dir->native(), KeyedRead);
    if (!db.has_value()) {
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits KeyedRead", .passed = false});
        return;
    }
    ...
    auto const by_group =
        db->get(Program, program_grp_key, {.grp = ids->grp});
    results.push_back(CaseResult{
        .name = "db.get(Program, programGrpKey, {grp}) answers the typed "
                "point read",
        .passed = by_group.has_value() && by_group->has_value()
            && (*by_group)->len() == 1
            && cell_is_u64(**by_group, {.row = 0, .column = 0},
                ids->program)
            && cell_is_text(**by_group, {.row = 0, .column = 2},
                "linear equations"),
    });
    ...
    results.push_back(CaseResult{
        ...
        .passed = by_id.has_value() && by_id->has_value()
            && (*by_id)->len() == 1
            && cell_is_u64(**by_id, {.row = 0, .column = 1}, ids->grp),
    });

    // snap.get — the same spelling inside a read scope (the symmetry
    // rule).
    auto const via_snap = db->read([&](bdb::Snapshot& snap)
            -> std::expected<bool, bdb::Error> {
        return snap.get(Program, program_grp_key, {.grp = ids->grp})
            .transform([&](std::optional<bdb::RowSet> rows) {
                return rows.has_value() && rows->len() == 1
                    && cell_is_u64(*rows, {.row = 0, .column = 0},
                        ids->program);
            });
    });
    results.push_back(CaseResult{
    ...
    auto mutated = db->write([&](bdb::WriteTx& tx) -> MutateResult {
        ...
        witnesses.keyed_saw_pending = keyed->has_value()
            && (*keyed)->len() == 1
            && cell_is_u64(**keyed, {.row = 0, .column = 0}, proofs);
        auto primary = tx.get(Program, {.id = proofs});
        if (!primary.has_value()) {
            return std::unexpected{std::move(primary).error()};
        }
        witnesses.primary_saw_pending = primary->has_value()
            && (*primary)->len() == 1
            && cell_is_text(**primary, {.row = 0, .column = 2}, "proofs");
        return bdb::commit(witnesses);
    });
    auto const committed = mutated.has_value()
        && std::holds_alternative<bdb::Committed<PendingReads>>(*mutated);
    results.push_back(CaseResult{
        .name = "the keyed read-modify-write commits",
        .passed = committed,
    });
    if (committed) {
        auto const& witnesses =
            std::get<bdb::Committed<PendingReads>>(*mutated).value;
        results.push_back(CaseResult{
