...
inline constexpr auto ExactPartition = bdb::schema<"ExactPartition">(
    Policy,
    Version,
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r26-{:08x}{:08x}", device(),
            device());
...
auto attempt(bdb::Db& db,
    std::vector<bdb::interval<std::int64_t>> const& versions)
    ...
    return db.write([&](bdb::WriteTx& tx)
            -> std::expected<
                ...
                bdb::Error> {
        auto landed = tx.alloc(Policy.id)
            .and_then([&](std::uint64_t policy)
                -> std::expected<bool, bdb::Error> {
                auto row = tx.insert(Policy,
                    PolicyRow{.id = policy,
                        .live =
                            bdb::interval<std::int64_t>::literal(0, 10)});
                for (auto const valid : versions) {
                    row = std::move(row).and_then([&](bool) {
                        return tx.insert(Version,
                            VersionRow{.policy = policy, .valid = valid});
                    });
                }
                return row;
            });
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r26") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r26 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r26 store directory", .passed = false});
        return;
    }
    auto db = bdb::Db::ephemeral(dir->native(), ExactPartition);
    if (!db.has_value()) {
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits ExactPartition",
            .passed = false});
        return;
    }
    ...
    auto gapped = attempt(*db,
        {bdb::interval<std::int64_t>::literal(0, 4),
            bdb::interval<std::int64_t>::literal(5, 10)});
    results.push_back(CaseResult{
        .name = "a gap in the versions is commit-rejected (forward "
                "coverage forbids gaps)",
        .passed = !gapped.has_value()
            && gapped.error().kind() == bdb::ErrorKind::CommitRejected
            && !gapped.error().violations().empty(),
    });

    // An overhang: [0,12) spills past the policy's [0,10) — reverse
    // coverage (versions ⊆ policy) rejects the write.
    auto overhang = attempt(*db,
        {bdb::interval<std::int64_t>::literal(0, 12)});
    results.push_back(CaseResult{
        .name = "a version overhang is commit-rejected (reverse coverage "
                "forbids overhang)",
        .passed = !overhang.has_value()
            && overhang.error().kind() == bdb::ErrorKind::CommitRejected
            && !overhang.error().violations().empty(),
    });
