...
inline constexpr auto ZoneLedger = bdb::schema<"ZoneLedger">(
    ...
    bdb::key(Zone.ledger, Zone.at),
    bdb::key(UnitSlot.ledger, UnitSlot.at),
    bdb::key(PairSlot.ledger, PairSlot.at),

    // Each kind's zones carry exactly its sidecar's points — mixed
    // widths, one element domain:
    bdb::mirrors(
        bdb::on(bdb::where(Zone, {.kind = Kind.Unit}),
            Zone.ledger, Zone.at),
        bdb::on(UnitSlot.ledger, UnitSlot.at)
    ),
    bdb::mirrors(
        bdb::on(bdb::where(Zone, {.kind = Kind.Pair}),
            Zone.ledger, Zone.at),
        bdb::on(PairSlot.ledger, PairSlot.at)
    )
);
...
static_assert(bdb::interval<std::uint64_t, 1>::make(4, 6).error()
    == bdb::TypeError::IntervalWidth);
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r29-{:08x}{:08x}", device(),
            device());
...
auto seed(bdb::Db& db) -> std::optional<std::uint64_t> {
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        ...
        auto rows_land =
            tx.insert(Ledger,
                  LedgerRow{.id = *ledger, .name = std::string{"main"}})
                .and_then([&](bool) {
                    return tx.insert(Zone,
                        ZoneRow{.ledger = *ledger,
                            .kind = Kind.Unit,
                            .at = bdb::interval<std::uint64_t>::literal(
                                4, 6)});
                })
                .and_then([&](bool) {
                    return tx.insert(UnitSlot,
                        UnitSlotRow{.ledger = *ledger,
                            .at = bdb::interval<std::uint64_t,
                                1>::literal(4, 5),
                            .entry = 7});
                })
                .and_then([&](bool) {
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r29") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r29 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r29 store directory", .passed = false});
        return;
    }
    auto db = bdb::Db::ephemeral(dir->native(), ZoneLedger);
    if (!db.has_value()) {
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits ZoneLedger", .passed = false});
        return;
    }
    ...
    auto uncovered = db->write([&](bdb::WriteTx& tx)
            -> std::expected<
                ...
                bdb::Error> {
        auto landed = tx.insert(UnitSlot,
            UnitSlotRow{.ledger = *ledger,
                .at = bdb::interval<std::uint64_t, 1>::literal(20, 21),
                .entry = 11});
        if (!landed.has_value()) {
    ...
    if (scanned.has_value()) {
        for (auto row = std::size_t{0}; row != scanned->len(); ++row) {
            ...
            if (cell.has_value()
                && std::holds_alternative<bdb::interval<std::uint64_t>>(
                    *cell)) {
                slots.push_back(
                    std::get<bdb::interval<std::uint64_t>>(*cell));
            }
        }
    }
    ...
    results.push_back(CaseResult{
        ...
        .passed = scanned.has_value() && slots.size() == 2
            && slots[0] == bdb::interval<std::uint64_t>::literal(4, 5)
