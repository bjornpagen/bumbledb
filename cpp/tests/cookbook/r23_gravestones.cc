...
inline constexpr auto Gravestones = bdb::schema<"Gravestones">(
    Step,
    Score,
    ActiveRun,
    Usage,
    ...
    bdb::key(Step.flow, Step.pos),
    bdb::key(Score.subject),
    bdb::key(ActiveRun.student),
    bdb::key(Usage.meter, Usage.used)
);
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r23-{:08x}{:08x}", device(),
            device());
...
auto main(int argc, char** argv) -> int {
    ...
    if (!db.has_value()) {
        std::println(
            "FAIL: Db::ephemeral rejected the Gravestones schema: {}",
            db.error().message());
        return 1;
    }
