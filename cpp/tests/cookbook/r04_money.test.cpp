...
inline constexpr auto Currency =
    bdb::closed<"Currency", "Usd", "Eur", "Gbp">();
...
inline constexpr auto Money = bdb::schema<"Money">(
    Currency,
    Account,
    Posting,
...
inline constexpr auto Totals =
    bdb::query(Money).rule([](auto r) consteval {
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r04-{:08x}{:08x}", device(),
            device());
...
auto seed(bdb::Db& db) -> std::optional<SeedIds> {
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        ...
        auto const posting = [&](std::uint64_t account,
                                 bdb::ref_to<Currency.id> currency,
                                 std::int64_t minor)
            -> std::expected<bool, bdb::Error> {
            return tx.alloc(Posting.id)
        ...
        auto rows_land =
            tx.insert(Account, AccountRow{.id = *cash, .name = "cash"})
                .and_then([&](bool) {
                    return tx.insert(Account,
                        AccountRow{.id = *bank, .name = "bank"});
                })
                .and_then([&](bool) {
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r04") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r04 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r04 store directory", .passed = false});
        return;
    }
    ...
    if (!db.has_value()) {
        ...
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Money", .passed = false});
        return;
    }
