...
inline constexpr auto Closure = bdb::schema<"Closure">(
    Node,
    Parent,
...
inline constexpr auto Step =
    bdb::query(Closure).rule([](auto r) consteval {
...
inline constexpr auto Reach = bdb::program(Closure,
    bdb::rec<"reach">(
        [](auto r) consteval {
            auto vars = r.vars(Node);
            return r
                .match(Node, {.id = vars.id})
    ...
    bdb::output([](auto r) consteval {
        auto vars = r.vars(Node);
        return r.idb(bdb::pred<"reach">, bdb::bind<"c">(vars.id))
            .find({}, bdb::as<"c">(vars.id));
    }));
...
// only rejects.
inline constexpr auto Roots =
    bdb::query(Closure).rule([](auto r) consteval {
        auto node = r.vars(Node);
        return r
            .match(Node, {.id = node.id})
            .not_match(Parent, {.child = node.id})
            .find({}, bdb::as<"c">(node.id));
    });

// The complement — negation OF the finished stratum.
inline constexpr auto Unreached = bdb::program(Closure,
    bdb::rec<"reach">(
        [](auto r) consteval {
            auto vars = r.vars(Node);
            return r
    ...
    bdb::output([](auto r) consteval {
        ...
        return r.match(Node, {.id = vars.id})
            .not_idb(bdb::pred<"reach">, bdb::bind<"c">(vars.id))
            .find({}, bdb::as<"c">(vars.id));
    }));
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r24-{:08x}{:08x}", device(),
            device());
...
auto seed(bdb::Db& db) -> std::optional<SeedIds> {
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        auto ids = SeedIds{};
        auto const mint = [&](std::uint64_t& out, std::string name)
            -> std::expected<bool, bdb::Error> {
            return tx.alloc(Node.id).and_then(
                [&](std::uint64_t minted)
                    -> std::expected<bool, bdb::Error> {
                    out = minted;
                    return tx.insert(Node,
                        NodeRow{.id = minted, .name = std::move(name)});
                });
        };
        auto rows_land = mint(ids.root, std::string{"root"})
            .and_then(
                [&](bool) { return mint(ids.mid, std::string{"mid"}); })
            .and_then(
                [&](bool) { return mint(ids.leaf, std::string{"leaf"}); })
            .and_then(
                [&](bool) { return mint(ids.lone, std::string{"lone"}); })
            .and_then([&](bool) {
                return tx.insert(Parent,
                    ParentRow{.child = ids.mid, .parent = ids.root});
            })
            .and_then([&](bool) {
                return tx.insert(Parent,
                    ParentRow{.child = ids.leaf, .parent = ids.mid});
            });
...
template<auto Query>
auto answer_set(bdb::Answers<Query> const& answers)
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r24") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r24 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r24 store directory", .passed = false});
        return;
    }
    auto db = bdb::Db::ephemeral(dir->native(), Closure);
    if (!db.has_value()) {
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Closure", .passed = false});
        return;
    }
    ...
    results.push_back(CaseResult{
        .name = "step / reach / unreached / roots all prepare through the "
                ...
                "EDB anti-join)",
        .passed = step.has_value() && reach.has_value()
            && unreached.has_value() && roots.has_value(),
    });
    if (!step.has_value() || !reach.has_value() || !unreached.has_value()
        || !roots.has_value()) {
        return;
    }
    ...
    auto native = db->execute(*reach, {.root = ids->root})
        .transform([](bdb::Answers<Reach> answers) {
            return answer_set<Reach>(answers);
        });
    auto expected =
        std::vector<std::uint64_t>{ids->root, ids->mid, ids->leaf};
    std::ranges::sort(expected);
    results.push_back(CaseResult{
        .name = "the two dialects agree, root for root",
        .passed = loop_sound && native.has_value() && *native == seen
            && seen == expected,
    });

    // The complement lands in-plan: every node the closure never
    // reached, judged against the full scan.
    auto complement = db->execute(*unreached, {.root = ids->root})
        .transform([](bdb::Answers<Unreached> answers) {
    ...
    results.push_back(CaseResult{
        .name = "negation of the finished stratum answers the complement "
                "(scan minus closure)",
        .passed = complement.has_value() && scanned.has_value()
            && *complement == outside
            && *complement == std::vector<std::uint64_t>{ids->lone},
    });

    // The EDB anti-join: the nodes no Parent row claims as a child —
    // root and lone (mid and leaf are somebody's children).
    auto parentless = db->execute(*roots, {})
        .transform([](bdb::Answers<Roots> answers) {
            return answer_set<Roots>(answers);
        });
    auto expected_roots =
        std::vector<std::uint64_t>{ids->root, ids->lone};
