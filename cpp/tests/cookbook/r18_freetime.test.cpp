...
inline constexpr auto FreeTime = bdb::schema<"FreeTime">(
    Person,
    Claim,
...
inline constexpr auto Busy =
    bdb::query(FreeTime).rule([](auto r) consteval {
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r18-{:08x}{:08x}", device(),
            device());
...
auto seed(bdb::Db& db) -> std::optional<SeedIds> {
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        ...
        auto rows_land =
            tx.insert(Person,
                  PersonRow{.id = *alice, .name = std::string{"alice"}})
                .and_then([&](bool) {
                    return tx.insert(Person,
                        PersonRow{.id = *bob, .name = std::string{"bob"}});
                })
                .and_then([&](bool) {
                    return tx.insert(Claim,
                        ClaimRow{.person = *alice,
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r18") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r18 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r18 store directory", .passed = false});
        return;
    }
    ...
    if (!db.has_value()) {
        ...
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits FreeTime", .passed = false});
        return;
    }
    ...
    auto segments = db->execute(*busy, {})
        .transform([](bdb::Answers<Busy> answers) {
            ...
            std::ranges::sort(rows, [](auto const& left,
                                        auto const& right) {
    ...
    auto expected_segments = std::vector<std::pair<std::uint64_t,
        bdb::interval<std::int64_t>>>{
    ...
    std::ranges::sort(expected_segments,
    ...
    auto totals = db->execute(*claimed, {})
        .transform([](bdb::Answers<Claimed> answers) {
            auto rows =
    ...
    auto expected_totals =
        std::vector<std::pair<std::uint64_t, std::uint64_t>>{
            {ids->alice, 30}, {ids->bob, 5}};
    std::ranges::sort(expected_totals, {},
        [](auto const& row) { return row.first; });
    results.push_back(CaseResult{
