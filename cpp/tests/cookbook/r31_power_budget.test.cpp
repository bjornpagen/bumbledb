...
inline constexpr auto Racks = bdb::schema<"Racks">(
    ...
    bdb::key(Model.id, Model.watts),
    bdb::contained(
        bdb::on(Device.model, Device.watts),
        bdb::on(Model.id, Model.watts)
    ),
    ...
    bdb::capacity(
        ...
        bdb::within(0, bdb::ref(Pool.supply)),
        bdb::on(Device.pool)
    )
);
...
inline constexpr auto Draw =
    bdb::query(Racks).rule([](auto r) consteval {
...
// (max over the scalar).
inline constexpr auto FleetFacts =
    bdb::query(Racks).rule([](auto r) consteval {
        auto vars = r.vars(Device);
        return r
            .match(Device,
...
inline constexpr auto TopDevice =
    bdb::query(Racks).rule([](auto r) consteval {
        auto vars = r.vars(Device);
        return r
            .match(Device,
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r31-{:08x}{:08x}", device(),
            device());
...
auto seed(bdb::Db& db) -> std::optional<SeedIds> {
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        ...
        auto rows_land = mint(Pool.id, ids.pool_a)
            .and_then([&](bool) { return mint(Pool.id, ids.pool_b); })
            .and_then([&](bool) { return mint(Model.id, ids.model_40); })
            .and_then([&](bool) { return mint(Model.id, ids.model_25); })
            .and_then([&](bool) { return mint(Model.id, ids.model_10); })
            .and_then(
                [&](bool) { return mint(Device.id, ids.device_40); })
            .and_then([&](bool) { return mint(Device.id, device_25); })
            .and_then([&](bool) { return mint(Device.id, device_10); })
            .and_then([&](bool) {
                return tx.insert(
                    Pool, PoolRow{.id = ids.pool_a, .supply = 100});
            })
            .and_then([&](bool) {
                return tx.insert(
                    Pool, PoolRow{.id = ids.pool_b, .supply = 30});
            })
            .and_then([&](bool) {
                return tx.insert(
                    Model, ModelRow{.id = ids.model_40, .watts = 40});
            })
            .and_then([&](bool) {
                return tx.insert(
                    Model, ModelRow{.id = ids.model_25, .watts = 25});
            })
            .and_then([&](bool) {
                return tx.insert(
                    Model, ModelRow{.id = ids.model_10, .watts = 10});
            })
            .and_then([&](bool) {
                return tx.insert(Device,
                    DeviceRow{.id = ids.device_40,
                        .pool = ids.pool_a,
                        .model = ids.model_40,
                        .watts = 40});
            })
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r31") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r31 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r31 store directory", .passed = false});
        return;
    }
    auto db = bdb::Db::ephemeral(dir->native(), Racks);
    if (!db.has_value()) {
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Racks", .passed = false});
        return;
    }
    ...
    results.push_back(CaseResult{
        .name = "draw / fleetFacts / topDevice prepare through the engine "
                "validator",
        .passed =
            draw.has_value() && fleet.has_value() && top.has_value(),
    });
    ...
    auto totals = db->execute(*draw, {})
        .transform([](bdb::Answers<Draw> answers) {
    ...
    results.push_back(CaseResult{
        ...
        .passed = totals.has_value() && totals->size() == 2
            && (*totals)[0].pool == ids->pool_a
            && (*totals)[0].total == 65
            && (*totals)[1].pool == ids->pool_b
            && (*totals)[1].total == 10,
    });

    // The fold roster: count / max per pool.
    using FleetRow = bdb::row_of<FleetFacts>;
    auto facts = db->execute(*fleet, {})
        .transform([](bdb::Answers<FleetFacts> answers) {
    ...
    if (facts_pass) {
        auto const& a = (*facts)[0].pool == ids->pool_a ? (*facts)[0]
                                                        : (*facts)[1];
        auto const& b = (*facts)[0].pool == ids->pool_b ? (*facts)[0]
                                                        : (*facts)[1];
    ...
    auto top_a = db->execute(*top, {})
        .transform([&](bdb::Answers<TopDevice> answers) {
    ...
    auto overdraw_cited = !overdraw.has_value()
        && overdraw.error().kind() == bdb::ErrorKind::CommitRejected;
    if (overdraw_cited) {
        auto const violations = overdraw.error().violations();
        overdraw_cited = !violations.empty()
    ...
    auto witnessed = db->write_witnessed(
        [&](bdb::Snapshot& snap, bdb::WriteTx& tx)
            -> std::expected<
                bdb::WriteDecision<std::uint64_t, std::monostate>,
                bdb::Error> {
            auto used = snap.execute(*draw, {}).transform(
                [&](bdb::Answers<Draw> answers) {
    ...
    auto witnessed_pass = witnessed.has_value()
