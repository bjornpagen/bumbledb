...
inline constexpr auto Severity = bdb::closed<"Severity", SeverityPayload>(
    bdb::member<"Info">(SeverityPayload{.pages = false}),
    bdb::member<"Warning">(SeverityPayload{.pages = false}),
    bdb::member<"Critical">(SeverityPayload{.pages = true}),
    bdb::member<"Fatal">(SeverityPayload{.pages = true}));
...
inline constexpr auto Escalation =
    bdb::relation<"Escalation", EscalationRow>;
...
inline constexpr auto Oncall = bdb::schema<"Oncall">(
    ...
    bdb::contained(
        bdb::on(Incident.severity),
        bdb::on(Severity.id)
    ),
    bdb::contained(
        bdb::on(Escalation.incident),
        bdb::on(Incident.id)
    ),
    ...
    // {Critical, Fatal}.
    bdb::contained(
        bdb::on(Escalation.severity),
        bdb::on(bdb::where(Severity, {.pages = true}), Severity.id)
    )
);

// who is being paged — the same ψ, on the read side.
inline constexpr auto Paged =
    bdb::query(Oncall).rule([](auto r) consteval {
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r08-{:08x}{:08x}", device(),
            device());
...
auto seed(bdb::Db& db) -> std::optional<SeedIds> {
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        ...
        auto rows_land =
            tx.insert(Incident,
                  IncidentRow{.id = *quiet, .severity = Severity.Info})
                .and_then([&](bool) {
                    return tx.insert(Incident,
                        IncidentRow{
                            .id = *paged, .severity = Severity.Critical});
                })
                .and_then([&](bool) {
                    return tx.insert(Escalation,
                        EscalationRow{.incident = *paged,
...
auto info_escalation_rejected(bdb::Db& db, std::uint64_t incident)
    -> bool {
    using Decision = bdb::WriteDecision<std::monostate, std::monostate>;
    using Result = std::expected<Decision, bdb::Error>;
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        auto landed = tx.insert(Escalation,
            EscalationRow{
                .incident = incident, .severity = Severity.Info, .at = 5});
        if (!landed.has_value()) {
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r08") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r08 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r08 store directory", .passed = false});
        return;
    }
    ...
    if (!db.has_value()) {
        ...
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Oncall", .passed = false});
        return;
    }
    ...
    auto answered = db->read([&](bdb::Snapshot& snap)
        ...
        return snap.execute(*paged, {})
            .transform([](bdb::Answers<Paged> answers) {
    ...
    results.push_back(CaseResult{
        ...
        .passed = answered.has_value()
            && *answered == std::vector{ids->paged},
    });
