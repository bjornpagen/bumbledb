...
inline constexpr auto AccountParent =
    bdb::relation<"AccountParent", AccountParentRow>;
...
inline constexpr auto Accounts = bdb::schema<"Accounts">(
    ...
    bdb::key(AccountParent.child),
    bdb::contained(
        bdb::on(AccountParent.child),
        bdb::on(Account.id)
    ),
    bdb::contained(
        bdb::on(AccountParent.parent),
        bdb::on(Account.id)
    ),
    bdb::contained(
...
inline constexpr auto FrontierStep =
    bdb::query(Accounts).rule([](auto r) consteval {
...
inline constexpr auto SubtreeRollup =
    bdb::query(Accounts).rule([](auto r) consteval {
...
inline constexpr auto NativeRollup = bdb::program(Accounts,
    bdb::rec<"sub">(
        [](auto r) consteval {
            auto vars = r.vars(Account);
            return r
                .match(Account, {.id = vars.id})
        ...
        [](auto r) consteval {
            ...
            return r
                .match(AccountParent,
                    {.child = vars.child, .parent = vars.parent})
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r25-{:08x}{:08x}", device(),
            device());
...
auto seed(bdb::Db& db) -> std::optional<SeedIds> {
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        auto ids = SeedIds{};
        auto const account = [&](std::uint64_t& out, std::string name)
            -> std::expected<bool, bdb::Error> {
            return tx.alloc(Account.id).and_then(
                [&](std::uint64_t minted)
                    -> std::expected<bool, bdb::Error> {
                    out = minted;
                    return tx.insert(Account,
                        AccountRow{
                            .id = minted, .name = std::move(name)});
                });
        };
        auto const posting = [&](std::uint64_t into, std::int64_t minor)
            -> std::expected<bool, bdb::Error> {
            return tx.alloc(Posting.id).and_then(
                [&](std::uint64_t minted)
                    -> std::expected<bool, bdb::Error> {
                    return tx.insert(Posting,
                        PostingRow{.id = minted,
                            .account = into,
                            .minor = minor});
                });
        };
        auto rows_land = account(ids.root, std::string{"root"})
            ...
            .and_then([&](bool) {
                return tx.insert(AccountParent,
                    AccountParentRow{
                        .child = ids.ops, .parent = ids.root});
            })
            .and_then([&](bool) {
                return tx.insert(AccountParent,
                    AccountParentRow{
                        .child = ids.eng, .parent = ids.root});
            })
            .and_then([&](bool) {
                return tx.insert(AccountParent,
                    AccountParentRow{.child = ids.fe, .parent = ids.eng});
            })
            .and_then([&](bool) { return posting(ids.root, 5); })
            .and_then([&](bool) { return posting(ids.ops, 10); })
            .and_then([&](bool) { return posting(ids.eng, 20); })
            .and_then([&](bool) { return posting(ids.eng, 20); })
            .and_then([&](bool) { return posting(ids.fe, 30); });
...
auto host_rollup(bdb::Db& db, bdb::Prepared<Step>& step,
    bdb::Prepared<Rollup>& rollup, std::uint64_t root)
    -> std::optional<std::int64_t> {
    auto subtree = std::vector<std::uint64_t>{root};
    auto frontier = std::vector<std::uint64_t>{root};
    while (!frontier.empty()) {
        auto next = db.execute(step,
            {.frontier = std::span<std::uint64_t const>{frontier}});
        if (!next.has_value()) {
            return std::nullopt;
        }
    ...
    auto folded = db.execute(rollup,
        {.subtree = std::span<std::uint64_t const>{subtree}});
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r25") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r25 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r25 store directory", .passed = false});
        return;
    }
    auto db = bdb::Db::ephemeral(dir->native(), Accounts);
    if (!db.has_value()) {
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Accounts", .passed = false});
        return;
    }
    ...
    results.push_back(CaseResult{
        ...
        .passed = step.has_value() && rollup.has_value()
            && native.has_value(),
    });
    ...
    auto const from_root =
        host_rollup<FrontierStep, SubtreeRollup>(*db, *step, *rollup,
            ids->root);
    ...
    results.push_back(CaseResult{
        ...
        .passed = from_root.has_value() && *from_root == 85
            && native_root.has_value() && native_root->size() == 1
    ...
    results.push_back(CaseResult{
        .name = "eng's rollup: host composition and native program both "
                "answer 70",
        .passed = from_eng.has_value() && *from_eng == 70
            && native_eng.has_value() && native_eng->size() == 1
