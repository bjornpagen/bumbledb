...
inline constexpr auto DeterministicGrading =
    bdb::relation<"DeterministicGrading", DeterministicGradingRow>;
inline constexpr auto CustomOperatorGrading =
    bdb::relation<"CustomOperatorGrading", CustomOperatorGradingRow>;

inline constexpr auto Grading = bdb::schema<"Grading">(
    Kind,
    Task,
    DeterministicGrading,
    CustomOperatorGrading,
    ...
    bdb::key(DeterministicGrading.task),
    bdb::key(CustomOperatorGrading.task),
    ...
    bdb::mirrors(
        ...
        bdb::on(DeterministicGrading.task)
    ),
    bdb::mirrors(
        bdb::on(bdb::where(Task, {.kind = Kind.CustomOperator}), Task.id),
        bdb::on(CustomOperatorGrading.task)
    )
);
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r02-{:08x}{:08x}", device(),
            device());
...
auto main(int argc, char** argv) -> int {
    ...
    if (!fixtures.has_value()) {
        std::println("FAIL: cannot read fixtures file {}",
    ...
    if (!db.has_value()) {
        std::println("FAIL: Db::ephemeral rejected the Grading schema: {}",
            db.error().message());
        return 1;
    }
    auto const fingerprint = db->fingerprint();
    if (!fingerprint.has_value()) {
        std::println("FAIL: fingerprint readback failed: {}",
    ...
    if (graded_by(Kind.Deterministic) == "tolerance"
        && graded_by(Kind.CustomOperator) == "operator") {
        std::println(
            "pass: gradedBy dispatches over the handle projection");
    } else {
