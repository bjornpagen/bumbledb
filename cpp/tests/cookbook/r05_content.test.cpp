...
inline constexpr auto Content = bdb::schema<"Content">(
    Region,
    Document,
    Replica,
    ...
    bdb::contained(
        bdb::on(Replica.payload),
        bdb::on(Document.payload)
    ),

    bdb::contained(
        bdb::on(Replica.region),
        bdb::on(Region.id)
    )
);

// Lookup by digest: the bytes param binds AT the match field.
inline constexpr auto ByDigest =
    bdb::query(Content).rule([](auto r) consteval {
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r05-{:08x}{:08x}", device(),
            device());
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        ...
        auto rows_land =
            tx.insert(Document,
                  DocumentRow{.id = *spec,
                      .name = "spec",
                      .payload = digest_of(0x11)})
                .and_then([&](bool) {
                    return tx.insert(Document,
                        DocumentRow{.id = *draft,
                            .name = "draft",
                            .payload = digest_of(0x22)});
                })
                .and_then([&](bool) {
                    return tx.insert(Replica,
                        ReplicaRow{.payload = digest_of(0x11),
                            .region = Region.Us});
                });
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r05") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r05 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r05 store directory", .passed = false});
        return;
    }
    ...
    if (!db.has_value()) {
        ...
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Content", .passed = false});
        return;
    }
