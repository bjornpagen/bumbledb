...
inline constexpr auto Ast =
    bdb::schema<"Ast">(Kind, Node, Lit, Add, Parent,
...
[[nodiscard]] auto make_store_dir() -> std::optional<std::filesystem::path> {
	...
	auto const dir = root / std::format("bumbledb-cookbook-r10-{:08x}{:08x}", device(), device());
...
[[nodiscard]] auto seed(bdb::Db& db) -> std::optional<SeedIds> {
	...
	auto written = db.write([&](bdb::WriteTx& tx) -> Result {
		...
		auto const mint = [&](std::uint64_t& out) -> std::expected<bool, bdb::Error> {
			return tx.alloc(Node.id).transform([&](std::uint64_t minted) {
				out = minted;
				return true;
			});
		};
		auto rows_land = mint(ids.two)
		                     .and_then([&](bool) {
			                     return mint(ids.three);
		                     })
		                     .and_then([&](bool) {
			                     return mint(ids.sum);
		                     })
		                     .and_then([&](bool) {
			                     return tx.insert(Node, NodeRow{.id = ids.two, .kind = Kind.Lit});
		                     })
		                     .and_then([&](bool) {
			                     return tx.insert(Lit, LitRow{.node = ids.two, .value = 2});
		                     })
		                     .and_then([&](bool) {
			                     return tx.insert(Node, NodeRow{.id = ids.three, .kind = Kind.Lit});
		                     })
		                     .and_then([&](bool) {
			                     return tx.insert(Lit, LitRow{.node = ids.three, .value = 3});
		                     })
		                     .and_then([&](bool) {
			                     return tx.insert(Node, NodeRow{.id = ids.sum, .kind = Kind.Add});
		                     })
		                     .and_then([&](bool) {
			                     return tx.insert(Add, AddRow{.node = ids.sum, .lhs = ids.two, .rhs = ids.three});
		                     })
		                     .and_then([&](bool) {
			                     return tx.insert(Parent, ParentRow{.child = ids.two, .parent = ids.sum});
		                     })
		                     .and_then([&](bool) {
			                     return tx.insert(Parent, ParentRow{.child = ids.three, .parent = ids.sum});
		                     });
...
auto run_cases(std::string_view fixtures_path, std::vector<CaseResult>& results) -> void {
	...
	auto const golden = fixtures.has_value() ? golden_of(*fixtures, "r10") : std::nullopt;
	if (!golden.has_value()) {
		results.push_back(CaseResult{.name = "fixtures file carries an r10 line", .passed = false});
		return;
	}

	auto const dir = make_store_dir();
	if (!dir.has_value()) {
		results.push_back(CaseResult{.name = "r10 store directory", .passed = false});
		return;
	}
	auto db = bdb::Db::ephemeral(dir->native(), Ast);
	if (!db.has_value()) {
		std::println("  Db::ephemeral: {}", db.error().message());
		results.push_back(CaseResult{.name = "Db::ephemeral admits Ast", .passed = false});
		return;
	}
	...
	results.push_back(CaseResult{
	    .name = "an Add header without its Add arm is commit-rejected "
	            "(arm totality)",
	    .passed = !armless.has_value() && armless.error().kind() == bdb::ErrorKind::CommitRejected && !armless.error().violations().empty(),
	});
