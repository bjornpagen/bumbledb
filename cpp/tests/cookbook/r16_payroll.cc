...
inline constexpr auto Payroll =
    bdb::schema<"Payroll">(FiscalYear, PayPeriod,
...
[[nodiscard]] auto make_store_dir() -> std::optional<std::filesystem::path> {
	...
	auto const dir = root / std::format("bumbledb-cookbook-r16-{:08x}{:08x}", device(), device());
...
[[nodiscard]] auto seed(bdb::Db& db) -> std::optional<std::uint64_t> {
	...
	auto written = db.write([&](bdb::WriteTx& tx) -> Result {
		...
		auto rows_land =
		    tx.insert(FiscalYear, FiscalYearRow{.id = *year, .span = bdb::interval<std::int64_t>::literal(0, 30)})
		        .and_then([&](bool) {
			        return tx.insert(PayPeriod, PayPeriodRow{.year = *year, .seq = 1, .span = bdb::interval<std::int64_t>::literal(0, 10)});
		        })
		        .and_then([&](bool) {
			        return tx.insert(PayPeriod,
			                         PayPeriodRow{.year = *year, .seq = 2, .span = bdb::interval<std::int64_t>::literal(10, 20)});
		        })
		        .and_then([&](bool) {
			        return tx.insert(PayPeriod,
			                         PayPeriodRow{.year = *year, .seq = 3, .span = bdb::interval<std::int64_t>::literal(20, 35)});
		        });
...
auto run_cases(std::string_view fixtures_path, std::vector<CaseResult>& results) -> void {
	...
	auto const golden = fixtures.has_value() ? golden_of(*fixtures, "r16") : std::nullopt;
	if (!golden.has_value()) {
		results.push_back(CaseResult{.name = "fixtures file carries an r16 line", .passed = false});
		return;
	}

	auto const dir = make_store_dir();
	if (!dir.has_value()) {
		results.push_back(CaseResult{.name = "r16 store directory", .passed = false});
		return;
	}
	auto db = bdb::Db::ephemeral(dir->native(), Payroll);
	if (!db.has_value()) {
		std::println("  Db::ephemeral: {}", db.error().message());
		results.push_back(CaseResult{.name = "Db::ephemeral admits Payroll", .passed = false});
		return;
	}
	...
	results.push_back(CaseResult{
	    .name = "an overlapping period is commit-rejected (no shared "
	            "instant)",
	    .passed = !overlapping.has_value() && overlapping.error().kind() == bdb::ErrorKind::CommitRejected &&
		...
		auto landed =
		    tx.alloc(FiscalYear.id)
		        .and_then([&](std::uint64_t minted) {
			        return tx.insert(FiscalYear, FiscalYearRow{.id = minted, .span = bdb::interval<std::int64_t>::literal(100, 120)})
			            .transform([&](bool) {
				            return minted;
			            });
		        })
		        .and_then([&](std::uint64_t minted) {
			        return tx.insert(PayPeriod,
			                         PayPeriodRow{.year = minted, .seq = 1, .span = bdb::interval<std::int64_t>::literal(100, 110)});
		        });
		if (!landed.has_value()) {
			return std::unexpected{std::move(landed).error()};
		}
	...
	results.push_back(CaseResult{
	    .name = "a holed fiscal year is commit-rejected (no gaps in the "
	            "source span)",
	    .passed = !holed.has_value() && holed.error().kind() == bdb::ErrorKind::CommitRejected && !holed.error().violations().empty(),
	});
