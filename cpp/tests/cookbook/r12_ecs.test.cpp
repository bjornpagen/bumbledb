...
inline constexpr auto Renderable =
    bdb::relation<"Renderable", RenderableRow>;

inline constexpr auto Ecs = bdb::schema<"Ecs">(
    Entity,
    Transform,
    Velocity,
    Renderable,
    ...
    bdb::key(Transform.entity),
    bdb::contained(
        bdb::on(Transform.entity),
        bdb::on(Entity.id)
    ),
    bdb::key(Velocity.entity),
    bdb::contained(
        bdb::on(Velocity.entity),
        bdb::on(Entity.id)
    ),
    bdb::key(Renderable.entity),
    ...
    // "Entity.id").
    bdb::contained(
        bdb::on(Renderable.entity),
        bdb::on(Transform.entity)
    )
);

// The physics join is the component intersection.
inline constexpr auto Physics =
    bdb::query(Ecs).rule([](auto r) consteval {
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r12-{:08x}{:08x}", device(),
            device());
...
auto seed(bdb::Db& db) -> std::optional<SeedIds> {
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        ...
        auto const mint = [&](std::uint64_t& out)
            ...
            return tx.alloc(Entity.id).transform(
                [&](std::uint64_t minted) {
                    out = minted;
                    return true;
                });
        ...
        auto rows_land = mint(ids.player)
            .and_then([&](bool) { return mint(ids.rock); })
            .and_then([&](bool) {
                return tx.insert(Entity,
                    EntityRow{
                        .id = ids.player, .name = std::string{"player"}});
            })
            .and_then([&](bool) {
                return tx.insert(Entity,
                    EntityRow{
                        .id = ids.rock, .name = std::string{"rock"}});
            })
            .and_then([&](bool) {
                return tx.insert(Transform,
                    TransformRow{.entity = ids.player, .x = 1, .y = 2});
            })
            .and_then([&](bool) {
                return tx.insert(Velocity,
                    VelocityRow{.entity = ids.player, .dx = 3, .dy = 4});
            })
            .and_then([&](bool) {
                return tx.insert(Renderable,
                    RenderableRow{.entity = ids.player,
                        .mesh = std::string{"cube"}});
            })
            .and_then([&](bool) {
                return tx.insert(Transform,
                    TransformRow{.entity = ids.rock, .x = 5, .y = 6});
            });
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r12") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r12 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r12 store directory", .passed = false});
        return;
    }
    ...
    if (!db.has_value()) {
        ...
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Ecs", .passed = false});
        return;
    }
    ...
    // containment.
    auto bare = db->write([&](bdb::WriteTx& tx)
            -> std::expected<
        ...
        auto landed = tx.alloc(Entity.id)
            .and_then([&](std::uint64_t ghost) {
                return tx.insert(Entity,
                        EntityRow{
                            .id = ghost, .name = std::string{"ghost"}})
                    .transform([&](bool) { return ghost; });
            })
            .and_then([&](std::uint64_t ghost) {
                return tx.insert(Renderable,
                    RenderableRow{.entity = ghost,
                        .mesh = std::string{"wisp"}});
            });
        if (!landed.has_value()) {
    ...
    results.push_back(CaseResult{
        ...
        .passed = !bare.has_value()
            && bare.error().kind() == bdb::ErrorKind::CommitRejected
            && !bare.error().violations().empty(),
    });
    ...
    results.push_back(CaseResult{
        ...
        .passed = moving.has_value() && moving->size() == 1
            && moving->rows().front().entity == ids->player
            && moving->rows().front().x == 1
            && moving->rows().front().y == 2
            && moving->rows().front().dx == 3
