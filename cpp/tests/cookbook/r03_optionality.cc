...
inline constexpr auto MailingAddress =
    bdb::relation<"MailingAddress", MailingAddressRow>;

inline constexpr auto Optionality = bdb::schema<"Optionality">(
    Business,
    MailingAddress,
    ...
    bdb::contained(
        bdb::on(MailingAddress.business),
        bdb::on(Business.id)
    )
);
...
inline constexpr auto Unaddressed =
    bdb::query(Optionality).rule([](auto r) consteval {
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r03-{:08x}{:08x}", device(),
            device());
...
auto seed(bdb::Db& db) -> std::optional<SeedIds> {
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        ...
        auto rows_land =
            tx.insert(Business,
                  BusinessRow{.id = *acme, .name = "acme"})
                .and_then([&](bool) {
                    return tx.insert(Business,
                        BusinessRow{.id = *shell, .name = "shell"});
                })
                .and_then([&](bool) {
                    return tx.insert(MailingAddress,
                        MailingAddressRow{.business = *acme,
                            .line = "1 Main St",
                            .city = "Springfield"});
                });
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r03") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r03 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r03 store directory", .passed = false});
        return;
    }
    ...
    if (!db.has_value()) {
        ...
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Optionality", .passed = false});
        return;
    }
    ...
    results.push_back(CaseResult{
        ...
        .passed = answers.has_value() && answers->size() == 1
            && answers->rows().front().id == ids->shell,
    });
