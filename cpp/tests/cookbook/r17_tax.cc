...
inline constexpr auto Status =
    bdb::closed<"Status", "Single", "MarriedJoint", "HeadOfHousehold">();
...
inline constexpr auto Tax = bdb::schema<"Tax">(
    Status,
    Regime,
    Bracket,
    Residency,
    ...
    // coordinate.
    bdb::contained(
        bdb::on(Earned.person, Earned.span),
        bdb::on(Residency.person, Residency.span)
    )
);
...
// closed-handle field.
inline constexpr auto Marginal =
    bdb::query(Tax).rule([](auto r) consteval {
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r17-{:08x}{:08x}", device(),
            device());
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        ...
        auto rows_land =
            tx.insert(Regime,
                ...
                .and_then([&](bool) {
                    return tx.insert(Bracket,
                        BracketRow{.regime = *regime,
                            .income = bdb::interval<std::int64_t>::literal(
                                0, 50),
                            .rate_bps = 1000});
                })
                .and_then([&](bool) {
                ...
                .and_then([&](bool) {
                    return tx.insert(Residency,
                        ResidencyRow{.person = 7,
                            .span = bdb::interval<std::int64_t>::literal(
                                0, 100)});
                })
                .and_then([&](bool) {
                    return tx.insert(Earned,
                        EarnedRow{.person = 7,
                            .regime = *regime,
                            .span = bdb::interval<std::int64_t>::literal(
                                10, 20),
...
auto marginal_rates(bdb::Db& db, bdb::Prepared<Marginal>& prepared,
    std::int64_t year, std::uint64_t status, std::int64_t taxable)
    -> std::optional<std::vector<std::int64_t>> {
    auto result =
        db.execute(prepared, {.y = year, .s = status, .taxable = taxable})
            .transform([](bdb::Answers<Marginal> answers) {
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r17") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r17 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r17 store directory", .passed = false});
        return;
    }
    ...
    if (!db.has_value()) {
        ...
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits Tax", .passed = false});
        return;
    }
    ...
    auto const at_30 = marginal_rates(*db, *marginal, 2024,
        Status.Single.index, 30);
    results.push_back(CaseResult{
        .name = "marginal(2024, Single, 30) answers {1000}",
        .passed = at_30.has_value()
            && *at_30 == std::vector<std::int64_t>{1000},
    });
    auto const at_75 = marginal_rates(*db, *marginal, 2024,
        Status.Single.index, 75);
    results.push_back(CaseResult{
        .name = "marginal(2024, Single, 75) answers {2000}",
        .passed = at_75.has_value()
            && *at_75 == std::vector<std::int64_t>{2000},
    });
    auto const at_top = marginal_rates(*db, *marginal, 2024,
        Status.Single.index, 1'000'000);
    results.push_back(CaseResult{
        .name = "marginal(2024, Single, 1000000) answers {3000} (the ray "
                "holds every point above its start)",
        .passed = at_top.has_value()
            && *at_top == std::vector<std::int64_t>{3000},
    });
    auto const wrong_status = marginal_rates(*db, *marginal, 2024,
        Status.MarriedJoint.index, 30);
    results.push_back(CaseResult{
    ...
    auto duplicated = db->write([&](bdb::WriteTx& tx)
            -> std::expected<
                ...
                bdb::Error> {
        auto landed = tx.alloc(Regime.id).and_then(
            [&](std::uint64_t minted) {
                return tx.insert(Regime,
                    RegimeRow{.id = minted,
                        .year = 2024,
                        .status = Status.Single});
            });
    ...
    auto outside = db->write([&](bdb::WriteTx& tx)
            -> std::expected<
                bdb::WriteDecision<std::monostate, std::monostate>,
                bdb::Error> {
        auto landed = tx.insert(Earned,
            EarnedRow{.person = 7,
                .regime = *regime,
                .span = bdb::interval<std::int64_t>::literal(150, 160),
                .minor = 900});
        if (!landed.has_value()) {
            return std::unexpected{std::move(landed).error()};
        }
    ...
    results.push_back(CaseResult{
        .name = "an Earned fact outside residency is commit-rejected "
                ...
                "period)",
        .passed = !outside.has_value()
            && outside.error().kind() == bdb::ErrorKind::CommitRejected
            && !outside.error().violations().empty(),
    });
