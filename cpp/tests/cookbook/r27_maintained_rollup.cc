...
inline constexpr auto Deriving =
    bdb::query(MaintainedRollup).rule([](auto r) consteval {
...
auto make_store_dir() -> std::optional<std::filesystem::path> {
    ...
    auto const dir = root
        / std::format("bumbledb-cookbook-r27-{:08x}{:08x}", device(),
            device());
...
auto seed(bdb::Db& db) -> bool {
    ...
    auto written = db.write([&](bdb::WriteTx& tx) -> Result {
        auto const claim = [&](std::uint64_t source, std::uint64_t person,
            auto arm, bdb::interval<std::int64_t> span)
            -> std::expected<bool, bdb::Error> {
            return tx.insert(Claim,
                ClaimRow{.source = source,
                    .person = person,
                    .arm = arm,
                    .span = span});
        };
        auto rows_land = claim(1, 1, Arm.Busy,
            bdb::interval<std::int64_t>::literal(10, 20))
            .and_then([&](bool) {
                return claim(2, 1, Arm.Busy,
                    bdb::interval<std::int64_t>::literal(20, 30));
            })
            .and_then([&](bool) {
                return claim(3, 1, Arm.Ooo,
                    bdb::interval<std::int64_t>::literal(40, 50));
            })
            .and_then([&](bool) {
                return claim(4, 2, Arm.Busy,
                    bdb::interval<std::int64_t>::literal(5, 10));
            });
...
auto store_span(bdb::Db& db, std::uint64_t person,
    bdb::interval<std::int64_t> span)
    ...
    return db.write([&](bdb::WriteTx& tx)
            -> std::expected<
                ...
                bdb::Error> {
        auto landed = tx.insert(BusySpan,
            BusySpanRow{.person = person, .span = span});
        if (!landed.has_value()) {
...
auto run_cases(std::string_view fixtures_path,
    ...
    auto const golden =
        fixtures.has_value() ? golden_of(*fixtures, "r27") : std::nullopt;
    if (!golden.has_value()) {
        results.push_back(CaseResult{
            .name = "fixtures file carries an r27 line", .passed = false});
        return;
    }
    ...
    if (!dir.has_value()) {
        results.push_back(CaseResult{
            .name = "r27 store directory", .passed = false});
        return;
    }
    auto db = bdb::Db::ephemeral(dir->native(), MaintainedRollup);
    if (!db.has_value()) {
        results.push_back(CaseResult{
            .name = "Db::ephemeral admits MaintainedRollup",
            .passed = false});
        return;
    }
    ...
    results.push_back(CaseResult{
        ...
        .passed = derived.has_value() && rows.size() == 2
            && rows[0].first == 1
            && rows[0].second
    ...
    auto unsound =
        store_span(*db, 1, bdb::interval<std::int64_t>::literal(40, 50));
    results.push_back(CaseResult{
        .name = "a rollup row with no Busy claim behind it is "
                "commit-rejected (soundness, not freshness)",
        .passed = !unsound.has_value()
            && unsound.error().kind() == bdb::ErrorKind::CommitRejected
            && !unsound.error().violations().empty(),
    });
