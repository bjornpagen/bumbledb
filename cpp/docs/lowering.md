...
produce (TODO_CPP.md:881-889, 1986-2001). Every claim below is cited against
...
1. The C++ frontend builds a `SchemaSpec` (named plain data) and hands it to
   ...
   source of truth (TODO_CPP.md:881).
...
fingerprinted (spec.rs:71-76, 91-97; TODO_CPP.md:574, 886). They exist for
...
7. **Fingerprint neutrality.** Class names flow ONLY into `newtype` slots and
   ...
   spec.rs:426-428; TODO_CPP.md:680-682). Getting a class name wrong cannot
   move a fingerprint — it fails handle resolution, the coherence check, or
   cross-host diagnostics instead.

C++ binding: TODO_CPP.md:886-889 makes this discipline normative
...
`get` panics out-of-range (TODO_CPP.md:186-190). Answers are SETS — no order
...
- **How TS reads it**: `native.dbFingerprint(db)` returns the 64-hex readback
  ...
  `ts/crate/src/fingerprint_lock.rs`). Per TODO_CPP.md:1993-2001 the
