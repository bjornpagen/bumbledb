// bumbledb.meta.row — reflected row-to-Value marshalling (TODO_CPP §24).
//
// GCC-only reflection zone. This is the dynamic-insert lane later used by
// `tx.insert(Relation, Row{...})`: a relation row product lowers to a
// contiguous array of tagged `bdb_value` cells, one per field, in
// declaration order, ready for `bdb_tx_insert`-style calls. The row
// declaration and reflection are the sole source of truth — no serializer
// registration, no manual to_bumbledb(), no per-relation specialization.
export module bumbledb.meta.row;
...
template<class Row>
inline constexpr std::size_t row_field_count =
    detail::field_count(^^Row);
...
template<class Row>
[[nodiscard]] auto marshal_row(Row const& row)
    -> std::array<foreign::bdb_value, row_field_count<Row>> {
    static_assert(
        detail::row_is_supported(^^Row),
        detail::unsupported_field_message(
            detail::row_subject(^^Row), ^^Row));
    ...
    template for (
        ...
        if constexpr (cls.kind == value_kind::boolean) {
            ...
            cell.kind =
                foreign::bdb_value_kind::BDB_VALUE_KIND_FIXED_BYTES;
            ...
            cell.kind =
                foreign::bdb_value_kind::BDB_VALUE_KIND_INTERVAL_U64;
            cell.interval_u64_start = value.lo();
            cell.interval_u64_end = value.hi();
        } else {
            cell.kind =
                foreign::bdb_value_kind::BDB_VALUE_KIND_INTERVAL_I64;
