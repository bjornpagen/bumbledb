...
consteval auto pred_rule_count(query_ir const& ir, std::size_t pred)
    -> std::size_t {
    return pred < ir.rec_count ? ir.recs[pred].rule_count : ir.rule_count;
}

consteval auto pred_rule(query_ir const& ir, std::size_t pred,
    std::size_t rule) -> wire_rule const& {
    return pred < ir.rec_count ? ir.recs[pred].rules[rule]
                               : ir.rules[rule];
}

consteval auto pred_head_count(query_ir const& ir, std::size_t pred)
    -> std::size_t {
    return pred < ir.rec_count ? ir.recs[pred].head_count : ir.head_count;
}

consteval auto pred_head(query_ir const& ir, std::size_t pred,
    std::size_t column) -> find_data const& {
    return pred < ir.rec_count ? ir.recs[pred].head[column]
                               : ir.head[column];
}
...
consteval auto find_of(wire_find const& find) -> bdb_find_term {
    ...
    out.op = bdb_agg_op{
        ...
        .arg_key_kind = find.key_is_measure
            ? bdb_arg_key_kind::BDB_ARG_KEY_KIND_MEASURE
            : bdb_arg_key_kind::BDB_ARG_KEY_KIND_VAR,
...
consteval auto binding_total(query_ir const& ir) -> std::size_t {
    ...
    for (auto pred = std::size_t{0}; pred != predicate_total(ir); ++pred) {
        for (auto rule = std::size_t{0};
            rule != pred_rule_count(ir, pred); ++rule) {
            auto const& wire = pred_rule(ir, pred, rule);
            for (auto atom = std::size_t{0}; atom != wire.atom_count;
                ++atom) {
                total += wire.atoms[atom].binding_count;
            }
            for (auto atom = std::size_t{0}; atom != wire.negated_count;
                ++atom) {
...
consteval auto make_bindings()
    ...
    for (auto pred = std::size_t{0}; pred != predicate_total(Query.ir);
        ++pred) {
        for (auto rule = std::size_t{0};
            rule != pred_rule_count(Query.ir, pred); ++rule) {
            auto const& wire = pred_rule(Query.ir, pred, rule);
            auto const copy = [&](wire_atom const& atom) {
                for (auto binding = std::size_t{0};
                    binding != atom.binding_count; ++binding) {
                    out[at] = bdb_binding{
...
consteval auto atom_source_of(wire_atom const& atom)
    -> bdb_atom_source_kind {
    return atom.idb ? bdb_atom_source_kind::BDB_ATOM_SOURCE_KIND_IDB
        ...
        for (auto rule = std::size_t{0};
            ...
            for (auto atom = std::size_t{0}; atom != wire.atom_count;
                ...
                out[at] = bdb_atom{
                    ...
                    .pred = source.pred,
                    .bindings = source.binding_count == 0
                        ? nullptr
...
template<auto Query>
consteval auto make_negated()
    -> std::array<bdb_atom, negated_total(Query.ir)> {
    ...
    for (auto pred = std::size_t{0}; pred != predicate_total(Query.ir);
        ...
        for (auto rule = std::size_t{0};
            ...
            for (auto condition = std::size_t{0};
                condition != wire.condition_count; ++condition) {
...
consteval auto make_finds()
    ...
    for (auto pred = std::size_t{0}; pred != predicate_total(Query.ir);
        ...
        for (auto rule = std::size_t{0};
            rule != pred_rule_count(Query.ir, pred); ++rule) {
            auto const& wire = pred_rule(Query.ir, pred, rule);
            for (auto find = std::size_t{0}; find != wire.find_count;
                ++find) {
    ...
    for (auto pred = std::size_t{0}; pred != predicate_total(Query.ir);
        ...
        for (auto rule = std::size_t{0};
            ...
            out[at] = bdb_rule{
                ...
                .negated = wire.negated_count == 0
                    ? nullptr
                    : program_negated<Query>.data() + negated_offset,
...
template<auto Query>
consteval auto make_heads()
    -> std::array<bdb_head_term, head_total(Query.ir)> {
    auto out = std::array<bdb_head_term, head_total(Query.ir)>{};
    auto at = std::size_t{0};
    for (auto pred = std::size_t{0}; pred != predicate_total(Query.ir);
        ++pred) {
        for (auto index = std::size_t{0};
            index != pred_head_count(Query.ir, pred); ++index) {
...
template<auto Query>
consteval auto make_predicates()
    -> std::array<bdb_predicate, predicate_total(Query.ir)> {
    ...
    for (auto pred = std::size_t{0}; pred != predicate_total(Query.ir);
        ...
        out[pred] = bdb_predicate{
            .head = heads == 0
                ? nullptr
                : program_heads<Query>.data() + head_offset,
            .head_count = heads,
            .rules = rules == 0
                ? nullptr
                : program_rules<Query>.data() + rule_offset,
            .rule_count = rules,
        };
        rule_offset += rules;
        head_offset += heads;
...
[[nodiscard]] auto wire_one(Member const& value, param_scratch&)
    -> bdb_param {
...
[[nodiscard]] auto wire_one(std::span<Element const> values,
    param_scratch& scratch) -> bdb_param {
...
[[nodiscard]] auto wire_params(Params const& params, param_scratch& scratch) {
    ...
    return std::array<bdb_param, sizeof...(values)>{
        wire_one(values, scratch)...};
}
...
template<auto Query>
consteval auto make_membership_cells()
    -> std::array<bdb_value, membership_cell_total(Query.ir)> {
    auto out =
        std::array<bdb_value, membership_cell_total(Query.ir)>{};
    auto at = std::size_t{0};
    for (auto index = std::size_t{0}; index != Query.ir.param_count;
        ...
        for (auto member = std::size_t{0};
            member != parameter.member_count; ++member) {
...
[[nodiscard]] auto wire_params_for(Params const& params,
    param_scratch& scratch)
    -> std::array<bdb_param, Query.ir.param_count> {
