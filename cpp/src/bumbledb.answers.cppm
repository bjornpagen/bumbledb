...
/// the query's named row product (`bdb::Answers<Query>` in bumbledb.db
...
class RowAnswers {
    ...
    /// packs; no reflection — this module belongs to both graphs). Never
    /// called: only its TYPE is read, so no member is ever
...
public:
    ...
    /// module belongs to both graphs, so the one honest spelling left is
    /// termination (the raii module's rule).
