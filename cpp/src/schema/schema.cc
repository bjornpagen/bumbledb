/**
 * The consteval schema elaborator and the schema value (lowering.md
 * §2–§3). Statements are VALUES — laws are first-class: `bdb::key(...)`,
 * `bdb::contained(...)`, `bdb::mirrors(...)`, `bdb::capacity(...)`
 * construct literal structural values whose IDENTITY rides their types,
 * so `bdb::schema<"Uptime">(...)` runs the whole class-law computation —
 * the union-find of lowering.md §3 — during constant evaluation, with
 * §34 diagnostics that name semantic coordinates.
 */
...
/**
 * The relations-as-members injection. The member-type branch must be
 * `if constexpr` — the name reader may only be instantiated for member
 * types — and a duplicate name skips so the injection stays total and
	...
	consteval {
		...
		[[maybe_unused]] auto const add = [&]<class A>() {
			if constexpr (is_member_type(^^A)) {
				...
				specs.push_back(std::meta::data_member_spec(^^A, {
				                                                     .name = spec_name(name.view())}));
			}
		};
...
struct fresh_pattern_types {
	...
	consteval {
		...
		for (auto const member : std::meta::nonstatic_data_members_of(^^Facade, std::meta::access_context::current())) {
			auto const args = std::meta::template_arguments_of(std::meta::dealias(std::meta::type_of(member)));
			if (std::meta::extract<bool>(args[5])) {
				specs.push_back(std::meta::data_member_spec(args[0], {.name = spec_name(std::meta::extract<name_text>(args[2]).view())}));
			}
		}
 ...
 */
template<class Facade>
using fresh_pattern_of = typename detail::fresh_pattern_types<Facade>::Pattern;
...
/**
 * A whole theory as one literal value: relations as members (argument
 * order = declaration order = RelationId mint), the flattened SchemaSpec
 * data (DECLARED statements only, written order), and the law-computed
 * class map with a consteval lookup. Structural — a schema travels as a
 * template argument.
 */
...
struct schema_value {
	 ...
	 * coordinate rendered "Relation.field"; class names feed newtype
	 * slots ONLY and never move the fingerprint (lowering.md §1.10,
	 * §3.7).
	 */
	template<class Coordinate>
	[[nodiscard]] consteval auto class_of(Coordinate) const -> std::optional<coord_ref> {
		static_assert(detail::is_coordinate_v<Coordinate>, "bumbledb schema::class_of(): the argument must be a relation "
		                                                   "coordinate (Relation.field)");
		for (auto const& entry : classes) {
			if (entry.coordinate.relation == Coordinate::relation_name && entry.coordinate.field == Coordinate::field_name) {
				if (!entry.classed) {
					return std::nullopt;
				}
...
/**
 * The consteval schema elaborator: relations first (in declaration
 * order), then statements (in written order). Runs the class laws and
 * every structurally decidable §34 wall; hands NOTHING semantic to the
 * wire beyond names — the engine's SchemaSpec::descriptor() remains the
 * authority. The value-borne payloads — capacity windows, face σ/ψ
 * selections, closed relations' sealed extensions — are copied off the
 * argument VALUES (the type tier carries only the sealed roster).
 */
template<fixed_string Name, class... Args>
[[nodiscard]] consteval auto schema(Args const&... args) -> schema_value<Name, Args...> {
	static_assert(detail::args_recognized<Args...>(), "bumbledb schema(): every argument must be a relation facade "
	                                                  ...
	                                                  "bdb::contained / bdb::mirrors / bdb::capacity)");
	static_assert(detail::relation_count<Args...>() > 0, "bumbledb schema(): a schema declares at least one relation");
	static_assert(detail::relations_lead<Args...>(), "bumbledb schema(): relations precede statements — declare every "
	                                                 "member facade before the first statement");
	static_assert(detail::relation_names_distinct<Args...>(), "bumbledb schema(): two member relations share one name");

	static_assert(detail::analyze_schema<Args...>().members_known, detail::membership_message<Args...>(Name.view()));
	static_assert(detail::analyze_schema<Args...>().lawful, detail::wall_message<Args...>(Name.view()));
	static_assert(detail::analyze_schema<Args...>().no_restated_implied_key, detail::restated_key_message<Args...>(Name.view()));
	static_assert(detail::analyze_schema<Args...>().no_duplicate_key, detail::duplicate_key_message<Args...>(Name.view()));
	...
	auto const fill = [&]<class A>(A const& argument) {
		if constexpr (detail::is_statement_v<A>) {
			if constexpr (detail::is_capacity_v<A>) {
				...
				copy_selection(out.statements[index].target, argument.target);
				copy_selection(out.statements[index].source, argument.source);
			}
...
concept Theory = requires(S const& theory) {
	requires std::same_as<std::ranges::range_value_t<decltype(theory.relation_table)>, relation_data>;
	requires std::same_as<std::ranges::range_value_t<decltype(theory.statements)>, statement_data>;
	requires std::same_as<std::ranges::range_value_t<decltype(theory.classes)>, class_entry>;
};
