...
/**
 * The key pattern injection: one member per projected coordinate, named
 * by the field, typed by the field's physical type — so
 * `db.get(Outage, outage_key, {.service = s, .window = w})` marshals
...
/**
 * A stored key law value: the functional-dependency law, which pairs
 * NOTHING in the class laws. The law itself is the selector — get()
