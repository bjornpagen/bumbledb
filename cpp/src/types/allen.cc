// :allen — the checked Allen-relation mask and the engine's named Allen
// vocabulary (TODO_CPP §7, §21). Query bind-time vocabulary only — never a
// row field type (lowering.md §1.6). The invalid-literal path
// uses the static failure spelling (see :interval's module comment).
...
// Never defined; see :interval's module comment. Reaching this in the
...
/// only — never a row field type (lowering.md §1.6).
class allen_mask {
	std::uint16_t bits_;
