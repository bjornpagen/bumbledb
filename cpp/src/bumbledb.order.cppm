// bumbledb.order — host-side answer ordering (ts/src/order.ts). Answers
...
struct Ordering {
    ...
    template<class Row>
    [[nodiscard]] constexpr auto operator()(Row const& left,
        Row const& right) const -> bool {
        if constexpr (sizeof...(Keys) == 0) {
            ...
            std::apply(
                [&](Keys const&... each) -> void {
                    static_cast<void>(
                        ((verdict = key_order(left, right, each),
                             verdict != 0)
                            || ...));
                },
                keys);
...
private:
    ...
    template<class T>
    static constexpr auto cell_order(T const& left, T const& right)
        -> std::strong_ordering {
        if constexpr (requires {
            ...
            if (auto const starts = left.lo() <=> right.lo();
                starts != 0) {
                return starts;
            }
            ...
            return std::lexicographical_compare_three_way(
                std::ranges::begin(left), std::ranges::end(left),
                std::ranges::begin(right), std::ranges::end(right));
        } else {
    ...
    static constexpr auto key_order(Row const& left, Row const& right,
        Key const& key) -> std::strong_ordering {
