...
/**
 * The typed answers carrier over one synthesized row product: wraps the
 * reusable flat buffer and decodes whole rows into the query's named row
...
class [[nodiscard]] RowAnswers {
	...
	/**
	 * Never called: only its TYPE is read, so no member is ever
	 * default-constructed (interval members have no default state by
	 * design).
	 */
	static auto member_tuple(Row const& row) {
		auto const& [... members] = row;
		return std::type_identity<std::tuple<std::remove_cvref_t<decltype(members)>...>>{};
	}

	using RowTuple = decltype(member_tuple(std::declval<Row const&>()))::type;
	...
	/**
	 * Parenthesized aggregate init on purpose: no member is ever
	 * default-constructed (interval members cannot be).
	 */
...
public:
	...
	/**
	 * One decoded row, bounds- and shape-checked: nullopt out of range
	 * or on a column/row-type mismatch (possible only when this carrier
	 * was filled by a DIFFERENT query's execution — the typed execute
	 * lane fills it with its own query).
	 */
