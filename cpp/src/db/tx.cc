...
/**
 * The lexical borrowed write capability: alive exactly for the Db::write
 * / Db::write_from callback. Non-copyable, non-movable, constructible
 * only by Db's trampoline. Nothing is judged until commit; the
 * callback's decision is the commit/abandon switch.
 */
class WriteTx {
...
public:
	...
	/**
	 * Records an insert into the delta (reflection-marshalled, member
	 * declaration order); true = the final state changed. Shape
	 * violations are the engine's typed FactShape error — this phase
	 * does not type-check Row against Facade.
	 */
	...
	/**
	 * Final-state keyed point read (base + pending delta): the stored
	 * key law value is the selector.
	 */
	...
	/**
	 * The fresh-field primary read against the final state.
	 */
	...
	/**
	 * Mints the next fresh id for the coordinate's field:
	 * `tx.alloc(Service.id)`. Fresh fields are u64 by construction, so
	 * only u64 coordinates allocate.
	 */
