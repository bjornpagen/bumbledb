...
/**
 * One member handle of a closed vocabulary: `Kind.DirectPass` is a value
 * of this type. The roster, the handle name, and the declaration-order
 * row id all ride the TYPE, so handle uses resolve and roster-check
 * during constant evaluation (queries resolve handles HOST-side —
 * lowering.md §7.8).
 */
...
/**
 * A closed-reference column's value type: `bdb::ref<Kind.id>` dealiases
 * to this. Physically the engine's u64 handle row id; the TYPE carries
 * the vocabulary, so a foreign handle cannot cross into the field (the
 * §34 wrong-vocabulary wall lives on the converting constructor). The
 * newtype label the wire carries stays LAW-COMPUTED (lowering.md §3);
