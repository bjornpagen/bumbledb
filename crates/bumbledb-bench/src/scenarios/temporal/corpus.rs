...
/// After the random loop, deterministic witnesses land on the low keys that
/// exist at every scale: per key `k in 0..8` one exact-abutment MEETS pair and
/// one strict-containment DURING pair, then two planted rays on keys 0 and 1
/// (so ≥ 2 rays exist even at SMOKE).
