//! Shared strict JSON and canonical scalar spelling for schema and durable metadata.
...
impl Cursor<'_> {
    ...
    fn value(&mut self, depth: usize) -> JsonResult<Json> {
        if depth > 64 {
            return Err("nesting limit");
        }
        self.skip_ws();
        match self.peek() {
            Some(b'{') => Ok(Json::Object(self.object(depth + 1)?)),
            Some(b'[') => Ok(Json::Array(self.array(depth + 1)?)),
    ...
    fn object(&mut self, depth: usize) -> JsonResult<BTreeMap<String, Json>> {
        self.eat(b'{')?;
        self.skip_ws();
    ...
    fn array(&mut self, depth: usize) -> JsonResult<Vec<Json>> {
        self.eat(b'[')?;
        self.skip_ws();
...
mod tests {
    ...
    #[test]
    fn duplicate_keys_and_excessive_nesting_refuse() {
        assert!(read_tree(r#"{"relations":[],"relations":[]}"#).is_err());
        assert!(read_tree(&format!("{}null{}", "[".repeat(65), "]".repeat(65))).is_err());
    }

    #[test]
    fn escaped_surrogate_pairs_preserve_unicode_without_admitting_lone_surrogates() {
        assert_eq!(read_tree(r#""\ud83d\ude00""#).unwrap().as_str(), Some("😀"));
        for raw in [r#""\ud83d""#, r#""\ude00""#, r#""\ud83d\u0041""#] {
            assert!(read_tree(raw).is_err(), "{raw}");
        }
    }
}
