...
//! scratch and return the written prefix — no oversized zeroing
...
impl Namespace {
    ...
    /// Parses a stored tag. `None` on any byte this enum does not name.
    #[must_use]
...
/// Narrows `key` to `expected`'s body. `None` on a missing, unnamed, or
/// other-namespace tag.
...
impl StatKind {
    /// Parses a stored discriminant. `None` on any byte the enum does not
    /// name — the sweeper keeps that unknown arm.
    #[must_use]
    pub const fn from_byte(byte: u8) -> Option<Self> {
        match byte {
...
/// sequence of `put_*` calls returning the written prefix.
struct KeyWriter<'a> {
    buf: &'a mut [u8],
    len: usize,
}
...
pub fn parse_reverse_key(key: &[u8]) -> Option<(StatementId, &[u8], RelationId, u64)> {
    // The split chain is the length check: tag narrowed to Reverse, then
    // statement(2) off the front, source_row(8) then source_rel(4) off
    // the back — anything shorter than R_OVERHEAD fails a split.
...
/// validation mints `key_projection` as source fields already in
/// target-key determinant order, so this gather is the only per-fact
/// encoder. Re-measure rides the end-of-campaign bench night (R20/R21).
...
mod tests {
    ...
    fn sealed_key_projection_lays_fields_in_target_key_order() {
        let layout = interval_layout();
        let fact = interval_fact();
        // Sealed `key_projection`: source fields in target-key determinant
        // order (f0, f2, interval f1). A 3-cycle against declaration order.
