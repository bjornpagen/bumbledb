...
/// belongs to the writing constructors (`Db` handles) and to nothing
/// else; the read-only lane opens `MDB_RDONLY` and takes none (a
/// read-only environment can corrupt nothing, so there is nothing for
/// a lock to protect). A held lock — another process, or another live
/// writing `Environment` on the same path in this process — is
/// `Error::EnvironmentLocked`, converting the silent derived-state
