...
trace_points! {
    ...
    /// `Environment::publish`'s catalog data copy. (bytes)
    PUBLISH_COPY => "publish_copy", Storage,
    /// Publish's durability boundary: the staging fsync sweep, then the
    /// destination dirent-chain fsync — one span each.
