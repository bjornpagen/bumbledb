//! PRD 17: interval machinery in the executor — the membership
//! point-var join (`PlanNode::point_probes`) and the decomposed
//! `Overlaps`/`Contains` word residuals over two-slot interval
//! variables (docs/architecture/20-query-ir.md, § normalization;
//! 40-execution, § access paths).

use super::*;
use crate::image::view::{FilterPredicate, ResolvedWordSource};
use crate::ir::normalize::{IntervalWord, PlacedWordComparison, SlotWidth, VarWord};
...
/// `A(ta, i), B(tb, j)` with the given decomposed word residuals —
/// exactly the shape normalization emits for a cross-atom
/// `Overlaps`/`Contains` (pinned in `ir/normalize/tests.rs`).
fn interval_pair_query(word_residuals: Vec<PlacedWordComparison>) -> NormalizedQuery {
    let occurrences = vec![
        Occurrence {
            occ_id: OccId(0),
...
fn surviving_tags(
    ...
    let query = interval_pair_query(word_residuals);
...
/// `Overlaps(i, j)` ≡ `i.start < j.end AND j.start < i.end` — of the
/// thirteen pairwise Allen configurations exactly the nine intersecting
/// ones survive (meets/met-by share no point under half-open intervals).
#[test]
fn overlaps_residual_keeps_exactly_the_nine_intersecting_configurations() {
    let overlaps = vec![
        PlacedWordComparison {
            op: CmpOp::Lt,
            lhs: side(1, IntervalWord::Start),
            rhs: side(3, IntervalWord::End),
        },
        PlacedWordComparison {
            op: CmpOp::Lt,
    ...
    assert_eq!(
        surviving_tags("run-overlaps-01", overlaps.clone(), &[0, 1]),
        expected
    );
    // Same answer with the join order flipped (placement recomputes).
    assert_eq!(
        surviving_tags("run-overlaps-10", overlaps, &[1, 0]),
        expected
    );
}

/// `Contains(i, j)` ≡ `i.start <= j.start AND j.end <= i.end` — exactly
/// the containment configurations survive: finished-by, contains,
/// equals, started-by.
#[test]
fn contains_residual_keeps_exactly_the_containment_configurations() {
    let contains = vec![
        PlacedWordComparison {
            op: CmpOp::Le,
            lhs: side(1, IntervalWord::Start),
        ...
        PlacedWordComparison {
            op: CmpOp::Le,
            lhs: side(3, IntervalWord::End),
            rhs: side(1, IntervalWord::End),
        },
    ];
    let expected: BTreeSet<u64> = [4, 5, 7, 8].into_iter().collect();
    assert_eq!(
        surviving_tags("run-contains-01", contains.clone(), &[0, 1]),
        expected
    );
    assert_eq!(
        surviving_tags("run-contains-10", contains, &[1, 0]),
        expected
    );
}
