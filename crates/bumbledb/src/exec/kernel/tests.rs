...
/// The unchecked twin reached 0.82–0.90 — the one repeatable single-digit lead,
/// but it forfeits the crucible's deleted-`unsafe` ruling for a wall-bound
...
/// The arms alternate per span inside one process (m2max.method.interleaved-ab)
/// on L1-resident buffers; the kernel has no data-dependent branch, so
/// repetition on fixed data is the valid protocol here (the TAGE caveat's own
