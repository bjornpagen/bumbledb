...
impl OverlapCache {
    ...
    pub(crate) fn query_into(&self, dir: u32, q_start: u64, q_end: u64, out: &mut Vec<u32>) {
        ...
        if len <= FLAT_SWEEP_CEILING {
            ...
            let mut base = 0;
            for ends in ends.as_chunks::<8>().0 {
                // Sorted starts let the last start admit the whole block.
                // The first partial block and the tail retain the scalar test.
                if starts[base + 7] >= q_end {
                    break;
                }
