...
function decodeEntry(reader: ByteReader, index: number, statementCount: number): FootprintEntry {
	function shape(): never {
		refuse({ kind: "footprint-shape", index }, `footprint entry ${index} is unparseable`)
	}
...
function decodeBatch(theory: LogTheory, bytes: Uint8Array): DecodedBatch {
	...
	if (!bytesEqual(magic, MAGIC)) {
		refuse({ kind: "magic" }, "batch magic is not BDBL")
	}
	...
	if (fingerprint !== descriptor.fingerprint) {
		refuse(
			{ kind: "fingerprint", carried: fingerprint, expected: descriptor.fingerprint },
	...
	if (members === undefined) {
		refuse({ kind: "braid-unknown", braid: braidId }, `batch braid ${braid} is not derived from this descriptor`)
	}
	...
	for (let opIndex = 0; opIndex < opCount; opIndex++) {
		...
		if (kind !== OP_KIND.insert && kind !== OP_KIND.delete) {
			refuse({ kind: "op-kind", opKind: kind }, `op kind ${kind} is unknown (3 was deleted with floor bumps)`)
		}
		const relationId = reader.u32le("op relation")
		const relation = descriptor.relations[relationId]
		if (relation === undefined || !members.includes(relationId)) {
			refuse(
				{ kind: "op-relation", relation: relationId, braid },
				`op relation ${relationId} is outside braid ${braid}`
			)
		}
		...
		for (let rowIndex = 0; rowIndex < rowCount; rowIndex++) {
			...
			relation.fields.forEach(function readCell(field) {
				const refusal: TaggedRefusal = {
					badTag(): never {
						refuse(
							{ kind: "row-shape", relation: relation.name, row: rowIndex, field: field.name },
							`relation ${relation.name} row ${rowIndex} field ${field.name}: tag does not match the layout`
						)
					},
					malformed(what: string): never {
						refuse(
							{ kind: "row-shape", relation: relation.name, row: rowIndex, field: field.name },
							`relation ${relation.name} row ${rowIndex} field ${field.name}: ${what}`
						)
					}
				}
	...
	for (let index = 0; index < fpCount; index++) {
		const entry = decodeEntry(reader, index, descriptor.statements.length)
		const last = footprint[footprint.length - 1]
		if (last !== undefined && compareEntries(last, entry) >= 0) {
			refuse({ kind: "footprint-order", index }, `footprint entry ${index} is unsorted or duplicate`)
		}
		footprint.push(entry)
	}
	if (reader.remaining() !== 0) {
		refuse({ kind: "trailing", bytes: reader.remaining() }, `${reader.remaining()} trailing bytes after the footprint`)
	}
