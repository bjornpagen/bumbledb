...
function loadDescriptors(): Map<string, Descriptor> {
...
function pinned(descriptors: ReadonlyMap<string, Descriptor>, schema: string, fingerprint: string): Descriptor {
	const descriptor = descriptors.get(schema)
	if (descriptor === undefined) {
		throw errors.new(`fixture cites schema ${schema}`)
	}
	return withFingerprint(descriptor, fingerprint)
}

function documentIdentity(kind: string): string {
	if (kind === "ManifestShape" || kind === "SidecarShape" || kind === "CheckpointShape") {
		return "Malformed"
	}
	if (kind === "ManifestVersion" || kind === "SidecarVersion") {
		return "Version"
	}
	return kind
}

export { corpusRoot, documentIdentity, loadDescriptors, pinned }
