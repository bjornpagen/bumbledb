...
function corpusBody(index: number): Uint8Array {
	...
	if (size === undefined) {
		throw new LogInputError({ message: `corpus has no object ${index}` })
	}
...
async function waitForGo(root: string): Promise<void> {
	...
	while (!fs.existsSync(go)) {
		if (Date.now() > deadline) {
			throw new LogInputError({ message: "start barrier never appeared" })
		}
...
async function main(): Promise<void> {
	...
	if (role === undefined || root === undefined) {
		throw new LogInputError({ message: "usage: interop-child.ts <role> <root> [args...]" })
	}
	const s3 = root === "s3"
	...
	if (role === "write") {
		for (let i = 0; i < CORPUS_SIZES.length; i++) {
			...
			if (outcome.tag !== "created") {
				throw new LogInputError({ message: `corpus object ${i} was not created` })
			}
	...
	if (role === "read") {
		for (const key of rest) {
			...
			if (fetched === null) {
				throw new LogInputError({ message: `object ${key} is absent` })
			}
			if (!s3 && fetched.etag !== blake3Hex(fetched.bytes)) {
				throw new LogInputError({ message: `object ${key}: the reported etag is not the blake3 of the bytes` })
			}
	...
	if (role === "race-create") {
		...
		if (id === undefined || slots === undefined) {
			throw new LogInputError({ message: "race-create needs <id> <slots>" })
		}
	...
	if (role === "race-swap") {
		...
		if (id === undefined || count === undefined) {
			throw new LogInputError({ message: "race-swap needs <id> <count>" })
		}
		await waitForGo(root)
		let swapped = 0
		let moved = 0
		...
		while (swapped < target) {
			...
			if (current === null) {
				throw new LogInputError({ message: "the counter is absent" })
			}
			...
			if (outcome.tag === "swapped") {
				if (!s3 && outcome.etag !== blake3Hex(next)) {
					throw new LogInputError({ message: "a swapped etag is not the blake3 of the written bytes" })
				}
				swapped += 1
			} else {
				moved += 1
...
function s3FromEnv(prefix: string): ObjectStore {
	...
	if (bucket === undefined || accessKeyId === undefined || secretAccessKey === undefined) {
		throw new LogInputError({ message: "s3 interop needs BUMBLEDB_S3_SMOKE_BUCKET and AWS keys" })
	}
