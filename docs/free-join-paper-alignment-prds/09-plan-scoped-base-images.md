...
- Real LMDB read-snapshot integration through the `ReadTxn`/`heed::RoTxn` introduced in PRD 08.
...
- Build base images by scanning LMDB keys visible to the current `heed::RoTxn`; do not use process-local storage mirrors.
...
- Treat the LMDB read transaction as the source of truth. Any loaded vectors are immutable snapshot-local copies derived from LMDB state.
...
- The stability test must hold an actual LMDB read transaction open across a concurrent committed write.
