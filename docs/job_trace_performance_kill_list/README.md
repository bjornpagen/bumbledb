...
|---:|---|---|---|
| P0 | Persist or cheaply derive planner stats | [`01_persisted_planner_stats.md`](01_persisted_planner_stats.md) | Cut cold `plan.stats` for broad/q09 by `>90%` |
...
| Query | Planner Stats | Indexed LFTJ Build | Factorized Count | Join Static Empty | Direct Count Kernel | LFTJ Key Reads | Frontend Cache | Build Tracing |
|---|---|---|---|---|---|---|---|---|
| `job_broad_cast_keyword_company` | Critical | Medium | High | Low | Low | High | Low | Medium |
| `job_broad_movie_info_star` | Medium | Medium | Critical | Low | Low | Critical | Low | Medium |
| `job_q01_top_production` | Low | Low | Low | Critical | Medium | Low | Medium | Low |
| `job_q09_voice_us_actor` | Critical | High | Medium | Low | Low | High | Low | High |
| `job_q16_character_title_us` | Low | Critical | Low | Low | Low | Low | Low | Critical |
| `job_q24_voice_keyword_actor` | Low | Critical | Low | Low | Low | Low | Low | Critical |
| `job_movie_link_bridge` | Medium | Low | Medium | Low | Critical | Medium | Low | Low |
| `job_q33_linked_series_companies` | Low | Low | Low | Medium | Low | Low | Critical | Low |
