...
parameters, source/Space integration and persistent identity remain M6 work.
...
This constructs exact arithmetic under the stated experiment assumption. It is
not yet an Event observation backed by a parameterized source owner. The original
...
`equivalent` and `included` are exact scoped judgments. Root presentations are
not canonical numeric bytes, so regions have no derived `Eq`, `Hash` or wire
identity. A witness's defining-polynomial indeterminate is a numerical
...
A general rational denominator may be negative. A future probability-law or
observation constructor must separately prove nonnegative mass, normalization,
