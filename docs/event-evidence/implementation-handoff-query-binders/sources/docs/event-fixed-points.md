...
one sealed finite Event presentation. These are native host APIs on `codex/event-algebra`.
...
`FiniteCarrier` captures the existing native `Space` and counts its **legal
logical atoms**, using `atom_count`. A diagram node count, completed code alias
count or sampled world count cannot supply this bound. Native presentations have
at most 62 coordinates, hence between 1 and 2^62 legal atoms. Constructing this
certificate does not enumerate them. `FiniteCarrier::atoms()` exposes the bound.

For N legal atoms, a strictly increasing inclusion chain can add an atom at
...
This certificate also covers [parameterized sources](event-parameter-sources.md).
An atom can contain infinitely many actual worlds; every Event is constant on
each atom and the sealed program cannot add guards during iteration. Zero-mass
possibilities still count. This does not certify an unbounded transcript or an
iteration that changes its source presentation. A finite environment readout
alone cannot establish the atom bound.
...
ranked/safe strategies. Query `Star(relation, product)` now retains an explicit
composition plan through [typed relation heads](event-queries.md).
[Scoped query binders](event-query-binders.md) now support nested least/greatest
solutions with fixed row inputs, and [belief memory](event-belief-memory.md)
now constructs reachable possibility histories. General host-program/strategy/
memory transport and integrated performance remain open gates.
