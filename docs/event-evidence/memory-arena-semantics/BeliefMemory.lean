...
/- Compiled code spaces retain exactly the indexed memory and action roster.
   The code injections below are native constructor premises, not codec proofs. -/
def codedRegion {M C : Type} (code : M → C) (B : Region M) : Region C :=
  fun c => ∃ m, code m = c ∧ B m

def codedStep {M A C D : Type} (cs : M → C) (ca : A → D)
    (R : Step M A) : Step C D :=
  fun s a t => ∃ m b n, cs m = s ∧ ca b = a ∧ cs n = t ∧ R m b n

theorem code_region_reflects_membership {M C : Type} (code : M → C)
    (injective : ∀ i j, code i = code j → i = j) (B : Region M) (m : M) :
    codedRegion code B (code m) ↔ B m := by
  constructor
  · rintro ⟨i, same, present⟩
    have eq := injective i m same
    subst i
    exact present
  · exact fun h => ⟨m, rfl, h⟩

theorem code_support_has_no_unused_states {M C : Type} (code : M → C) (c : C)
    (unused : ∀ m, code m ≠ c) : ¬ codedRegion code (fun _ => True) c := by
  rintro ⟨m, eq, _⟩
  exact unused m eq

theorem coded_transition_reflects_all_three_roles {M A C D : Type}
    (cs : M → C) (ca : A → D) (R : Step M A)
    (states : ∀ i j, cs i = cs j → i = j)
    (actions : ∀ i j, ca i = ca j → i = j) (m n : M) (a : A) :
    codedStep cs ca R (cs m) (ca a) (cs n) ↔ R m a n := by
  constructor
  · rintro ⟨i, b, j, hi, hb, hj, edge⟩
    have ei := states i m hi
    have eb := actions b a hb
    have ej := states j n hj
    subst i; subst b; subst j
    exact edge
  · exact fun h => ⟨m, a, n, rfl, rfl, rfl, h⟩

def controlled {M A : Type} (R : Step M A) (G : Region M) (m : M) :=
  ∃ a, (∃ n, R m a n) ∧ ∀ n, R m a n → G n

theorem compilation_preserves_enabled_controlled_predecessor {M A C D : Type}
    (cs : M → C) (ca : A → D) (R : Step M A) (G : Region M)
    (states : ∀ i j, cs i = cs j → i = j)
    (actions : ∀ i j, ca i = ca j → i = j) (m : M) :
    controlled (codedStep cs ca R) (codedRegion cs G) (cs m) ↔ controlled R G m := by
  constructor
  · rintro ⟨d, ⟨t, i, b, j, hi, hb, _, edge⟩, all⟩
    have ei := states i m hi
    subst i
    refine ⟨b, ⟨j, edge⟩, ?_⟩
    intro n h
    exact (code_region_reflects_membership cs states G n).mp
      (all (cs n) ⟨m, b, n, rfl, hb, rfl, h⟩)
  · rintro ⟨a, ⟨n, edge⟩, all⟩
    refine ⟨ca a, ⟨cs n, m, a, n, rfl, rfl, rfl, edge⟩, ?_⟩
    rintro t ⟨i, b, j, hi, hb, hj, h⟩
    have ei := states i m hi
    have eb := actions b a hb
    subst i; subst b
    exact ⟨j, hj, all j h⟩

theorem compiled_knowledge_preserves_hidden_world_meaning {M C S : Type}
    (code : M → C) (injective : ∀ i j, code i = code j → i = j)
    (belief : M → Region S) (G : Region S) (m : M) :
    codedRegion code (fun i => known (belief i) G) (code m) ↔ known (belief m) G :=
  code_region_reflects_membership code injective _ m
...
#print axioms BeliefMemory.code_region_reflects_membership
#print axioms BeliefMemory.code_support_has_no_unused_states
#print axioms BeliefMemory.coded_transition_reflects_all_three_roles
#print axioms BeliefMemory.compilation_preserves_enabled_controlled_predecessor
#print axioms BeliefMemory.compiled_knowledge_preserves_hidden_world_meaning
