...
The type deliberately has no derived `Eq`, `Hash` or canonical wire envelope.
A polynomial plus an isolating interval is a checked description, not a canonical
real-algebraic encoding. Minimal-polynomial/root-index normalization and stable
source descriptor integration remain separate work. Reconstructing a root through
its exposed polynomial and interval rechecks the claim; retained roots survive
release of the original polynomial and other root handles.
...
for the proposal's full semialgebraic source language. Boolean domain assembly,
multivariate feasibility, guarded rational functions, parameterized Space laws,
...
separators supply the open-cell witnesses. This does not yet provide canonical
real-algebraic bytes, multivariate elimination or parameterized Event sources.
