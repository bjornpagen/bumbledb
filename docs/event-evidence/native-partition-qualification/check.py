#!/usr/bin/env python3
"""Qualify finite Event maps and, with --relations, relations, programs and partitions.
...
def main():
    ...
    parser.add_argument("--relations", action="store_true", help="Include native relation and fixed-point result integration")
    ...
    if args.relations:
        checks[4][1].extend(["--test", "event_relations", "--test", "event_fixed_points",
                             "--test", "event_partitions"])
    result = {
        "scope": ("Native finite Event maps, relations, diagram inspection, information, sealed fixed-point programs and indexed partitions; source/query integration and full M0–M8 remain open; no performance qualification"
