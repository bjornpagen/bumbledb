...
def sources():
    ...
    files.add(ROOT / "scripts/event-number-wire.cjs")
    files.add(ROOT / "scripts/event-predicate-wire.cjs")
    files.add(ROOT / "scripts/event-guard-wire.cjs")
...
def main():
    ...
    if args.queries:
        checks[4][1].extend(["--test", "event_queries", "--test", "event_pack", "--test", "event_query_maps", "--test", "event_query_relations", "--test", "event_query_fixed_points", "--test", "event_expectations", "--test", "event_query_payoffs", "--test", "event_observation_stages", "--test", "event_observation_numbers", "--test", "event_query_numbers", "--test", "event_query_predicates", "--test", "event_predicate_guards"])
    ...
    result = {
        "sdk_scope": ("Owned Event field/carriers, worker-backed host algebra and supported Event/descriptor ingress, all seven BEDC structural constructors/descriptions/inspections, BEBM checked possibility-memory recipes with native admission/description/bounded graph inspection and owned indexed histories, BEBA named memory compilation with exact code/role inspection, hidden knowledge/possibility and native ranked reach/continuing-safety exports, BEAC general arena/strategy admission and owned replay with bounded inspection, enabled/good/controlled-predecessor operations, native reach/safety solving and checked policy restriction, explicit BEBA-to-BEAC export and shared worker arithmetic, exact BERA scalars, BEPL named polynomial authoring/arithmetic/substitution/evaluation/explicit Beta moments and owned term descriptions, BEPR exact univariate regions/inhabited domains, BEAR numerical roots and BESC v2 partial parameter functions with distinct ambient/defined domains, all set/sign masks and owned descriptions, live BEVT v3 source/guard construction and actual-world witnesses/cardinality, total signed family functions with exact designation/weighted maps/descent and owned function-valued host observations, checked finite/family function covers with local agreement and exact structural coverage, individual/common BESC v2 guard refinements with exact lift/descent, family channels and exact numerical FD descent, law-preserving parameter restrictions and replayed conditioning/likelihood/Jeffrey receipts with original-prior pullbacks and indexed unsupported regions, BESC finite functions, kernels and explicit replayed revisions, weighted images, strict law designation and owned fixed-law observations with shared admission arithmetic, Event/Test/map/relation and hygienic least/greatest query builders/imports and exact source-retaining final Probability heads, contextual full projections and physical-only key lookup, exact Event schema selections and owned closed Event rosters with pointwise keys, coverage, portable fingerprints and resolver-bound queries, worker-owned input/output on managed core and log paths, complete operand diagnostics, generated bindings, native dependencies, persistence/reopen, literals/parameters, Pack and ownership/cancellation, BEVT v3 shared-parameter carriers and logical atom counts; final grouped Expectation heads with integer/ratio columns and owned exact rational/finite/family imports, pointwise function covers, shared admission budgets, signed values/partial families and owned coverage rosters; typed observation staging/imports, same-kind identity bindings/joins and owned description replay/decoding with producer errors and parameter holes retained; typed numerical query builders/components/operators, BENO imports and owned partial results, canonical grouping before folds and exact domain retention; typed predicate queries/quantifiers, BENP imports and exact owned truth partitions, direct exact region membership with complete origins and v1/v2 replay; captured/row-bound source Guard plans with truth cases, canonical common bound-predicate and equal-domain peer-source rosters and explicit Event lift/exact descent, typed description replay and combined budgets; complete SDK and aggregate retained-memory policy remain open" if args.sdk else None),
        "query_scope": ("Complete-binding Event/Test heads, captured maps/faces/products, typed relation operators and Star, grouped Event Pack, exact Probability and Expectation heads with native source-owned observation interiors, identity joins and shared contraction budgets, nested monotone least/greatest query binders with sealed scopes and cumulative work limits, per-occurrence context faults and raw Node descriptors; completed numerical query values, native/macro/SDK arithmetic, domain/BENO imports and owned result replay; source-retaining predicate query heads, explicit quantifiers, BENP imports and combined expression limits; captured/row-bound guard plan heads, canonical common bound-predicate and equal-domain peer-source rosters, exact parameter-region predicates and explicit Event transport; complete host-program SDK and certified factoring remain open" if args.queries else None),
        "scope": ("Native finite Event maps, relations, diagram inspection, information, sealed fixed-point programs, indexed partitions, fully observed strategy witnesses and exact finite laws/BEVT v2, weighted images, conditional source closure, fixed-law revisions, signed expectations and BESC v1 checked source transport, univariate shared-parameter sources/BEVT v3, fibre-normalized guarded laws, owned conditional functions, whole-target-cell parameter maps/domain inclusions and common-domain products, checked common guard refinements/lifts/exact descent/readout transport, explicit law-preserving domain restriction, materialized conditional families with owned evidence-mass/defined-domain receipts and retained zero-posterior outcomes, signed finite-payoff and parameter-dependent conditional functions with original evidence and undefined endpoints, total family function arithmetic/sign refinement/weighted images/law designation, explicit family likelihood receipts and normalized posteriors, everywhere-normalized family channels with marginal preservation and numerical information FD descent, Jeffrey replacement families with owned indexed unsupported regions and original-prior translations, BESC v2 family function/channel/refinement/restriction transport and complete update replay with domain/receipt/context checks, and finite logical-atom fixed points; exact reachable belief memory with uniform action enabledness, symbolic possibility updates, ordinary ActionArena lowering and owned persisted database consumers; BEBM possibility-memory recipe transport with shared admission/product/normalization arithmetic, cumulative nested descriptor bounds and native replay before policy compilation; BEBA checked named arena compilation with cumulative recipe/identity limits and explicit new-owner kernel budgets, retained code partitions and SDK knowledge/strategy consumers; final source-owned Probability heads and grouped complete-payoff Expectation heads with shared execution/delivery arithmetic, source-pair grouping, scratch spill and owned evidence-relative rosters; explicit source-owned Beta integration of total polynomial-almost-everywhere raw observations with retained isolated exceptions; checked finite/family function masking and gluing of query-produced patches and imported native payoff heads with exact constants, owned covers and complete source diagnostics; source-retaining native host partial numerical arithmetic and exact true/false/undefined predicates with owned derivations; BENO v1 portable derivation identity and checked replay of full source/component/payoff leaves with explicit stacks and shared exact work; native observation staging with distinct query types, owned identity registries, producer error boundaries, spill inheritance and shared cross-stage exact arithmetic; native/macro/Node/SDK numerical query arithmetic with execution-owned derivation identities, shared exact budgets and tested spill/grouping; native source-retaining predicate composition/BENP replay with combined numerical budgets, explicit complete truth guard/refinement construction and persisted dependency/Free Join consumers; typed native/macro/Node/SDK predicate query heads and quantifiers with exact identity staging, combined numerical shape bounds, shared work, spill/rollback and worker admission; captured/row-bound source query Guard cases, common bound-predicate and equal-domain source rosters with canonical permutation/duplication invariance and retained origins, and lift/exact descent across native/macro/Node/SDK, full row-source admission with explicit bytes<32> identities, resolved-cell refusal and complete transport context faults with shared admission/execution limits; exact native/macro/Node/SDK parameter-region membership with original region/domain identity and BENP v2 replay preserving old v1 bytes; BEAC checked general arena/reach/safety transport with original product direction, retained selected policies, solver recomputation, finite/family source reconstruction and shared admission/normalization arithmetic; multivariate solving/source binding, general strategy query consumers, EventProgram transport, native query memory construction and M0–M8 remain open; no performance qualification"
