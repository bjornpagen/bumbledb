...
| M6 sources / inference | Fixed finite dynamics, shared univariate families/functions and family conditioning/likelihood implemented; full gate open | Exact arithmetic, univariate domains/guards, normalized fibre laws, owned conditional functions with endpoint holes, domain restriction, BEVT v3, family function arithmetic/weighted images/law designation and finite channels/revisions; multivariate/general function solving, family channels/Jeffrey updates/transport/prior integration, TypeSafe adapters and query observation heads remain |
...
The [current native semantic run](event-evidence/native-family-function-semantics/check.json) adds
**503 reports / 31 files**, using only permitted standard Lean axioms.
