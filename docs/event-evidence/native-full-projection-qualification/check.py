#!/usr/bin/env python3
"""Qualify native Event field/full dependencies; not the complete M3 or M0–M8 gate.
...
def native_sources():
    ...
    files.add(ROOT / "scripts/check-event-projection-wire.cjs")
    for path in (ROOT / "ts/src").rglob("*.ts"):
        files.add(path)
...
def main():
    ...
    checks = [
        ...
        ("integration", ["cargo", "test", "-p", "bumbledb", "--test", "event_dependencies",
                         "--test", "event_full", "--test", "event_storage", "--test", "keyed_get", "--test", "api",
                         "--test", "schema_macro"]),
    ...
    metadata = json.loads(subprocess.check_output(
        ["cargo", "metadata", "--manifest-path", "ts/crate/Cargo.toml", "--no-deps", "--format-version", "1"],
        cwd=ROOT, text=True))
    library = {"darwin": "libbumbledb_node.dylib", "linux": "libbumbledb_node.so", "win32": "bumbledb_node.dll"}[sys.platform]
    checks += [
        ("node-build", ["cargo", "build", "--manifest-path", "ts/crate/Cargo.toml"]),
        ("projection-wire", ["node", "scripts/check-event-projection-wire.cjs",
                             str(Path(metadata["target_directory"]) / "debug" / library)]),
        ("typescript", ["pnpm", "--dir", "ts", "exec", "tsc", "--noEmit"]),
    ]
    result = {
        "scope": "Native unmeasured Event field/full dependencies; complete M3, M4–M8 and performance remain open",
