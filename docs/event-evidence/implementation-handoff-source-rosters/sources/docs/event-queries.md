...
grouped Event `Pack(...)` and [Probability](event-query-probability.md) and
[Expectation](event-query-expectation.md) heads
in Rust macros and pure-data IR. [Completed observations](event-observation-stages.md)
can flow through later stages in Rust and the typed SDK. This is the
complete-binding structural slice of M5, with scoped predicate fixed-point
programs. [Numerical arithmetic](event-observation-numbers.md) and
[predicates and explicit guard heads](event-observation-predicates.md) now bring
exact comparisons back into ordinary Event fields. Certified branch factoring
remains a separate gate. These forms are not part of public v1.3.1.
...
refuses rather than publishing an approximation. [Scoped Least/Greatest binders](event-query-binders.md)
now compose through the Rust IR and SDK, including nested predicates. Their
iteration budget is shared with Star within each computed head/binding; the
Rust macro grammar adds no recursion syntax.
...
[Final Expectation heads](event-query-expectation.md) now aggregate exact integer
payoff rosters, with complete evidence-relative coverage and owned observations.
