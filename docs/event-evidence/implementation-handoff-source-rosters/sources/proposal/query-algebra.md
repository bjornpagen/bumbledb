...
modalities, finite Star and final source-owned Probability/Expectation heads are also
implemented. [Nested Least/Greatest query binders](../docs/event-query-binders.md)
now cross the Rust IR, Free Join and SDK boundaries. Completed observations also
stage through Rust and typed SDK queries. Exact numerical query arithmetic now
retains full source/evidence derivations. Exact predicate query values and explicit
quantifiers share that boundary. Captured and row-bound guard plans return ordinary Event cases, including common
truth refinement and exact transport. Equal-domain peer source rosters also
resolve shared parameter partitions. The complete Coup templates remain required.
...
The base Event grammar and final Probability/Expectation heads are implemented
on the branch, including observation staging and exact Number heads. The base
Event grammar is:
...
| --- | --- | --- |
| [`query!` grammar](../crates/bumbledb-query-macros/src/lib.rs) | Bindings, interiors, imports, aggregate heads | Parse Event/Test, Probability/Expectation and Number heads |
| [`ir.rs`](../crates/bumbledb/src/ir.rs), `FindTerm` / `Interior` | Pure-data outputs; topologically ordered stages | Event expressions, completed observations and exact Number programs |
...
final Probability/Expectation forms and scoped fixed-point IR binders compile too;
completed observations and exact numerical derivations flow through Rust and SDK
interiors. Number programs canonicalize their full derivations before grouping,
and share exact execution work with observation contraction. Exact predicate
heads and quantifiers now share this staging boundary. Captured and row-bound single-source guard construction, common predicate refinement
and exact transport share this boundary. CommonSources extends it to equal-domain
peer source presentations. Certified factoring remains open.
...
unions under canonical context minima. Exact final observations retain their
source/evidence and shared arithmetic budgets.
...
application, ITE, inclusive cardinality over a nonempty expression roster,
captured maps, typed relation views/modalities and scoped fixed-point binders. `RelationExpr` carries the
face/product imports and the relation operators in §9.
...
existing `Pack { over: VarId }` and observation staging are now implemented.
Exact Number and Predicate programs also run before relational grouping.
Explicit predicate quantifiers produce ordinary Bool. `FindTerm::Guard(GuardExpr)`
returns ordinary Events using an explicit captured or row-bound source and predicate roster.
...
finite closure, nested fixed-point binders and final measured query heads.
They do not qualify observation staging/arithmetic or the complete Coup program.
...
layer, including exact finite Star. [Query binders](../docs/event-query-binders.md)
now retain a full `EventScope` and lexically scoped predicate depths, distinct
from row variables. Every inner solution must be monotone in its own predicate;
its direction in outer predicates follows the body variance. A shared work budget
covers nested applications and Star within a head/binding. SDK callbacks close
hygienically to owned data; native iteration invokes no user callbacks.
`QueryBinders.lean` proves nested-solution covariance and relevant relation
variance laws. The existing finite-detection proof remains the termination
reference; neither module claims Rust or SDK refinement.
...
they can model repeated partially observed play. The native
[belief-memory constructor](../docs/event-belief-memory.md) now supplies that
possibility memory from explicit transition/observation rosters and lowers it
into the action arena; its query binder and posterior-law memory remain open.

## 11. Exact expectation and Free Join execution

`name: Expectation(value, when, given)` is now a [native final aggregate head](../docs/event-query-expectation.md). The three
...
coverage. Host [finite-function expectation](../docs/event-revisions.md) now
contracts signed values with a designated finite law and retains its evidence.
The native query collector now supplies integer and exact fraction payoffs,
group extraction, coverage admission and owned observation result slots.
`Expectation(Ratio(numerator, denominator), when, given)` reads ordinary integer
columns; equivalent presentations merge before coverage checks. Every divisor
participates, even on an empty region. One finalization arithmetic budget covers
normalization, admission and contraction after complete context checking.
Arbitrary rational literals/function payoff query slots and observation
staging/arithmetic remain open.

The [native function-cover algebra](../docs/event-function-covers.md) supplies
the corresponding contract for local function payoffs: functions must agree on
their actual overlap and their regions must cover the evidence. Global function
equality is unnecessary. Duplicate patches do not add utility; supplied zeros
remain separate from missing coverage. The host implementation and SDK retain
the original patches. Integrating this contract into native functional-payoff
query heads remains work, including owned result slots and complete diagnostics.
...
## Native final probability heads

The [native query observation contract](../docs/event-query-probability.md)
implements final `Probability(E,G)` in Rust/IR/Node/TypeScript, retaining source
pairs through Free Join and exact contraction before publication. Fixed and
univariate-family observations keep original evidence, numerator/mass and exact
partial domains. [Observation stages](../docs/event-observation-stages.md) now
carry completed probability and expectation values through Rust interiors and
typed SDK relation expressions. Their binding/result domain stays distinct from
the producer operation. Exact numerical arithmetic and predicate queries are
implemented, including captured guard/refinement heads and common truth rosters.
Integer/ratio and exact rational/finite/family expectation payoffs share the
owned-result boundary.
This slice does not change the full algebra or the M0–M8 completion criteria.


## Exact partial numerical foundation

[Native observation numbers](../docs/event-observation-numbers.md) now retain
completed probability/expectation leaves in an owned arithmetic derivation.
Fixed optional rationals and shared-parameter partial functions support exact
add/subtract/multiply/divide, negate/absolute value, natural powers, min/max and
sign comparisons. Undefined evidence and divisor zeros remain outside the
result's defined domain; zero multiplication and exponent zero do not erase
inherited holes. Fixed values lift to the retained parameter domain; combining
parameter functions requires explicit ambient-domain agreement/restriction.

Comparisons produce a complete true/false/undefined partition, with all sixteen
strict partial Boolean operations and explicit possible/always judgments.
Numerical equivalence includes definedness and does not equate observation
provenance. Multiplication of marginal numerical values never asserts stochastic
independence or substitutes for contracting an Event conjunction.

The query extension must retain this exact value/partition/derivation structure
through IR, staging, metadata, transport and SDK decoding. It must also connect
predicate regions to explicit guard/refinement instructions rather than
silently conditioning or changing an Event source. Host implementation alone
does not satisfy these query gates. The general multivariate solver, complete
retained-memory policy, certified factoring and full Coup consumers remain required.


## Portable numerical derivation boundary

The native numerical foundation now includes BENO v1 checked replay through
`ObservationNumberImport`. Exact derivation identity retains the selected
components, full source Events and indexed scalar/finite/family payoff rosters.
The transport has no claimed numerical answer to trust: admission rechecks the
sources and coverage, contracts observations and replays the arithmetic under
shared exact work. Canonical reencoding rejects presentation aliases. Whole
envelope bytes/items and per-source/expression limits bound admission, with
explicit traversal stacks and a hard depth ceiling. These are not an aggregate
retained-memory policy or a performance qualification.

Numerical instructions/heads, execution-local registries and typed macro/Node/SDK
authoring and decoding are now implemented, including exact predicate values and
explicit quantifiers and captured guard/refinement instructions. Eight numerical
Lean reference reports check the abstract value-stack
operand order, isolation and source-roster contract, not the Rust byte parser.
Every other M0–M8 gate remains required.


## Owned truth partitions and explicit guards

The [native predicate API](../docs/event-observation-predicates.md) now retains
sign, negation, all sixteen strict partial Boolean operators and explicit domain
restriction in an owned derivation. BENP replay shares numerical source admission
and arithmetic budgets. Three exact truth regions can be represented through
existing source guards or an explicitly named, law-preserving refinement.

A native consumer takes query-produced numbers into this host API, persists the
resulting true/false/undefined Event partition under ordinary pointwise keys and
full containments, then consumes it through Free Join after reopening. This does
not itself add captured refinement opcodes. Predicate-valued query heads/staging
and Node/SDK builders now consume the same retained algebra. Captured guard heads
and common predicate rosters return ordinary Event fields with exact transport.
Forty PredicateGuards Lean reports prove the reference world/truth transport,
row-bound source preservation and common-partition laws, not the Rust implementation.


## Retained predicate query values

`Predicate(number > threshold)` and all six exact comparisons now produce owned
truth partitions. `!`, `&`, `^`, `|` and `Bool4` compose strict partial predicates.
`PredicateTest(Possibly(p))`, `Always(p)` and `IsTotal(p)` explicitly produce Bool.
Full BENP derivation identity drives grouping and identity joins before folds;
truth equivalence does not identify distinct source/evidence receipts.

For a shared parameter, `Possibly(a & b)` requires one assignment satisfying
both. Separately quantifying each operand discards that relation. Fourteen
PredicateQueries Lean reports state the quantifier/definedness laws and exhibit
this counterexample. Native, macro, raw Node and SDK tests exercise identity
staging, mixed observations, explicit domain refusals, endpoint holes, spills,
rollback, worker replay and owner closure. Neither semantic proofs nor these
functional tests qualify production factoring or performance.


### Exact parameter-region predicates

The native query surface now accepts `Predicate(Region(domain, region))` with
`use number_domain` and `use parameter_region` imports. The SDK uses
`PredicateExpr.region(domain, region)`. This explicitly interprets an exact
BEPR set on an inhabited ambient domain, yielding true on their intersection,
false on the ambient difference and no undefined points. The complete authored
region remains in the derivation; identical truth on a smaller domain does not
identify different origins. No source law is required.

The result uses ordinary predicate staging, strict partial composition and
quantifiers, and enters Common guard plans alongside numerical comparisons.
BENP v2 replays Region leaves while existing trees keep v1 identity. Sources and
explicit presentation names can now bind from rows as described below.
General semialgebraic solving remains a separate obligation.


### Relational source binding

A full `event` column supplies the source and a `bytes<32>` column supplies an
explicit new presentation name. `Guard(Holds(p, Existing(source)))` interprets
truth in existing cells; `Guard(Holds(p, Refine(source,identity)))` constructs a
named parameter refinement. Both accept Common predicate rosters and explicit
lift/exact descent. SDK equivalents are `GuardPlan.bound(source)` and
`GuardPlan.boundRefine(identity,source)`. These are typed query selectors using
ordinary relations and staging, with no new stored type.

Source/name demands survive projection. Each participating binding undergoes
full canonical source admission, including the designated law, under shared
execution work. A proper or empty Event is not a full marker, regardless of
predicate truth. `Event(Full(e))` can explicitly extract the universe in an
earlier stage. Different rows keep their own source identity and law; binding
does not couple them, assert independence or identify worlds across sources.
A bound producer failure survives later filtering. `CommonSources` additionally
imports the guards of peer sources, as described below.


### Shared parameter partitions across sources

`CommonSources(Refine(source,identity), peer, other)` resolves the primary source
against the exact guards of the peer source roster. All peers bind ordinary full
Event values on exactly the same named domain. `Common(CommonSources(...),p,q)`
adds predicate distinctions. SDK `Guard.commonSources(plan, peers)` composes with
`Guard.common` in either order. Canonical ordering and duplication do not change
the result within resource bounds; every authored operand still validates.

Each query head constructs only its explicitly named primary presentation,
preserving its outcomes and law. Other heads may choose other primary sources
and names with the same roster. Their presentations then resolve the same
parameter partition. Maps and couplings remain explicit; equal guard partitions
neither identify sources nor determine their probabilities. Exact lift/descent
acts only between a primary source and its new presentation.

Existing plans refuse unresolved peer distinctions. Proper/empty/finite peers
and unequal or foreign domains refuse even behind constant truth and downstream
filters. Both rosters share node/byte/work limits and retain every source demand.
All these operations now run through the existing complete-binding Free Join
sink; no new row type or source-binding registry is introduced.
