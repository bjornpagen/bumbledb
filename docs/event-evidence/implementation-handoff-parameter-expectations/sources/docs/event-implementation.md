...
| M6 sources / inference | Fixed finite dynamics, shared univariate source families and family conditioning implemented; full gate open | Exact arithmetic, univariate domains/guards, normalized fibre laws, owned conditional functions with endpoint holes, explicit domain restriction, materialized conditional families, BEVT v3, finite weighted images/channels/revisions and receipts; multivariate/general function solving, general family dynamics/receipt transport/prior integration, TypeSafe adapters and query observation heads remain |
| M7 information / fixed points | Finite information, partition, expectation, fixed-point and fully observed strategy core implemented; other gates open | Readouts, indexed partitions, typed programs, closure, enabled actions, uniform permissions and ranked/safe policies; signed host expectations under fixed and univariate-family laws; general parameter-valued payoffs, belief-memory construction and complete query/source integration remain |
...
The [current native semantic run](event-evidence/native-parameter-expectation-semantics/check.json) adds
**486 reports / 30 files**, using only permitted standard Lean axioms.
