...
| M2 structural type / persistence | Native structural and fixed-law slices implemented; full gate open | Event values/macros, canonical rows, image words, parameters/sets/literals, Free Join equality, spill and owned results; raw Node wire uses checked BEVT bytes. Finite structural BEDC descriptors now reconstruct checked owners; BEVT v2 retains checked finite laws; complete SDK and parameterized source descriptors remain |
...
| M6 sources / inference | Fixed finite rational slice implemented; full gate open | Exact arithmetic, normalized density partitions, owned conditional observations and BEVT v2; conditional channels, shared parameters, solver capabilities, TypeSafe adapters and query observation heads remain |
...
The [current native semantic run](event-evidence/native-finite-law-semantics/check.json) adds
**286 reports / seventeen files**, using only permitted standard Lean axioms.
...
and [current handoff](event-evidence/implementation-handoff-finite-laws-final/check.json)
record this slice with 246 proposal plus 286 native reports. M6 remains open for
conditional channels, shared parameters, solver-backed constructors, TypeSafe
imports and owned query observations. Full M0–M8 qualification is not complete.
