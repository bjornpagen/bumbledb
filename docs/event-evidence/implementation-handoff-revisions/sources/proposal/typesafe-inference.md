...
**Revision 0.9; source-construction and application proposal.** No TypeSafe
...
## 8. Native conditional-source boundary

[Finite functions and conditional kernels](../docs/event-functions.md) now
implement normalized finite channels and closure under an explicit joint prior.
The normalization check includes zero-prior worlds. `factors_through` checks the
FD for a supplied actor-information-plus-outcome readout; choosing that readout
still requires the application visibility contract. Distinct authored outcome
extensions model fresh draws; repeated Event intersection reuses evidence.

Native tests and a persisted query consumer reproduce the §5 Tax probabilities
through one shared joint source. They use the exact supplied two-proposition deck
marginal and focused conditional forecasts, without provider calls. This is not
yet the complete Coup engine fixture or a TypeSafe adapter.
[Native fixed-law revisions](../docs/event-revisions.md) now distinguish
conditioning, likelihood reweighting and Jeffrey replacement, retain their
mathematical receipts and return explicit translation maps. Their exact
semantics have separate Lean evidence. Provider import intents, raw-response
receipts, actual observation identity, law-family constraints, continuous shared
parameters and observation query heads keep their gates open.
