...
and [contextual `true` projections](../docs/event-projections.md).
The [native query API](../docs/event-queries.md) now implements Event/Test, Pack
and typed map/relation heads; [fixed finite sources](../docs/event-sources.md)
implement rational density construction and host probability observations.
[Finite channels](../docs/event-functions.md) now construct conditional joint
sources with an explicit map back to their prior.
[Finite revisions and signed expectations](../docs/event-revisions.md) now retain
their mathematical inputs, impossible outcomes and translated posterior Events.
The higher-level source constructors and probability query heads below remain
proposed. See the [native ledger](../docs/event-implementation.md) before using
an example as code.
