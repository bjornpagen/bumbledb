...
### Native explicit univariate prior binding

[BetaSource](../docs/event-priors.md) now captures an explicit positive rational
Beta prior on a measured source with full named `[0,1]` domain. It contracts
raw joint/utility and evidence functions before division. The original source
and its conditional observation remain owned; binding does not replace actual
parameter worlds, fill zero-evidence holes or change structural Event equality.

A total integrand must agree with one polynomial outside a certified finite
set of parameter points. The original function, polynomial and exceptional
region survive in the result. Non-atomicity permits integration of these point
deviations; it does not identify their Events or functions. Undefined points,
nonpolynomial open sectors and truncated prior domains refuse explicitly.
BESC v2 tag 6 and the SDK replay source/shape admission under shared budgets.
This host fragment leaves multivariate source binding, parameter-changing maps,
general solving, query observation arithmetic and TypeSafe imports open.
