...
are described below; compiled memory/strategy transport and native query memory
construction remain required consumer work.
...
Eleven core tests cover 2,048 small-game/input cases against an independent dense
...
`BeliefMemory.lean` contains twenty-four reference reports, including exact agreement
...
application inputs; BEBM does not embed a compiled-arena certificate.
...
through the ordinary Event field; SDK compilation/strategy consumers remain open.
