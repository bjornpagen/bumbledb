...
failure clears the answer buffer. Owned answers retain their sources after query,
...
combined shape bounds and the current interior refusal. SDK tests cover owned
...
Probability outputs in interiors, query arithmetic over observations, expectation
aggregates, general parameter solving and certified branch factoring remain open.
Current interior use fails explicitly with `ObservationInterior`; the SDK refuses
to treat such output as a relation field. This is an implementation boundary to
remove, not a narrowing of the proposal. There is no new performance claim or
independent scalar-SQL oracle for observation heads.
