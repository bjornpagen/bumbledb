...
This is native host functionality. [BEDC v1](event-descriptors.md) now transports
finite maps, products, relation views, composition plans and complete squares.
Typed [query programs](event-queries.md) now retain faces and product plans through
Rust macros and raw Node descriptors. Complete SDK transport and parameterized
sources remain separate work.
[Diagram inspection](event-inspection.md) is now available on every Event region.
[Information readouts](event-information.md) also construct checked
indistinguishability relations and evidence-relative Event answers.
...
and workspace identities is explicit in the host API. Query `use product` imports
now retain the same descriptors for composition, residuals and finite Star.
...
identity. [BEDC v1](event-descriptors.md) separately transports role views and
product/map descriptors. Persisting a region alone cannot reconstruct those roles;
callers retain/import the checked descriptor before constructing a relation view.
...
after the database closes using an independently imported BEDC descriptor.
...
include source/continuous-parameter integration, SDK descriptor transport,
general bound fixed-point query programs and integrated performance. Complete-binding
relation queries now have [their own tests and proof boundary](event-queries.md). Diagram inspection
