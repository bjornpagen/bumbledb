...
| M5 query / Pack / Free Join | Complete-binding Event/Test, relation/readout programs, SDK builders/diagnostics, Pack and final Probability/Expectation heads implemented; full gate open | Boolean/ITE/cardinality, structural tests, six readout operators, typed relation operators and Star with captured faces/products, grouped union, scratch claims, staging and owned occurrence fault sets; general fixed-point binders and certified factoring remain |
...
The [current native semantic run](event-evidence/native-query-expectation-semantics/check.json) adds
**567 reports / 34 files**, using only permitted standard Lean axioms.
