...
modalities, finite Star and final source-owned Probability heads are also implemented. Observation staging, Expectation and
...
probability, general fixed-point binder and expectation forms retain their separate gates.
...
existing `Pack { over: VarId }` is now implemented. observation staging, `Expectation`
