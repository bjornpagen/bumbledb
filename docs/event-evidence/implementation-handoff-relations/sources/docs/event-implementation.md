...
| M4 faces / maps / relations | Finite structural relation core implemented; integration gates remain | Full face/fibre products, complete square certificates, membership-FD roles, converse, composition, modalities, both residuals and graph/readout conversion. Diagram inspection, descriptor transport and parameter/source/query integration remain |
...
The [current native semantic run](event-evidence/relation-semantics/check.json) adds
**85 reports / five files**, using only permitted standard Lean axioms.
