...
now compiles Event/Test Boolean, ITE and fixed-roster cardinality heads and staged
interiors. Event Pack, Probability/Expectation and relational/face-import query
forms remain proposed; the complete Coup templates are not yet executable.
...
Event Pack storage and exact probability results still need implementation.
...
pure-data trees; shape admission precedes normalization. `Probability` and the
Event domain of existing `Pack { over: VarId }` remain future additions.
...
fault-set equality; native same-space Event/Test heads now own canonical, sorted fault descriptors
...
to Pack, imported faces and factored schedules remains required.
...
bindings for same-space constructive Event/Test trees and stable context faults.
The richer typed relational query program and captured face imports remain M5.
This host layer remains their algebraic building block.
