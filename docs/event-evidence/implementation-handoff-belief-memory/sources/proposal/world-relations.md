...
The equations here suppress representation plans. Native query syntax retains
them explicitly: `use product path = &composition`, then `Compose(R,Q,path)`,
`LeftResidual(R,V,path)`, `RightResidual(V,Q,path)` or `Star(R,path)`. The imported
BEDC Composition descriptor names all three pair products and the shared
workspace. This fixes output identity without changing any algebraic law. See
the [implemented query contract](../docs/event-queries.md).
...
The [native action API](../docs/event-actions.md) now implements this one-step
contract. `ActionArena::good` supplies enabled state/action pairs; the prepared
relational plan's `uniform_permissions` computes the inhabited residual over
all supplied hidden alternatives. It retains the distinction between each
hidden state having an action and one shared action working throughout the case.
...
The [native finite implementation](../docs/event-fixed-points.md) now provides
Star and all three predicate helpers. Star is itself an inspectable program
`X ↦ Id | R;X` over the prepared result pair context, with checked reindexing and
one shared middle witness. Predicate helpers validate both endpoint spaces and
environment readouts. Iteration uses symbolic Event operations and original
legal-support counts, with explicit budget/cancellation refusal.
...
The roster is a proof object, not a runtime enumeration. Exact detection needs
at most one further application. [Programs](../crates/bumbledb-event/semantics/Programs.lean)
proves the variance gate; [Closure](../crates/bumbledb-event/semantics/Closure.lean)
identifies the extremal programs with paths, reachability, invariant safety and
well-founded forcing. Rust kernels and program evaluation remain tested refinements.
...
The native `winning_reach` now retains first-entry rank cells and all choices
whose every successor belongs to an earlier cell. `winning_safe` retains enabled
choices inside the greatest invariant. Their `with_policy` methods admit only
covered subrelations of those choices. The
[Actions proofs](../crates/bumbledb-event/semantics/Actions.lean) establish a
rank bound for every retained policy choice/outcome and continuing invariant
safety. This is a concrete fully observed finite contract, not a claim that
remaining inside a winning set alone guarantees eventual progress.
...
requires updating the information state (including memory). The native
[belief-memory construction](../docs/event-belief-memory.md) now explores exact
reachable Event subsets, with up to exponentially many memory states, and lowers
them into the existing action arena. Actions must be enabled at every possible
hidden state; each update intersects the relational post-image with the newly
observed cell. Equal screens can retain different memories. This is possibility
memory, not a posterior distribution or a solution to all hidden-goal objectives.
