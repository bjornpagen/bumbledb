...
def main():
    ...
    result = {
        "sdk_scope": ("Owned Event field/carriers, worker-backed host algebra and supported Event/descriptor ingress, all seven BEDC structural constructors/descriptions/inspections with owned projections, Event/Test/map/relation query builders and description imports, complete structured Event operand diagnostics, stored-field generated bindings, native dependency admission, persistence/reopen, literals/parameters, Pack, ownership/cancellation and related SDK regressions; BESC source construction, complete SDK and aggregate retained-memory policy remain open" if args.sdk else None),
