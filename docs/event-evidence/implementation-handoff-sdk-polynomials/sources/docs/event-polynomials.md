...
[Exact real roots](event-algebraic-roots.md) now provide the univariate
isolation/sign capability required for irrational parameter witnesses. They
reconstruct isolation claims and compare numeric values across presentations;
they do not yet admit a complete source domain or canonical real-algebraic bytes.

The [TypeScript expression API](event-sdk-polynomials.md) now exposes this same
native algebra, including simultaneous substitution and explicit Beta moments,
through owned BEPL carriers and cancellable workers. Its persisted consumer
checks shared-versus-separate prior correlation after runtime/database release.
The [univariate source layer](event-parameter-sources.md) supplies checked domains,
guards and normalized family laws in Rust; SDK domain/family authoring remains
separate work. Neither the arithmetic SDK nor prior specialization replaces it.
