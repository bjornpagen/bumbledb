...
/-- Peer sources contribute their parameter distinctions, not outcome worlds. -/
theorem common_sources_exact {T : Type} (sources : List (List (T → Option Bool)))
    (a b : T) : AgreeOn sources.flatten a b ↔ ∀ ps ∈ sources, AgreeOn ps a b := by
  constructor
  · intro same ps member p hp
    exact same p (List.mem_flatten.mpr ⟨ps, member, hp⟩)
  · intro same p member
    obtain ⟨ps, hps, hp⟩ := List.mem_flatten.mp member
    exact same ps hps p hp

theorem reordered_sources_change_no_cells {T : Type}
    (ps qs : List (List (T → Option Bool))) (permutation : ps.Perm qs) (a b : T) :
    AgreeOn ps.flatten a b ↔ AgreeOn qs.flatten a b := by
  rw [common_sources_exact, common_sources_exact]
  constructor
  · intro same source hs; exact same source (permutation.mem_iff.mpr hs)
  · intro same source hs; exact same source (permutation.mem_iff.mp hs)

theorem repeated_source_changes_no_cells {T : Type} (source : List (T → Option Bool))
    (rest : List (List (T → Option Bool))) (a b : T) :
    AgreeOn (source :: source :: rest).flatten a b ↔
      AgreeOn (source :: rest).flatten a b := by
  simp only [common_sources_exact, List.mem_cons, forall_eq_or_imp]
  exact and_self_left

theorem resolved_peer_guard_adds_no_cells {T O : Type} (view : T → O)
    (guard : T → Option Bool) (resolved : Resolves view guard) (a b : T) :
    (view a, guard a) = (view b, guard b) ↔ view a = view b := by
  constructor
  · exact congrArg Prod.fst
  · intro same; exact Prod.ext same (resolved a b same)

structure PresentedLaw where
  guards : Unit → List Bool
  density : Unit → Bool → Rat

/-- Even positive normalized laws on the same outcomes can disagree while
their entire parameter presentation agrees. Common guards cannot merge laws. -/
theorem common_partition_does_not_identify_law : ∃ a b : PresentedLaw,
    (∀ t, a.guards t = b.guards t) ∧
    (∀ t, a.density t false + a.density t true = 1 ∧
      b.density t false + b.density t true = 1) ∧
    (∀ t o, a.density t o > 0 ∧ b.density t o > 0) ∧
    a.density () true ≠ b.density () true := by
  refine ⟨⟨fun _ => [true], fun _ o => if o then 1/4 else 3/4⟩,
    ⟨fun _ => [true], fun _ o => if o then 3/4 else 1/4⟩, ?_, ?_, ?_, ?_⟩
  · intro t; rfl
  · intro t; cases t; decide +kernel
  · intro t o; cases t; cases o <;> decide +kernel
  · decide +kernel
...
#print axioms PredicateGuards.common_sources_exact
#print axioms PredicateGuards.reordered_sources_change_no_cells
#print axioms PredicateGuards.repeated_source_changes_no_cells
#print axioms PredicateGuards.resolved_peer_guard_adds_no_cells
#print axioms PredicateGuards.common_partition_does_not_identify_law
end PredicateGuards
