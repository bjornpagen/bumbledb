...
observation staging/arithmetic and certified factoring retain their separate gates.
...
partial domains. Probability output in interiors still refuses explicitly;
observation staging/arithmetic remains a proposal gate. Final integer-payoff
Expectation aggregates now follow the same owned-result boundary.
