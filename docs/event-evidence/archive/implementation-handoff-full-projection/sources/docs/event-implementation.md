...
The [current native semantic run](event-evidence/full-projection-semantics/check.json) adds
**37 reports / three files**, using only permitted standard Lean axioms.
...
malformed constants. The [updated handoff](event-evidence/implementation-handoff-full-projection/check.json)
