...
objects and their canonical bytes for [readout query heads](event-queries.md).
Query face/program imports and complete SDK transport remain separate gates.
...
This envelope is separate from BEVT v1, which continues to encode Event regions
only. BEDC v1 is finite and unmeasured. It does not encode source laws, solver
certificates, Event programs, strategies or new database field types.
...
Readout query imports now accept `Map` and `Surjective` descriptors. Query face
and relation imports, SDK descriptors, measured-source descriptors and integrated
performance remain separate acceptance work. This transport does not change
