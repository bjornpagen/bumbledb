...
| M2 structural type / persistence | Native unmeasured slice implemented; full gate open | Event values/macros, canonical rows, image words, parameters/sets/literals, Free Join equality, spill and owned results; raw Node wire uses checked BEVT bytes. Finite structural BEDC descriptors now reconstruct checked owners; complete SDK and future source descriptors remain |
...
| M4 faces / maps / relations | Finite structural relation core implemented; integration gates remain | Full face/fibre products, complete square certificates, membership-FD roles, converse, composition, modalities, both residuals, graph/readout conversion and safe diagram inspection/reconstruction. Finite structural BEDC transport is implemented; SDK and parameter/source/query integration remain |
| M5 query / Pack / Free Join | Complete-binding Event/Test, readout imports and Pack implemented; full gate open | Boolean/ITE/cardinality heads, structural tests, six readout operators with captured contexts, grouped union, scratch claims, staging and per-occurrence fault sets; face imports, relational programs, SDK diagnostics and certified factoring remain |
...
BEDC now carries checked finite map descriptors. Face-import query heads remain
future work.

## Lean evidence and remaining correspondence

The proposal suite was rerun: **246 reports / 30 files, 160 axiom-free**.
The [current native semantic run](event-evidence/native-descriptor-semantics/check.json) adds
**237 reports / fourteen files**, using only permitted standard Lean axioms.
