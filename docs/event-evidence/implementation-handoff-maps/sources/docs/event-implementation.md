...
| M4 faces / maps / relations | Checked finite map foundation implemented; full gate open | Arbitrary Event readouts, pullback, image, universal/nonvacuous image and distinct surjectivity admission. Full fibre products, role certificates, world relations/residuals and graph views remain |
...
The [current native semantic run](event-evidence/map-semantics/check.json) adds
**63 reports / four files**, using only permitted standard Lean axioms.
