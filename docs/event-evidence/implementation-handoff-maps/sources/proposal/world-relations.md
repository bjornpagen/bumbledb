...
**Revision 0.8; normative direction.** Event's unary Boolean operations are one
...
Native implementation has reached the
[checked finite map foundation](../docs/event-maps.md): arbitrary Event readouts,
pullback, image and separate surjectivity admission. The relation/product
constructors and complete-fibre certificates specified here remain M4 work.
This status does not change their mathematical contract.
