...
The native semantic modules add **334 checked reports across nineteen files** in
[the current native run](../../docs/event-evidence/native-revision-semantics/check.json):
...
[QueryMaps](../../crates/bumbledb-event/semantics/QueryMaps.lean) and
[QueryRelations](../../crates/bumbledb-event/semantics/QueryRelations.lean), and
[FiniteDensity](../../crates/bumbledb-event/semantics/FiniteDensity.lean), and
[WeightedMaps](../../crates/bumbledb-event/semantics/WeightedMaps.lean), and
[Revisions](../../crates/bumbledb-event/semantics/Revisions.lean). These are
...
| Weighted image and normalized conditional source closure | WeightedMaps: `weighted_pairing`, `total_mass_preserved`, `fibre_factor`, `channel_preserves_prior`, `closed_channel_normalized`, last-use sum law and counterexamples (18 reports; four axiom-free) | Nonnegative common-denominator reference; signed arithmetic, raw partition operations, source dependency extraction, exactly-once scheduling and memoization remain tested Rust. Continuous parameters and arbitrary source priors are not inferred |
| Fixed-law revisions and signed finite expectations | Revisions: nonnegative normalized Pearl/Jeffrey laws, likelihood scaling/composition, target masses, preserved within-cell ratios, idempotence, signed contraction and explicit counterexamples (30 exact rational reports) | Native partition admission must denote the total bucket function; Rust arithmetic, contraction, identity translations, receipt ownership and query aggregate integration remain tested implementation obligations |
| Native fixed finite density/count contraction and measured contexts | FiniteDensity: `contraction_exact`, `merge_equal_density`, `skipped_outcome_multiplicity`, conditional/evidence laws and correlation/context counterexamples (20 reports; six axiom-free) | Rust legal counts, common-denominator correspondence, arbitrary-precision rational arithmetic, canonical BERA/BELW/BEVT v2 parsing and source ownership remain tested implementation obligations |
...
| Typed relation query programs across equivalent presentations | QueryRelations: endpoint-indexed whole-program transport, identity/tests, converse, composition, residuals, finite paths and all three modalities | Instantiate legal endpoint bijections within one fixed environment; Rust must check actual environment maps, preserve orientation and authored products/workspaces, and refine finite iteration; no native checker or resource proof is claimed |
...
attainment or source feasibility. Fixed rational Jeffrey/Pearl updates and signed
expectation contraction now have separate native reference proofs in Revisions.
