...
stage through Rust and typed SDK queries. Numerical query arithmetic and the
...
on the branch, including observation staging. Numerical query arithmetic remains open:
...
Numerical query arithmetic remains a future addition.
...
the producer operation. Numerical observation query arithmetic remains a proposal
gate; integer/ratio and exact rational/finite/family expectation payoffs share the
...
This makes numerical expressions portable as native host data. The query layer
still needs numerical instructions/heads and execution-local registries, typed
macro/Node/SDK authoring and decoding, and exact predicate/guard/refinement
consumers. Eight new Lean reference reports check the abstract value-stack
