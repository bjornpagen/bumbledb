...
| M5 query / Pack / Free Join | Complete-binding Event/Test, relation/readout programs, SDK builders/diagnostics, Pack and staged Probability/Expectation values implemented; full gate open | Boolean/ITE/cardinality, structural tests, six readout operators, typed relation operators and Star with captured faces/products, grouped union, scratch claims, staging and owned occurrence fault sets; integer/ratio and exact rational/finite/family payoff heads; typed SDK observation imports, identity joins and owned forwarding; nested least/greatest binders retain sealed contexts and shared work budgets; certified factoring and numerical observation arithmetic remain |
| M6 sources / inference | Fixed finite dynamics, shared univariate families/functions and family conditioning/likelihood/Jeffrey and channels plus explicit univariate Beta binding implemented; full gate open | Exact arithmetic, univariate domains/guards, normalized fibre laws, owned conditional functions with endpoint holes, domain restriction, BEVT v3, family function arithmetic/weighted images/law designation and finite channels/revisions; multivariate/general function solving, multivariate source binding, TypeSafe adapters, numerical observation arithmetic and complete retained-memory policy remain |
| M7 information / fixed points | Finite information, partition, expectation, fixed-point, strategy and reachable belief-memory core implemented; other gates open | Readouts, indexed partitions, typed programs, closure, enabled actions, uniform permissions and ranked/safe policies; exact reachable possibility memory lowers to ActionArena; signed host expectations with rational and guarded parameter-dependent payoffs plus checked local function covers; memory descriptor/SDK/query construction, general semialgebraic functions and complete query/source integration remain |
...
[handoff](event-evidence/implementation-handoff-sdk-observation-stages/check.json)
...
`event-evidence/sdk-observation-stages-portable`.
