...
Revision 0.9 retains the full relational algebra and selects an initial general
backend from the completed representation research. These are
...
| R2 | One exact canonical resident manager per aligned space; start with completed symbolic functions and essential-coordinate tables | Initial cutoff nine is internal; dense and packed512 remain controls; no universal winner or automatic backend switch is claimed |
...
| A5 | Star and monotone fixed points require a stable finite state quotient | Lean establishes a legal-roster iteration bound; shared environments need a uniform bound and fibre-preserving operators; native refinement remains required |
...
| A10 | New Event validation returns a deterministic set of participating operand faults | Stable logical descriptors order presentation; physical traversal and arena IDs cannot select the error; resource refusal is explicit |
| R13 | Canonical per-fact codec and schema compatibility must pass M2 before persistent Event publication | Batch transport is evidence, not an already-qualified persistent format |
