...
This fixture exercises host gluing of query-produced patches. Native
[`Expectation` heads](event-query-expectation.md) now also admit imported finite
and family functions, promoting column or exact-constant payoffs when needed.
They retain the checked cover and contract its assembled function under the
original law. Observation staging/arithmetic remains implementation work.
