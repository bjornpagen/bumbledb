...
Work is paused at the user’s request; [the checkpoint](event-checkpoint.md) records
the unfinished validation and exact resumption point.
...
The failed run remains evidence; the latest qualification contains this fix but
was stopped at the user’s checkpoint request before all checks completed.
