...
Research is closed at the user's request. This review records its evidence.
Revision 0.8 now chooses the completed-function/essential-table direction as the
initial general implementation backend, with dense and packed controls; it does
not claim universal dominance. The [current contract](proposal.md),
[proof matrix](semantics/README.md) and [implementation plan](implementation-plan.md)
define the implementation baseline. The [handoff](RESEARCH-HANDOFF.md) records
the earlier research evidence boundary.
