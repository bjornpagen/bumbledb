...
persistent bytes compare source identity, original support, designated law if
present, and supported membership. The native query registry performs that alignment for its bindings;
...
as a fixed point. The current certificate counts original finite support even under a fixed law;
...
only admission returns executable owners. Measured endpoints now use BEVT v2,
and captured map/face/product query imports use these admitted descriptors.
Function/kernel/revision-receipt transport and parameterized source formats
retain separate implementation gates.
...
proves these reference boundaries. Captured face/product imports and typed
relation programs are integrated; bound Least/Greatest query programs and
certified factoring retain their separate gates.
...
Fixed rational finite laws are the implemented initial exact arithmetic fragment;
the [native source contract](../docs/event-sources.md) specifies density
construction, owned observations and measured persistence. The
[finite arithmetic/channel layer](../docs/event-functions.md) adds weighted
images and normalized conditional source extensions; old parent marginals and
source identities are retained through an explicit readout.
[Finite revisions and signed expectations](../docs/event-revisions.md) distinguish
conditioning, likelihood factors and posterior replacement, retain mathematical
receipts, and translate old Events without changing structural possibility. Shared
...
[BEVT codecs](../docs/event-value-format.md) select fixed semantic-coordinate
order for structural Events. Version 2 adds a canonical fixed finite law,
including zero-mass legal worlds, to version 1's unchanged structural format.
Both are integrated into native rows; neither encodes continuous parameters.
