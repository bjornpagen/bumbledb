...
These SDK results are fixed rational observations. The native shared-parameter
observation retains its explicit function/domain; SDK authoring and result
transport remain open. Scalar observation APIs refuse parameter-family laws.
Native [family conditioning](event-parameter-conditioning.md) now owns exact
posterior families and receipts; its SDK authoring and receipt transport remain
open. Existing carriers can retain the posterior Event and translation map.
Native signed [family expectations](event-parameter-expectations.md) also retain
their complete conditional function; this SDK does not yet transport that result.
The native [family function algebra](event-family-functions.md) now also supports
parameter-dependent payoffs, weighted images and likelihood receipts. Their SDK
construction and function/receipt/result transport remain open.
...
The complete SDK still needs general fixed-point programs, family channels and
revisions/domain restrictions/common refinements, and native query observation/expectation slots.
