...
| M3 dependencies / admission | Native field and contextual full projections implemented; full gate open | Pointwise overlap, union coverage, full partitions, final-state repair/deletion, empty-key lookup refusal and conservative planner witnesses work. Exact Event selections are implemented; capacity positions and Event-valued closed rosters remain guarded |
...
Remaining log schema ingress and complete query/source APIs remain open.
...
Event capacity projections and Event-valued closed rosters remain guarded;
complete SDK/log authoring still awaits the later consumer milestone.
...
Event literals; Event capacity positions, Event-valued closed rosters and the
remaining M0–M8 gates remain open.
...
preserve selections and fingerprints. Historical synchronous and managed log
schema converters still need migration; the full SDK gate remains open.

The [selection qualification](event-evidence/native-selection-qualification/check.json)
...
[selection handoff](event-evidence/implementation-handoff-selections/check.json)
...
faithfulness; they are not a Rust implementation proof. M3 still has Event-valued
closed rosters and Event capacity projections to complete; M0–M8 remains active.
