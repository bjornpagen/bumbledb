...
and law descriptor. The native unmeasured slice now uses
[BEVT v1](../docs/event-value-format.md), a fixed semantic-order graph codec
inside canonical rows. The full M2 gate must establish:
...
formally verify its byte parser or encoder. Measured sources and continuous
parameter descriptors still require their own format and compatibility evidence.
