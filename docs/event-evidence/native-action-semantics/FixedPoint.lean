...
/-! Retained first-entry layers supply exact ranks for controlled strategies.
    The native loop retains changing differences only; the following reference
    also allows the empty differences after stabilization. -/

theorem ascending_between {W : Type} (F : Region W → Region W) (mono : monotone F)
    (i j : Nat) (ordered : i ≤ j) : included (iterate F i) (iterate F j) := by
  induction j with
  | zero =>
  ...
  | succ j ih =>
    by_cases same : i = j + 1
    · subst i; exact fun _ present => present
    · exact fun w present => ascending F mono j w (ih (by omega) w present)

def firstEntry {W : Type} (F : Region W → Region W) (n : Nat) (w : W) :=
  iterate F (n + 1) w = true ∧ iterate F n w = false

theorem first_entry_unique {W : Type} (F : Region W → Region W) (mono : monotone F)
    (i j : Nat) (w : W) (left : firstEntry F i w) (right : firstEntry F j w) : i = j := by
  have notLeft : ¬ i < j := by
    intro below
    have impossible := ascending_between F mono (i + 1) j (by omega) w left.1
    rw [right.2] at impossible
    cases impossible
  have notRight : ¬ j < i := by
    intro below
    have impossible := ascending_between F mono (j + 1) i (by omega) w right.1
    rw [left.2] at impossible
    cases impossible
  omega

theorem first_entries_cover_iterate {W : Type} (F : Region W → Region W) (mono : monotone F)
    (n : Nat) (w : W) : iterate F n w = true ↔ ∃ k, k < n ∧ firstEntry F k w := by
  induction n with
  | zero => simp [iterate, empty]
  | succ n ih =>
    constructor
    · intro present
      cases old : iterate F n w with
      | false => exact ⟨n, by omega, present, old⟩
      | true =>
        obtain ⟨k, before, entry⟩ := ih.mp old
        exact ⟨k, by omega, entry⟩
    · rintro ⟨k, before, entry⟩
      exact ascending_between F mono (k + 1) (n + 1) (by omega) w entry.1
...
#print axioms FixedPoint.ascending_between
#print axioms FixedPoint.first_entry_unique
#print axioms FixedPoint.first_entries_cover_iterate
