...
conditional functions and original evidence. [`FamilyFunction`](event-family-functions.md)
now adds parameter-dependent payoff values, arithmetic, weighted images and
likelihood revision. Owned observation query/SDK results remain separate gates.
...
parameter-family channels remain open. `FamilyFunction` now supplies weighted
images and explicit law pushforward designation, with zeros on unreachable domains.
...
and Jeffrey revisions over families, observation query heads, TypeSafe import intents,
