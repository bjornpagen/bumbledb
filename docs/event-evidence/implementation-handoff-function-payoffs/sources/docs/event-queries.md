...
grouped Event `Pack(...)` and final [Probability](event-query-probability.md) and
[Expectation](event-query-expectation.md) heads
in Rust macros and pure-data IR. This is the
complete-binding structural slice of M5. Bound predicate fixed-point programs,
observation staging/arithmetic and certified branch factoring remain
