...
**Revision 0.8; proposed extension, before implementation.** This specifies the query
...
show the proposed additions together. None of the new forms compile today.
...
The proposed grammar addition is local:
...
| --- | --- | --- |
| [`query!` grammar](../crates/bumbledb-query-macros/src/lib.rs) | Bindings, interiors, imports, aggregate heads | Parse event and probability heads |
...
structural Rust IR today; the macro does not yet parse these proposed forms.
Numeric/interval slot layouts, `ScalarExpr`, pack storage, and output ownership
do not already support this event carrier or exact probability result. These
are real implementation requirements.

A possible IR shape, with names still provisional:

```rust
enum EventExpr {
    Var(VarId), Empty(VarId), Full(VarId),
    Not(Box<EventExpr>),
    And(Box<EventExpr>, Box<EventExpr>),
    Or(Box<EventExpr>, Box<EventExpr>),
    Xor(Box<EventExpr>, Box<EventExpr>),
}

// Proposed additions to FindTerm, not definitions present in the engine:
Event(EventExpr)
Probability { event: EventExpr, given: EventExpr }
// Existing Pack { over: VarId }, extended to the event domain.
```
...
fault-set equality; native descriptors and existing error integration still
require implementation. Resource refusal cannot masquerade as a complete set.
...
This is a finite semantic oracle, not execution of `query!` or Free Join. Native
typing, equality, persistence, admission, and machine-code lowering remain
implementation obligations. Existing engine tests would not validate syntax
that has deliberately not yet been implemented.
...
The proposed macro can import owned face descriptors, without adding another
...
boundary. This **new macro import form** lowers to a retained descriptor binding.
...
The proposed typed expression grammar adds:
...
relationexpr := Relation(eventexpr, faces)
              ...
              | Converse(relationexpr) | Compose(relationexpr,relationexpr)
              | LeftResidual(relationexpr,relationexpr)
              | RightResidual(relationexpr,relationexpr) | Star(relationexpr)
...
For example, `Region(Compose(Relation(r, first), Relation(q, second)))`
...
The typed pure-data IR should contain EventProgram, RelationProgram, EventTest,
and descriptor-import tables. RelationProgram carries a signature plus child
programs; a compiled composition retains a RelProductPlan, not executable user
code. Binding slots carry EventKeys and operation owners. Library authors can
...
`Star` validates an endorelation and its finite stable-state certificate. It
...
functions, so simplification cannot erase their participation. Multi-binding
query programs, constructive heads and complete stable fault sets remain M5;
this host layer is their algebraic building block, not a claim of query integration.
