...
| M7 information / fixed points | Finite information and fixed-point core implemented; other gates open | Readout saturation, evidence cases, FD factors, indistinguishability, typed programs, least/greatest fixed points and Star; observables, expectation, permissions/strategies and query/source integration remain |
...
The [current native semantic run](event-evidence/native-fixed-point-semantics/check.json) adds
**150 reports / nine files**, using only permitted standard Lean axioms.
...
[current handoff](event-evidence/implementation-handoff-fixed-points/check.json)
audits current documents against 246 proposal plus 150 native reports. Finite
observables, measured expectation, action availability/uniform strategies,
