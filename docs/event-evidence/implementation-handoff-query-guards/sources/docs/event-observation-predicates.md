...
field type or serialized value format is added. A refinement must use the same
predicate and plan for its cases and transport operations. Independent predicates
may require different guard presentations; reusing an identity does not silently
construct their common refinement.
...
query-authored common refinements and direct parameter-region consumers remain
...
Twenty-four `PredicateGuards.lean` reports establish exact truth coding,
...
shared arithmetic budget and recover after cancellation. Three macro fixtures
...
Parameter-region query consumers, dynamic/common query refinement,
