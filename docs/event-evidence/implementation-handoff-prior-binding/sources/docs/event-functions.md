...
[Shared-parameter expectations](event-parameter-expectations.md) now contract
the same total finite payoff against a designated family law. The owned result
retains an exact signed function of the parameter, evidence mass and defined
domain. It does not integrate over parameters or add parameter-dependent payoff
values, native query observation heads or SDK result transport by itself.
The [family function algebra](event-family-functions.md) now supplies
parameter-dependent values, signed arithmetic, weighted images and explicit
family likelihood updates. Family channels and consumer integration remain open.
