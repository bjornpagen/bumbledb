...
nonrecursive derived stages. Revision 0.9 retains a
[canonical completed-function carrier](../representation.md) and its
[candidate ARM64 kernels](../kernels.md). The expression forms, packing, and
exact measurements have different implementation status: Boolean Event/Test
heads and Event Pack work on the implementation branch; probability heads remain
pending. The complete Coup templates still require that integration. The engine need
