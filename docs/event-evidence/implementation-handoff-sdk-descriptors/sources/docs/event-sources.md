...
and checked conditional sources. Parameter families, solver capabilities, source
revisions, TypeSafe adapters and query observation heads remain open.
...
bit. A parameter guard is not a stochastic coordinate; parameterized source
construction needs its separate interpretation and cannot use this constructor
to turn feasible guard valuations into equiprobable outcomes.
...
that every future observation has a scalar value. Probability observations have
no database-field or Node/TypeScript observation wire format yet.
...
separate contracts and retained mathematical receipts. Shared polynomial parameters,
guarded rational functions, certified semialgebraic solving, model import
intents and source-aware observation query results still keep M6 open.
