...
| M6 sources / inference | Fixed finite laws and channels implemented; full gate open | Exact arithmetic, normalized density partitions, owned conditional observations, BEVT v2, weighted images and checked conditional channels; source revisions, shared parameters, solver capabilities, TypeSafe adapters and query observation heads remain |
...
still remain. Full M0–M8 qualification is not complete.
...
and [current handoff](event-evidence/implementation-handoff-conditional-sources/check.json)
record 246 proposal plus 304 native reports. M6 still requires source revisions,
shared parameters, solver capabilities, TypeSafe adapters and observation heads;
