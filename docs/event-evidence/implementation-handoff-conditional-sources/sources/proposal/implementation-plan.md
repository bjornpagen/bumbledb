...
includes complete consumer APIs, retained-memory policy, function/kernel
transport and parameterized descriptors required by M6. Scope each passing gate to the supported format;
...
query integrations and 31 N-API executions qualify that slice. General
...
captured measured maps. WeightedMaps adds 18 reports. Source revisions, parameter
families, solver capabilities, TypeSafe adapters and query observation heads
keep this gate open. Function/kernel descriptor transport and SDK construction
remain explicit integration obligations.
...
coverage and every retained progress/invariant witness. Exact expectation,
belief/information-memory construction, continuous-source certificates and general
