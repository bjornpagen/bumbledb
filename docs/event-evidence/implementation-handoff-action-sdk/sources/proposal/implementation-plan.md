...
Captured and row-bound source plans now construct truth guards and explicitly lift/descend
Events in native/macro/Node/SDK queries, with common bound-predicate rosters.
Captured exact parameter regions now enter the same predicate/guard algebra.
Equal-domain multiple-source guard refinement is also implemented. Certified
factoring keeps M5 open.
...
memory now lowers to the existing action arena. BEBM recipe transport and BEBA
named compilation with SDK knowledge/strategy consumers are implemented. BEAC v1
now replays native general arenas and selected reach/safety strategies. Native
query memory construction, general action/strategy query consumers,
host-program transport and continuous-source certificates remain required by
this gate.
...
[Exact region membership](../docs/event-observation-predicates.md) now enters
native/Rust macro/Node/SDK predicate queries on an explicit ambient domain.
The total membership predicate retains its full authored region and domain;
no numerical observation, source law or prior is invented. Existing strict
partial composition, quantifiers, staging and Common guard refinement apply.
BENP v2 carries the new leaf; all old derivations retain canonical v1 bytes.

Tests cover exact irrational and endpoint witnesses, foreign names, complete
origins, shared admission bounds, both Free Join paths, ordinary partition
coverage, ownership and replay. Six PredicateQueries reference laws increase
the native roster to 762 reports across 44 modules. They are denotational laws,
not native solver/codec/performance proofs. The remaining full M0–M8 gates stay
open, including dynamic/multiple-source query refinement. No release or bump.


## Row-bound single-source guard progress

[Guard source binding](../docs/event-observation-predicates.md) now uses ordinary
full Event columns and explicit `bytes<32>` presentation names in native, Rust
macro, Node and SDK queries. Common predicate rosters, truth cases and exact
transport share captured-plan semantics. Complete source replay occurs for each
participating row under shared execution work; no evidence restriction, new
identity, prior or independence is inferred. Wrong-width identities, proper or
empty source markers, and finite-source parameter refinements refuse.

Both Free Join paths, stored/reopened sources, per-row laws/identities, retained
outputs, spill/cancellation/budget refusal and SDK/description/raw ingress have
consumer evidence. Six row-indexed PredicateGuards reference laws retain all
worlds and their source law, and characterize full source markers. The semantic
roster is 768 reports across 44 modules, without a native refinement claim.

This closes single-source dynamic guard binding only. Multiple-source common
refinement and every other incomplete M0–M8 gate remain required. No release,
tag or version bump.


## Common source guard query progress

`CommonSources(plan, peers...)` now imports peer parameter distinctions into the
primary source, composed with predicate Common rosters and exact transport.
Native/Rust macro/Node/SDK consumers retain every full source, explicit identity,
primary law and exact domain. Permuting or duplicating peer rosters preserves
canonical output within resource bounds. A common parameter partition does not
assert a coupling or a joint probability law. Every participating source is
admitted under shared execution work, including for constant predicates.

Both Free Join paths, independent host constructor comparisons, separate
predicate holes, retained outputs, spill/budget/cancellation, description replay,
malformed wire and semantic refusals qualify this slice. Five source-roster laws
bring the native reference suite to 773 reports across 44 modules. Equal-domain
multiple-source query guard refinement is complete; certified factoring and all
other open M0–M8 requirements remain required. No release or version bump.


## Checked belief-memory recipe progress

[Belief-memory transport](../docs/event-belief-memory.md) now carries full source,
initial evidence, indexed observations and checked action-product recipes in
BEBM v1. Native admission reconstructs all sources, maps, products and the complete
reachable graph, preserving zero-mass possibilities, authored roles/indices and
shared actual parameters. Nested source checks and product normalization consume
one arithmetic allowance; transport/partition/graph/kernel bounds remain explicit.

SDK `EventMemory` provides owned transport, native recipe admission/description
and bounded graph inspection. Native consumers reconstruct from BEBM before
compiling policies and using existing FD/IND, persistence and both Free Join paths.
SDK consumers preserve hidden-history memory after runtime release and join its
ordinary Event regions. Six recipe replay laws bring the reference roster to
779 reports across 44 modules without claiming native code/codec refinement.

This closes possibility-memory recipe transport, not compiled memory/strategy/
program transport or native query memory construction. Every other unfinished
M0–M8 gate remains required. No release, tag or version bump.


## Named belief-arena compilation progress

[BEBA compiled memory](../docs/event-belief-memory.md) now owns a BEBM recipe plus
five authored space identities. Native admission reconstructs both memory and its
exact code/transition presentation under cumulative descriptor and explicit owner
kernel bounds. SDK compilation/description/inspection retains names and rosters;
hidden knowledge/possibility and memory-space ranked reach/continuing-safety results
use the existing native solvers. Ordinary Event regions and BEDC policies feed
stored database consumers after owner/runtime release.

Replayed tiny-game oracles, independent envelope authoring, the erased-screen
strategy, both Free Join paths, shared parameter replay, aggregate budgets,
malformed/foreign inputs and cancellation cover this slice. Five code/role/controlled-
predecessor reference laws bring the native roster to 784 reports across 44 modules.
They assume injective finite codes; native BFS numbering and codec checks remain
tested correspondence obligations. General host-program/strategy transport, native
query memory construction and all other unsatisfied M0–M8 gates stay open.
No release, tag or version bump.


## Checked native action/strategy transport progress

[BEAC arena and strategy recipes](../docs/event-actions.md) now retain the complete
state/action product, transition roles/region, goal or invariant, and optional
selected-policy region. Admission recomputes the solver and checks inclusion plus
required-state coverage; capture preserves actual selected choices. No saved
winning-set or rank assertion is trusted. Finite, fixed-law and supported shared
parameter source admission shares one arithmetic allowance with composition,
product reconstruction and policy normalization. Original converse coordinates,
zero-mass possibilities and explicit source identities survive.

All 256 tiny arenas and 2,048 goal/solver cases now replay before independent
policy enumeration. Tests additionally cover restricted policies, changed goals,
unsafe/incomplete choices, shared parameters, exact work exhaustion, independent
wire authoring, malformed envelopes and cancellation. Database consumers replay
the full strategy before FD/IND, persistence/reopen and both Free Join paths.
Five extensional replay/checked-restriction laws bring the native reference
roster to 789 reports in 44 modules. Native Rust, wire encoding, solver execution,
allocation and performance remain tested correspondence obligations.

This closes native arena/reach/safety transport. General action/strategy SDK
and query consumers, EventProgram transport, native query memory construction,
and every other unfinished M0–M8 gate remain required. No release or version bump.


## Checked action/strategy SDK progress

[`EventAction`](../docs/event-actions.md) exposes the BEAC recipe through owned
SDK carriers, native admission/description/inspection, enabled/good/predecessor
operations, reach/safety solving and restriction of retained policies. All solver
work and source/role checks run in the native worker with shared exact arithmetic.
Descriptions, input operands and full rank exports have cumulative bounds.
`fromMemory` reconstructs BEBA before exporting the named arena; hidden-world
interpretation stays with the memory object.

Runtime-release replay, independent wire authoring, raw copy/shape/arity refusal,
shared parameters, distinct outcome roles, persisted Event joins and the erased-
screen policy consumer cover this slice. The native mathematical semantics and
789-report reference roster are unchanged. Constructive action/strategy query
consumers, EventProgram transport, native query memory construction and all other
unfinished M0–M8 gates remain required. No release or version bump.
