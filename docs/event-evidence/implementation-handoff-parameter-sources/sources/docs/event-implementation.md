...
| M4 faces / maps / relations | Structural relation core and equivalent-partition parameter maps implemented; integration gates remain | Full face/fibre products, complete square certificates, membership-FD roles, converse, composition, modalities, both residuals, graph/readout conversion and safe diagram inspection/reconstruction. BEDC transport, SDK construction/inspection and typed relation queries are implemented; parameter maps/products retain one actual assignment. Common guard refinement, scope-changing maps and complete SDK remain |
...
The [current native semantic run](event-evidence/native-parameter-source-semantics/check.json) adds
**450 reports / 27 files**, using only permitted standard Lean axioms.
