# Event value encoding, versions 1, 2 and 3

These are the implemented codecs in `bumbledb-event`: version 1 for finite
structural spaces, version 2 for exact fixed rational joint laws, and version 3
for sealed univariate parameter sources with optional rational-function laws.
...
complete parameter-family descriptors. Separate [BEDC v1](event-descriptors.md) carries native
...
## Version 3: a shared real parameter and optional family law

BEVT v3 contains three little-endian `u64`-length-prefixed blobs: a canonical
BEVT v1 logical Event, a BEPX v1 parameter binding, and an optional BEPM v1 law.
An empty final blob means unmeasured. BEPX retains the inhabited BEPR domain and
coordinate-sorted deterministic guards. BEPM retains definition-sorted disjoint
density Events with BEGF guarded rational-function definitions. Source identity
includes this binding and declared law; it does not use probability as a key.

Decoding reconstructs guard feasibility, rejects missing parameter fibres,
checks canonical support/binding, and proves density definedness, nonnegativity
and unit normalization on every admitted parameter assignment. It compares the
validated law bytes before publishing an Event. Nested measured Events and
trailing bytes refuse. Existing v1/v2 bytes remain unchanged.

The [parameter-source contract](event-parameter-sources.md) specifies component
formats, declared-definition identity and admission limits. The
[shared-bias fixture](../crates/bumbledb-event/tests/fixtures/event-v3-parameter-source.hex)
is pinned in both Rust and TypeScript. It retains two outcomes sharing one
unknown bias, including the two endpoint guards and the first-head Event.
