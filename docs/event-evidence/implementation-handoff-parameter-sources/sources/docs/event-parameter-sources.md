...
the source, contracted numerator, evidence function and exact positive-evidence
...
parameter assignment**. Its domains must match and its guard presentations must
resolve the same exact partition. Every target guard readout is checked against
...
Different guard partitions currently require an explicit common-refinement
constructor, which remains open. A coarse matching code is insufficient for an
exact parameter-preserving image, so the constructor returns
`ParameterRefinementRequired`. Mapping between a parameterized and parameterless
...
images, each target case admits exactly one source guard pattern, so the existing
finite-code sum counts each outcome once; it does not assign a distribution over
parameters. General parameter-family channels and law pushforwards remain open.

`Event::saturate` may forget outcome coordinates while retaining the parameter;
it refuses direct guard-bit elimination. A separate source refinement/readout
operation must define any change in parameter information.
...
functions, common guard refinement and parameter-changing maps, source channels
and revisions over families, observation query heads, TypeSafe import intents,
