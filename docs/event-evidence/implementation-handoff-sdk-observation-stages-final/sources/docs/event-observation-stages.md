...
## Typed SDK composition

A query returning an observation is a relation expression. `v(query)` mints
variables with the exact probability/expectation result domain; `match`,
`interior`, projection and same-kind equality retain that domain.

```typescript
const beliefs = query(Game).rule((r) => {
    const row = v(Outcome)
    return r.match(Outcome, row).find({
        action: row.action,
        chance: probability(row.success, row.evidence)
    })
})
const legalBeliefs = query(Game).rule((r) => {
    const belief = v(beliefs)
    return r.match(beliefs, belief)
        .match(LegalAction, { action: belief.action })
        .find(belief)
})
```

`QueryRow<typeof legalBeliefs>["chance"]` is an owned `ProbabilityAnswer`. An
expectation producer composes in the same way. `queryFromDescription` accepts
`probabilityResult` / `expectationResult` for forwarded columns as well as
producer heads; descriptions remain owned and author/import without native
loading. Named interiors can produce observations. Projection-only recursive
rules can carry completed observations without another contraction.

The SDK's query domain is `QueryValue`: stored field descriptors plus branded
observation result descriptors. These are absent from stored `AnyField` and
schema builders. Import variables have a query owner, not a fabricated stored
relation. Head signatures, imported bindings and answer decoding use the value
domain; the producer expression separately controls contraction. An imported
observation must bind another observation of the same kind. Stored carrier
class rules continue to govern stored values. Scalar/parameter coercions and
numeric operators refuse at both the typed and dynamic authoring boundaries.
...
The SDK staging suite covers pure imports, exact result typing and compile-time
refusals, dynamic/description refusals, identity joins/antijoins/grouping, named
interiors, recursive forwarding, projection unions, mixed producer/projected
heads, evidence/source identity, prepare/rebind/release, close/reopen, paging,
owned results and unhideable producer errors. The probability/expectation
suites also compare staged and direct shared-parameter results, including
undefined endpoints and imported family payoffs. These SDK tests use the
normal native execution path; forced cursor/spill evidence belongs to the
native tests above, not an SDK execution override.

Observation arithmetic/comparison by numerical value and the complete retained
memory policy remain open. This checkpoint does not complete M5/M6 or the
remaining M0–M8 obligations. No release, tag or version bump.
