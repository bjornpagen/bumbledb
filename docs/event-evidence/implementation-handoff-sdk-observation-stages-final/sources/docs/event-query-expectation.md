...
[Observation stages](event-observation-stages.md) let Rust and typed SDK queries
project and join completed expectations through interiors. Imports, descriptions
and result decoding retain the observation domain separately from the fold that
produced it. Numerical observation arithmetic remains open; this boundary does
not narrow the proposal.
