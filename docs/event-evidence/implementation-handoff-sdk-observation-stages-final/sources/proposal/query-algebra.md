...
now cross the Rust IR, Free Join and SDK boundaries. Observation staging/arithmetic
and the complete Coup templates remain required work.
...
on the branch. Observation staging and arithmetic remain open:
...
existing `Pack { over: VarId }` is now implemented. Observation staging/arithmetic
