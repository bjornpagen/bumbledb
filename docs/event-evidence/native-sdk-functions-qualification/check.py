...
def main():
    ...
    result = {
        "sdk_scope": ("Owned Event field/carriers, worker-backed host algebra and supported Event/descriptor ingress, all seven BEDC structural constructors/descriptions/inspections, exact BERA scalars/BESC finite functions, weighted images, strict law designation and owned fixed-law observations with shared admission arithmetic, Event/Test/map/relation query builders/imports, complete operand diagnostics, generated bindings, native dependencies, persistence/reopen, literals/parameters, Pack and ownership/cancellation; kernel/revision SDK operations, parameter families, complete SDK and aggregate retained-memory policy remain open" if args.sdk else None),
