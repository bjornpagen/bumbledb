...
def main():
    ...
    result = {
        "sdk_scope": ("Owned Event field/carriers, worker-backed host algebra and supported Event/descriptor ingress, Event/Test/map/relation query builders and description imports, complete structured Event operand diagnostics, stored-field generated bindings, native dependency admission, persistence/reopen, literals/parameters, Pack, ownership/cancellation and related SDK regressions; source/descriptor construction and aggregate retained-memory policy remain open" if args.sdk else None),
