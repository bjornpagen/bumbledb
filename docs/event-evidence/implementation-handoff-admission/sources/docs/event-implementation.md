...
| M3 dependencies / admission | Native field projections implemented; full gate open | Pointwise overlap, union coverage, final-state repair/deletion, empty-key lookup refusal and conservative planner witnesses work. Contextual full projection constants remain pending; Event selections, capacity positions and closed rosters still refuse `EventContractPending` |
...
Contextual `true` is not implemented by these field-only projections. It needs
a typed projection term throughout schema declarations, wire/fingerprints,
compiled metadata and admission; a scalar Boolean or fabricated field ID cannot
stand in for it. Event selections, capacity and closed rosters remain explicitly
guarded until their native contracts are qualified.

## Lean evidence and remaining correspondence

The proposal suite was rerun: **246 reports / 30 files, 160 axiom-free**.
The [native semantic run](event-evidence/admission-semantics-initial/check.json) adds
**30 reports / three files**, using only permitted standard Lean axioms.
...
for duplicate contributors and subtracting deleted coverage. These laws assume
