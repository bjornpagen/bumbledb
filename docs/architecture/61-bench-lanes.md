...
| `bench-nosync-r1..r3` (×3) | ledger + calendar | the same roster | `bench --nosync --out` | `report.json` (RunReport, `config.store` nosync) | Report — the NOSYNC flag lane (hidden constructors, not a kind) | the same session minus the sync boundary: `synchronous=OFF`, `fullfsync=OFF` — the honest `MDB_NOSYNC` twin |
...
`"lane"` key** and is classified by `config.store` into the durable/nosync
