...
Queries are **plain Rust data structures** — serializable, inspectable, no behavior. The
...
variables is unification and obeys identical type rules. Literal-vs-literal and
param-free constant comparisons are validation errors (write the query you mean).

**Params:** a param's type is inferred from its anchors — the fields it binds and the
typed terms it compares against. A param with no anchor, or with conflicting anchors, is
a validation error. At execution bind time the supplied values are checked for count and
structural type; String/Bytes params (and literals) resolve to intern ids by read-only
dictionary lookup **per execution** — a miss means the query is empty on this snapshot,
never an error, and a value interned by a later write is picked up on the next
execution (no stale-resolution trap; the lookup is one LMDB get, allocation-free).
...
3. Output: distinct-variable atoms + a per-atom filter list + a **residual list** (the
   comparisons whose sides come from different atoms).
...
param-anchor type mismatches; enum ordinal out of range for the field's variant list;
comparisons violating the type rules above; constant comparisons; unbound find
variables; comparison-only variables; empty finds; duplicate find terms; no atoms;
