...
`codex/event-algebra`. It is the first M4 slice, not the complete relation or
source API. It neither changes BEVT v1 nor adds a release.
...
projection/substitution square or a law pushforward. Those remain different
descriptors with different obligations in the full M4/M6 contract.
...
Map descriptors do not yet have a persistent wire format. Their **result Events**
already use ordinary BEVT identity, storage, dependency admission and Free Join
equality. Constructive map heads in database queries belong to M5 and are not
implemented by this host API.
...
records source hashes and test logs. Complete fibre products, role admission,
relations/residuals, graph views, solver-backed parameter maps and query stages
remain subject to the full implementation plan.
