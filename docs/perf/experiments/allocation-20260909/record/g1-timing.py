...
from g1_experiment import fingerprint
...
builds = {'A': ROUND/f'g1-time-build-baseline-{args.baseline_build}',
          'B': ROUND/f'g1-time-build-candidate-{args.candidate_build}'}
...
source = fingerprint()
assert source == load(ROUND/'g1-gates-2/STATE.json')['source_fingerprint']
phase = Phase(f'g1-timing-{args.attempt}')
...
phase.state.update(diagnostic_only=False, source_fingerprint=source, cases=cases, order=order,
    executables={key: dict(path=str(path/'g1-timing'), sha256=states[key]['binary_sha256'],
