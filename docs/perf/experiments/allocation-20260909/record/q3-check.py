#!/usr/bin/env python3
"""Serial correctness/lint/oracle gate for Q3, no timing or full trace."""
import argparse
import json
import shutil
from diagnostics import ENV, Phase, ROUND, digest
from q3_experiment import TREE, Q2, fingerprint, freeze, prior_identities

parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument('attempt', type=int)
args = parser.parse_args()
identity = fingerprint()
phase = Phase(f'q3-gates-{args.attempt}')
shutil.copy2(__file__, phase.path/'check.py')
shutil.copy2(ROUND/'q3_experiment.py', phase.path/'q3_experiment.py')
phase.state.update(source_fingerprint=identity, source_tree=str(TREE),
    base_q2_fingerprint=Q2['source_fingerprint'], prior_identities=prior_identities(),
    acceptance='Hardening/correctness only. Single row cardinality and ordinal entry unmeasured; no speed/RSS/Pi/full-suite acceptance.')
phase.save()
...
try:
    ...
    artifacts = [json.loads(line) for line in (phase.path/'release-build.log').read_text().splitlines()
    ...
    phase.finish('Q3-CORRECTNESS-GATES-COMPLETE-REVIEW-REQUIRED')
