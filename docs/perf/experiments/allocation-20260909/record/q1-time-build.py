...
from q1_time_experiment import TREE,GATE,git,source,dependencies
...
phase=Phase(f'q1-time-build-{args.variant}-{args.attempt}')
phase.state.update(variant=args.variant,source_tree=str(TREE),source_files=files,
    candidate_identity=GATE['source_fingerprint'],dependencies=deps,prior_identities=prior_identities(),
...
shutil.copy2(ROUND/'q1_time_experiment.py',phase.path/'q1_time_experiment.py')
