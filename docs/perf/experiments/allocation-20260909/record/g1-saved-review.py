...
paths = {'baseline': ROUND/'m1-saved-baseline-1', 'candidate': ROUND/f'g1-saved-{args.attempt}'}
...
for label in labels:
    assert a[0][label][:3] == b[0][label][:3]
...
for key, before in a[1].items():
    after = b[1][key]
    assert before[:2] == after[:2], ('row/map count', key)
    assert before[3] == after[3], ('published table bytes', key)
    assert before[7] == after[7], ('map shape histogram', key)
    assert after[4] == after[3], ('retired map content remains', key)
...
root = b[1][('cold-1', 1, 0)]
assert root[2] == [(131072, 196608), (262144, 393216), (43236, 65536)]
assert root[4:6] == (2401168, 3604480)
print('PASS all 20 SQL windows and 78 ownership pairs reviewed;', changed, 'changed snapshots')
print('All published map shapes/bytes unchanged. Every candidate arena contains live tables only.')
print('Warm/repeat and second-rotation windows request zero; all clones zero.')
