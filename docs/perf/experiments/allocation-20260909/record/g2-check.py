#!/usr/bin/env python3
"""Serial G2 red/green checks; no timings or tracing."""
import argparse
import json
import shutil
...
source = fingerprint()
phase = Phase(f'g2-{args.mode}-{args.attempt}')
...
try:
    ...
    if args.mode == 'baseline':
        ...
        try:
            ...
            raise AssertionError('Baseline must reproduce the retention discriminator')
    else:
        phase.run('focused',cargo('test','--locked','-p','bumbledb','--lib',
            'exec::colt::tests::','--','--nocapture','--test-threads=1'))
        if args.mode=='gates':
            for name,command in [
