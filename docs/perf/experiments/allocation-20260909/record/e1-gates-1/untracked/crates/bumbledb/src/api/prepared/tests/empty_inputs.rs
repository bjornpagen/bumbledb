...
fn u64_memo(plan: &crate::plan::fj::ValidatedPlan) -> ViewMemo {
        ...
        let columns = occurrence
            ...
            .map(|level| {
                level
                    .iter()
                    .map(|var| {
                        let (field, _) = occurrence.vars.iter().find(|(_, v)| v == var).unwrap();
            ...
            .collect();
        memo.push(
            Colt::new(crate::image::view::View::Unbound, &[], columns),
            Binding::Unbound,
        );
    }
    memo
...
fn an_empty_rule_does_not_erase_its_live_alternative() {
    ...
    fix.insert_dyn(GATE, &[vec![Value::U64(9)]]);
    let live = Rule {
        finds: vec![FindTerm::Var(VarId(0))],
        atoms: vec![gate(true)],
        negated: vec![],
    ...
    for (lo, hi) in [(20, 21), (0, 1), (30, 31)] {
        let out = fix
            .execute(&mut prepared, &[BindValue::U64(lo), BindValue::U64(hi)])
            .unwrap();
        let mut expected: Vec<_> = (0..8).filter(|id| lo <= *id && *id < hi).collect();
        expected.push(9);
        assert_eq!(accounts(&out), expected);
    }
}
...
fn empty_fold_inputs_still_finalize_and_reuse_their_sinks() {
    ...
    for grouped in [false, true] {
        ...
        for (lo, hi) in [(20, 21), (0, 8), (20, 21)] {
            ...
            .unwrap();
            if lo == 20 {
                // BumbleDB has no aggregate group without a witness, even
                // for global Count/Sum (unlike SQL's synthetic empty group).
                assert!(out.is_empty());
            } else {
                assert!(!out.is_empty());
            }
...
struct CancelOnLastEpoch<'a> {
    epochs: Vec<(RelationId, crate::image::ViewEpoch)>,
    calls: std::cell::Cell<usize>,
    work: &'a crate::WorkContext,
}

impl crate::image::ImageBind for CancelOnLastEpoch<'_> {
    fn epoch(
        &self,
        _: &Schema,
        relation: RelationId,
    ) -> crate::error::Result<crate::image::ViewEpoch> {
        let index = self.calls.get();
        let (expected, epoch) = self.epochs[index];
    ...
    fn image(
        ...
        _: &Schema,
        _: RelationId,
    ) -> crate::error::Result<std::sync::Arc<crate::image::RelationImage>> {
        panic!("warmed bindings must hit the memo")
    }
    ...
    fn peek(
        ...
        _: &Schema,
        _: RelationId,
    ) -> crate::error::Result<Option<std::sync::Arc<crate::image::RelationImage>>> {
        panic!("warmed bindings must hit the memo")
    }
    ...
    fn selection_image(
        ...
        _: &Schema,
        _: RelationId,
        _: &[crate::plan::fj::Selection],
        _: &[crate::image::view::ResolvedWords],
    ) -> crate::error::Result<Option<std::sync::Arc<crate::image::RelationImage>>> {
        panic!("warmed bindings must hit the memo")
    }
}
...
#[test]
fn empty_input_and_selection_miss_flush_current_cancellation() {
    let fix = store("empty-input-cancellation");
    for selection_miss in [false, true] {
        let mut query = triangle();
        let mut args = vec![BindValue::U64(20), BindValue::U64(21)];
        if selection_miss {
            query.conditions = vec![ConditionTree::Leaf(Comparison {
                op: CmpOp::Eq,
                lhs: Term::Var(VarId(0)),
                rhs: Term::Param(ParamId(0)),
            })];
        ...
        let mut prepared = fix.prepare(&Query::single(query)).unwrap();
        assert!(fix.execute(&mut prepared, &args).unwrap().is_empty());
        let work = crate::WorkContext::new();
        let [PreparedRule::FreeJoin(rule)] = prepared.pipeline.main_rules_mut() else {
            panic!("free join fixture")
        };
        assert_eq!(
            rule.resolved_selections.iter().any(|keys| !keys.is_empty()),
            selection_miss
        );
        let epochs = rule
            .plan
            ...
            .zip(&rule.memo.occs)
            .map(|(occ, memo)| {
                let Binding::Bound(bound) = &memo.active else {
                    panic!("warmed binding")
                };
                (occ.source().edb().unwrap(), bound.epoch)
            })
            .collect();
        let images = CancelOnLastEpoch {
            epochs,
            calls: std::cell::Cell::new(0),
            work: &work,
        };
        let mut sink = ProjectionSink::new(vec![rule.plan.slot_of(VarId(0))]);
        let result = super::super::run_join::run_join(
            &rule.plan,
            &prepared.schema,
            &images,
            &work,
        ...
        assert_eq!(images.calls.get(), images.epochs.len());
        assert!(
            matches!(result, Err(Error::Store(error))
            if matches!(*error, crate::storage::store::StoreError::Work(crate::work::WorkError::Cancelled))),
            "cancellation became a successful empty result; selection_miss={selection_miss}"
        );
        assert!(fix.execute(&mut prepared, &args).unwrap().is_empty());
        args[0] = BindValue::U64(0);
        let expected: Vec<_> = if selection_miss {
            vec![0]
        } else {
            (0..8).collect()
        };
        assert_eq!(
            accounts(&fix.execute(&mut prepared, &args).unwrap()),
            expected
        );
    }
}
