...
from e1_experiment import fingerprint

parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument('attempt', type=int)
parser.add_argument('--baseline-build', type=int, default=1)
parser.add_argument('--candidate-build', type=int, default=2)
args = parser.parse_args()
builds = {'A': ROUND/f'e1-time-build-baseline-{args.baseline_build}',
          'B': ROUND/f'e1-time-build-candidate-{args.candidate_build}'}
...
source = fingerprint()
assert source == load(ROUND/'e1-gates-1/STATE.json')['source_fingerprint']
phase = Phase(f'e1-timing-{args.attempt}')
...
phase.state.update(diagnostic_only=False, source_fingerprint=source, cases=cases, order=order,
    executables={key: dict(path=str(path/'e1-timing'), sha256=states[key]['binary_sha256'],
    ...
    protocol='Per-case ABBA. Four existing triangle draws and four point controls, S/seed 1. '
             ...
             'Rotating: alternate draw (i+1)%4 outside timer, then target, 8 warmups +64 samples. '
             'Memoized: 8 warmups +64 timed batches of16 same-target calls. All raw elapsed values '
