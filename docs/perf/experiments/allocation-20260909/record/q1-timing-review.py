#!/usr/bin/env python3
"""Audit every fixed Q1 timing sample; preserve tails, clock flags and adverse cases."""
import argparse
import json
import math
import shutil
...
path = ROUND/f'q1-timing-{args.timing}'
state = load(path/'STATE.json')
assert state['status'] == 'ORDINARY-Q1-TIMING-COMPLETE-REVIEW-REQUIRED'
assert state['processes_completed'] == 112 and state['distributions'] == 672
assert len(state['steps']) == 112 and all(s['status'] == 'PASS' for s in state['steps'])
...
cases = [(f, d) for d in range(4) for f in ['saved_triangle', 'saved_point', 'saved_range']]
cases += [('computed', i) for i in [0, 1, 4]] + [('union', i) for i in [2, 4]]
cases += [('groups', i) for i in [1, 3, 4]]
cases += [('dnf_groups', 0), ('dnf_groups', 2), ('pairs', 2), ('dense', 2),
          ('interior', 4), ('reach', 2), ('interior_reach', 2), ('union_groups', 2)]
order = ['A0', 'B0', 'B1', 'A1']
...
phase = Phase(f'q1-timing-review-{args.attempt}')
...
try:
    ...
    assert len(rows) == 672 and len(stamps) == 336 and len(logs) == 112
    ...
    phase.finish('ALL-ORDINARY-Q1-DISTRIBUTIONS-VERIFIED-INTERPRETATION-REQUIRED')
