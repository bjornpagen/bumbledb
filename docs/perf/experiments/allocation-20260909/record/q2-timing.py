#!/usr/bin/env python3
"""One predeclared ordinary Q2-vs-Q1 ABBA panel; no retries, profile capture or full suite."""
import argparse
import json
import shutil
import subprocess
...
builds={'A':ROUND/f'q2-time-build-q1-{args.baseline_build}','B':ROUND/f'q2-time-build-q2-{args.candidate_build}'}
states={key:load(path/'STATE.json') for key,path in builds.items()}
for key,state in states.items():
    assert state['status']=='ORDINARY-TIMING-BINARY-FROZEN' and state['variant']=={'A':'q1','B':'q2'}[key]
...
# Fixed in q2_time_experiment before observing Q2 allocation or timing results.
cases=CASES
...
phase=Phase(f'q2-timing-{args.attempt}')
...
else:phase.finish('ORDINARY-Q2-TIMING-COMPLETE-REVIEW-REQUIRED')
