...
fn empty_positive_inputs_leave_all_join_maps_unforced() {
    ...
    for missing_relation in [false, true] {
        ...
        fix.db.read(crate::api::db::test_operation(), |instance| {
            ...
            let node = |pairs: &[(u16, u16)]| Node {
                subatoms: pairs.iter().map(|&(occ, var)| Subatom {
                    occ: OccId(occ), vars: vec![VarId(var)],
                }).collect(),
                estimate: 512,
            };
            ...
            if missing_relation {
                nodes[2].subatoms.push(Subatom { occ: OccId(3), vars: vec![VarId(0)] });
            }
            let plan = crate::plan::fj::validate(
                &FjPlan { nodes }, &normalized[0], schema, &witness.rule(0).sink_vars(),
            ).expect("valid late-empty plan");
            let mut memo = ViewMemo::new();
            for occurrence in plan.occurrences() {
                assert!(occurrence.selections.is_empty());
                let columns = occurrence.trie_schema.iter().map(|level| {
                    level.iter().map(|var| {
            ...
            let filters: Vec<_> = plan.occurrences().iter().map(|occ| occ.filters.clone()).collect();
            let selections = vec![Vec::new(); plan.occurrences().len()];
            let source = super::super::source::QuerySource::store(instance.snapshot(), instance.work());
            ...
            super::super::run_join::run_join(
                &plan, schema, &images, instance.work(), &mut executor, &mut bindings,
                &filters, &selections, &mut memo, &super::super::reach::OccImages::default(),
                &mut Vec::new(), &mut sink, &mut crate::exec::run::NoopCounters,
            )?;
            assert_eq!(sink.len(), 0);
            observed.push((missing_relation, memo.colts.iter()
                .map(|colt| colt.forced_capacity(Colt::root())).collect::<Vec<_>>()));
            Ok(())
        }).unwrap();
    }
    assert!(observed.iter().all(|(_, capacities)| capacities.iter().all(Option::is_none)),
