#!/usr/bin/env python3
"""Verify unmounted G1 gate source and preservation of all other candidates."""
import argparse
import runpy
import shutil
from diagnostics import Phase, ROUND, load
from g1_experiment import TREE, fingerprint as g1
...
phase = Phase(f'g1-closeout-{args.attempt}')
shutil.copy2(__file__,phase.path/'closeout.py')
expected = dict(load(ROUND/'e1-closeout-1/STATE.json')['identities'])
expected['G1'] = load(ROUND/'g1-gates-2/STATE.json')['source_fingerprint']
def identities():
    return {'E1':e1(),'M1':m1(),'P3':p3(),'G1':g1(),
