...
fn result_backing_waits_for_actual_rows() {
    let mut set = SpillSet::new(1, true);
    assert_eq!(
    ...
    assert_eq!(set.ram.retained_bytes_for_test(), 167);
    assert_eq!(
...
fn all_widths_grow_preserve_order_and_reuse_then_release() {
    for width in [0, 1, 2, 4, 8, 9] {
        for ordered in [false, true] {
            let mut set = SpillSet::new(width, ordered);
...
fn aggregate_regimes_do_not_reserve_speculative_tables() {
    ...
    let spans = [(0, 1), (1, 1)];
    let sinks = [
        AggregateSink::for_bindings(&finds, 2, &[]),
        AggregateSink::for_union(&finds, 2),
        AggregateSink::for_dnf_union(&finds, 2, &spans),
    ];
    for sink in sinks {
    ...
    let dense = AggregateSink::for_bindings(&finds, 2, &[2]);
