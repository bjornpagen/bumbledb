...
#[test]
fn borrowed_shape_cache_follows_layout_contents_at_the_same_address() {
    ...
    bindings.set(group, 7);
    bindings.set(amount, i64_to_word(3));
    let mut key_slots = [group];
    let address = key_slots.as_ptr();
    for (slot, word, constant_group) in [
        (group, 7, false),
        (amount, i64_to_word(5), true),
        (amount, i64_to_word(8), true),
        (group, 9, false),
    ] {
        key_slots[0] = slot;
        assert_eq!(key_slots.as_ptr(), address);
        assert_eq!(
        ...
        assert_eq!(sink.cached_key_slots, key_slots);
        assert_eq!(sink.cached_constant_group, constant_group);
        assert_eq!(sink.binding_scratch.capacity(), 0);
    }
    assert_eq!(
        sink.into_answers().unwrap(),
        [vec![7, i64_to_word(16), 3], vec![9, i64_to_word(3), 1]]
    );
}
