#!/usr/bin/env python3
"""Freeze an ordinary release binary for matched per-draw G1 timing."""
import argparse
import json
import shutil
...
production = load(ROUND/'g1-gates-2/STATE.json')['ordinary_file_sha256']
...
phase = Phase(f'g1-time-build-{args.variant}-{args.attempt}')
...
try:
    ...
    phase.run('clippy', cargo('clippy', '--locked', '-p', 'bumbledb-bench', '--bin', 'g1-timing',
        '--', '-D', 'warnings'))
    phase.run('build', cargo('build', '--locked', '--release', '-p', 'bumbledb-bench',
        '--bin', 'g1-timing', '--message-format=json'))
    ...
    binaries = [r['executable'] for r in artifacts if r['target']['name'] == 'g1-timing' and r['executable']]
    assert len(binaries) == 1
    binary = phase.path/'g1-timing'
