#!/usr/bin/env python3
"""Frozen serial E1 correctness/build checks, never CPU tracing."""
import argparse
import shutil
from diagnostics import ENV, Phase, REPO, digest
from e1_experiment import TREE, fingerprint, freeze

parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument('mode', choices=['focused', 'gates'])
parser.add_argument('attempt', type=int)
args = parser.parse_args()
source = fingerprint()
phase = Phase(f'e1-{args.mode}-{args.attempt}')
shutil.copy2(__file__, phase.path/'check.py')
phase.state.update(source_fingerprint=source, source_tree=str(TREE),
    acceptance='Correctness/build only. No timing, CPU trace, other candidates or releases.')
phase.save()
...
try:
    ...
    checks = [
        ...
        ('focused', cargo('test', '--locked', '-p', 'bumbledb', '--lib',
            'empty_inputs::', '--', '--nocapture', '--test-threads=1')),
    ]
    if args.mode == 'gates':
        checks += [
    ...
    phase.finish('CHECKS-COMPLETE-REVIEW-REQUIRED')
