...
| `rewrites` | 13 | the dual-pipeline differential: every query × draw executed through the rewritten pipeline (grounding + statically-empty fold on) and the rewrite-free one (the `ground-off`/`fold-off` thread-local switches — ONE build; cargo refuses a dual-build dependency on one package), demanding identical result sets; non-vacuity tallied off the profile surface |
...
| 2026-07-16 | `theory` | harness oracle wrong, not engine: the fresh-store assert demanded `verify_store` green for EVERY accepted schema, but a genesis-violating schema is legal by the recorded division of authority (`30-dependencies.md` — acceptance judges the theory, the sweeper re-verifies the extension-sourced classes): a closed-parent floor window (`Holder <={2..*} Account`, `Holder` closed with one axiom) accepts, creates, and correctly reports `WindowViolation{count: 0}` on the empty store — a spurious crash on documented behavior. Fixed: the oracle now demands the citation set EQUAL the schema's genesis debt (`genesis_debt_statements` — closed-source/ordinary-target containments and closed-parent floor windows with selected axioms), nothing more, nothing less | `trophies/theory/genesis-debt-closed-parent-window` via `tests/replay.rs` |
