...
#[path = "occupancy_scratch.rs"]
mod scratch;
#[path = "occupancy_words.rs"]
mod words;

pub(super) fn intersection_count<const K: u32>(
    arena: &Arena<K>, views: [Restricted; 2], variables: u64,
) -> u64 {
    words::intersection_count(arena, views, variables)
}
...
pub(super) struct Restricted {
    pub(super) root: Ref,
    pub(super) pinned: u64,
    pub(super) values: u64,
}
impl Restricted {
    pub(super) fn new(root: Ref) -> Self {
        Self {
            root,
            pinned: 0,
            values: 0,
    ...
    pub(super) fn remaining<const K: u32>(self, arena: &Arena<K>) -> u64 {
        arena.variables(self.root) & !self.pinned
    }
    pub(super) fn cofactor<const K: u32>(self, arena: &Arena<K>, variable: u32, high: bool) -> Self {
...
pub fn classify_with<const K: u32>(
    ...
    fn visit<const K: u32, const MODE: u8, const N: usize>(
        arena: &Arena<K>,
        order: &[u32],
        views: [Restricted; 3],
        memo: &mut FxHashMap<[Restricted; 3], u8>,
        stats: &mut Observation,
        workspace: &mut scratch::Workspace<N>,
    ) -> u8 {
        if views[0].root == 0 {
            return 0;
        }
        ...
        let signature = if variables.count_ones() <= K {
            stats.local_cubes += 1;
            if MODE == 2 {
                scratch::classify(arena, views, variables, workspace, stats)
            } else if MODE == 1 {
    ...
    result.signature = match kernel {
        Kernel::Scalar => visit::<K, 0, 0>(
            arena,
            order,
            inputs,
            &mut memo,
            &mut result,
            &mut scratch::Workspace::new(),
        ),
        Kernel::Words => visit::<K, 1, 0>(
            arena,
            order,
            inputs,
            &mut memo,
        ...
        Kernel::Scratch => {
            // One workspace per classification, not per recursive frame.
            // All local contractions return before either child reuses it.
            if K <= 6 {
                visit::<K, 2, 1>(
                    arena,
                    order,
...
mod tests {
    ...
    fn both<const K: u32>(
        ...
        let scratch = classify_with(Kernel::Scratch, arena, order, support, a, b);
        ...
        assert_eq!(scratch.signature, words.signature);
        assert_eq!(scratch.subproblems, words.subproblems);
        assert_eq!(scratch.memo_entries, words.memo_entries);
        assert_eq!(scratch.local_cubes, words.local_cubes);
        assert_eq!(scratch.word_batches, words.word_batches);
        assert_eq!(scratch.borrowed_planes, words.borrowed_planes);
        assert_eq!(scratch.aligned_planes, words.aligned_planes);
        assert_eq!(scratch.constant_planes, words.constant_planes);
        assert_eq!(scratch.scratch_alignments, words.alignment_allocations);
        assert_eq!(scratch.alignment_allocations, 0);
        assert_eq!(scratch.local_assignments, 0);
        scratch
    }
