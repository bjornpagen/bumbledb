    ...
    for name in inputs:
        ...
        for run in data['runs']:
            ...
            for row in run['rows']:
                ...
                if row['scenario'] == 'axes_65536':
                    if axes_reference is None:
                        axes_reference=json.loads((LAB/'results/axes-reference.json').read_text())
                        assert axes_reference['passed']
                    expected=next(q for q in axes_reference['queries'] if q['shape']==row['shape'])
                    assert row['histogram']==expected['histogram']
                    assert row['rows']==expected['rows'] and row['accepted']==expected['accepted'][row['predicate']]
                    if row['candidate'].startswith('essential'):
                        assert row['signature_cache_entries']==(expected['normalized_alignment_pairs'] if row['memo'] else 0)
