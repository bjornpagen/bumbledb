...
ap.add_argument('--view-reuse',choices=['off','bounded'],default='off')
ap.add_argument('--view-normalize',choices=['deferred','source'],default='deferred')
args=ap.parse_args()
...
env=dict(os.environ,CARGO_TARGET_DIR=str(SCRATCH/'view-target'),
    EVENT_LAB_VIEW_NORMALIZE=args.view_normalize,EVENT_LAB_VIEW_REUSE=args.view_reuse,EVENT_LAB_VIEW_KERNEL=args.view_kernel,EVENT_LAB_ESSENTIAL_KERNEL='derived',EVENT_LAB_ESSENTIAL_LAYOUT=args.layout)
...
result=dict(passed=run.returncode==0,source_hashes=before,command=command,log=log,
    essential_layout=args.layout,essential_kernel='derived',view_kernel=args.view_kernel,view_reuse=args.view_reuse,view_normalize=args.view_normalize,
