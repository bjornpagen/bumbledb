            ...
            for policy in ["joint", "active", "witness"] {
                ...
                for completion in ["full", "needed", "local", "local-needed"] {
                    ...
                    for execution in ["staged", "fused"] {
                        c.set_projection_execution(execution);
...
fn finite<const K: u32, const PREFIX: bool>() {
    ...
    let mut raw = Retraction::<K, PREFIX>::new(&[0b01101011], 8);
    let transforms = raw.check_pair_transform();
    println!(
        concat!(
            "EVENT_LAB {{\"kind\":\"fused_projection_verification\",",
            "\"candidate\":\"{}\",\"product_cases\":{},\"coupled_cases\":{},",
            "\"policies\":3,\"strategies\":4,\"substitution_cases\":{},\"execution_policies\":2,\"transform_cases\":{},\"passed\":true}}"
        ),
