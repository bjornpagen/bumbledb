"""Compare staged and direct construction of completed projections."""
...
KEYS = ['domain', 'width', 'bits', 'readout_family', 'readout_faces', 'layout', 'memo',
        'candidate', 'essential_layout', 'retraction_count', 'retraction_factor', 'retraction_normalize', 'retraction_project', 'retraction_complete', 'retraction_execute']
FIELDS = [('domain', 'EVENT_LAB_LEGAL_DOMAIN'), ('readout_family', 'EVENT_LAB_READOUT_FAMILY'),
          ...
          ('retraction_normalize', 'EVENT_LAB_RETRACTION_NORMALIZE'), ('retraction_project', 'EVENT_LAB_RETRACTION_PROJECT'), ('retraction_complete', 'EVENT_LAB_RETRACTION_COMPLETE'), ('retraction_execute', 'EVENT_LAB_RETRACTION_EXECUTE')]
...
def collect(inputs):
    ...
    for name in inputs:
        ...
        for run in data['runs']:
            ...
            for r in run['rows']:
                ...
                assert r['retraction_execute'] in ['staged','fused']
                assert r['retraction_complete'] in ['full','needed','local','local-needed']
                ...
                if not r['candidate'].startswith(('prefix','retraction')):
                    assert r['retraction_normalize'] == 'staged' and r['retraction_project'] == 'joint' and r['retraction_complete'] == 'full' and r['retraction_execute'] == 'staged'
    ...
    for key, (r, source) in rows.items():
        choices = []
        if r['retraction_execute'] == 'fused':
            choices.append(('execution',dict(retraction_execute='staged')))
        if r['retraction_complete'] != 'full':
            choices.append(('completion',dict(retraction_complete='full')))
        if r['retraction_complete'] == 'local-needed':
            choices.append(('locality',dict(retraction_complete='needed')))
            choices.append(('traversal',dict(retraction_complete='local')))
        ...
        if candidate.startswith(('prefix', 'retraction')):
            ...
            choices.append(('anchor', dict(candidate='essential'+suffix, retraction_factor='joint', retraction_normalize='staged',retraction_project='joint',retraction_complete='full',retraction_execute='staged')))
        ...
        if candidate.startswith(('prefix', 'retraction', 'essential')):
            ...
            for control in ['packed512', 'dense-dispatched']:
                choices.append(('control', dict(candidate=control, essential_layout='enum', retraction_factor='joint', retraction_normalize='staged',retraction_project='joint',retraction_complete='full',retraction_execute='staged')))
        ...
        for comparison, changes in choices:
            ...
            if comparison in ['projection','projection-strength','completion','locality','traversal','execution']:
...
def main():
    ...
    parser.add_argument('--output', default='FUSED-PROJECTION-MEASUREMENTS.md')
    parser.add_argument('--record', default='fused-comparison.json')
    ...
    lines = ['# Same-binary direct completed projection after Free Join', '',
    ...
    if incomplete:
        ...
        for p in incomplete:
            ...
            cells = [j.get('EVENT_LAB_CANDIDATE', 'verification'),
                     j.get('EVENT_LAB_RETRACTION_NORMALIZE', '')+'/'+j.get('EVENT_LAB_RETRACTION_PROJECT','')+'/'+j.get('EVENT_LAB_RETRACTION_COMPLETE','')+'/'+j.get('EVENT_LAB_RETRACTION_EXECUTE',''),
    ...
    for group in sorted({key[:7] for key in rows}):
        ...
        for key, (r, _) in sorted(rows.items()):
            if key[:7] != group:
                continue
            values = [r['candidate']+'/'+r['essential_layout']+'/'+r['retraction_normalize']+'/'+r['retraction_project']+'/'+r['retraction_complete']+'/'+r['retraction_execute'], r['retraction_factor']]
