"""Native ownership/symbolic acceptance for local completion paths."""
...
ap.add_argument('--completion',nargs='+',choices=['full','needed','local','local-needed'],default=['local','local-needed'])
...
jobs = [dict(common, EVENT_LAB_CANDIDATE=candidate, EVENT_LAB_LANE=lane,
             EVENT_LAB_PRODUCT=product, EVENT_LAB_RETRACTION_NORMALIZE=mode, EVENT_LAB_RETRACTION_PROJECT=gates, EVENT_LAB_RETRACTION_COMPLETE=completion)
        ...
        for mode in args.normalize for gates in args.gates for completion in args.completion]
