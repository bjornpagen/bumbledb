...
ap=argparse.ArgumentParser();ap.add_argument('--output',default='build-pack-native.json');args=ap.parse_args()
check_engine_sources()
engine=SCRATCH/'pack-engine';engine.mkdir(exist_ok=True)
...
def sources():return {str(p.relative_to(LAB)):digest(p) for p in sorted((LAB/'pack-native-src').glob('*.rs'))}
...
print('Building isolated pack/native comparison.',flush=True)
