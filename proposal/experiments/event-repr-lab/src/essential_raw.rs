...
mod storage;
#[path = "view_product.rs"]
mod view_product;
pub use view_product::Statistics as ViewProductStatistics;
use view_product::{LocalKernel, Normalization, Reuse};
...
pub struct Arena<const K: u32> {
    ...
    conditionals: FxHashMap<(Ref, Ref, Ref), Ref>,
    cofactors: FxHashMap<(Ref, u32, bool), Ref>,
    word_kernels: bool,
    derived: bool,
    view_kernel: LocalKernel,
    view_reuse: Reuse,
    view_normalization: Normalization,
...
impl<const K: u32> Arena<K> {
    ...
    pub fn cache_bytes(&self) -> usize {
        self.applications.capacity() * (std::mem::size_of::<((u8, Ref, Ref), Ref)>() + 1)
            + self.conditionals.capacity() * (std::mem::size_of::<((Ref, Ref, Ref), Ref)>() + 1)
    ...
    pub fn new(dimensions: u32, order: &[u32]) -> Self {
        ...
        Self {
            ...
            rank,
            store: Store::new(
                std::env::var("EVENT_LAB_ESSENTIAL_LAYOUT")
                    .as_deref()
                    .unwrap_or("enum"),
            ),
            applications: FxHashMap::default(),
            conditionals: FxHashMap::default(),
            ...
            view_kernel: LocalKernel::from_env(),
            view_reuse: Reuse::from_env(),
            view_normalization: Normalization::from_env(),
    ...
    /// Build the conditional directly, without interning its two gated terms.
    /// Polarity normalization is valid for every storage/kernel control.
    pub fn ite(&mut self, mut selector: Ref, mut high: Ref, mut low: Ref) -> Ref {
        if selector & 1 != 0 {
            selector ^= 1;
        ...
        high ^= flip;
        low ^= flip;
        self.ite_inner(selector, high, low) ^ flip
    }
    fn ite_inner(&mut self, selector: Ref, high: Ref, low: Ref) -> Ref {
        if selector == 0 {
            return low;
        }
        if high == low {
        ...
        if let Some(&out) = self.conditionals.get(&(selector, high, low)) {
            return out;
        }
        let variables = self.variables(selector) | self.variables(high) | self.variables(low);
        let out = if variables.count_ones() <= K {
            let data = if self.word_kernels {
                word_kernels::select(
                    &self.aligned(selector, variables),
                    &self.aligned(high, variables),
                    &self.aligned(low, variables),
                )
            } else {
                let names = axes(variables);
                words(1usize << names.len(), |i| {
            ...
            let v = self.top(variables);
            let sl = self.cofactor(selector, v, false);
            let sh = self.cofactor(selector, v, true);
            ...
            let out_low = self.ite(sl, hl, ll);
            let out_high = self.ite(sh, hh, lh);
            self.branch(v, out_low, out_high)
        };
        self.conditionals.insert((selector, high, low), out);
        out
    }
    /// Read-only graph census. Complements share records; caches are not roots.
    /// Temporary traversal storage is deliberately excluded from retained bytes.
    pub fn reachable(&self, roots: &[Ref]) -> (usize, usize, usize) {
        let mut seen = FxHashSet::default();
        let mut todo = roots.to_vec();
        let (mut tables, mut words) = (0, 0);
        while let Some(r) = todo.pop() {
            ...
            match self.view(r) {
                View::Constant(_) => {}
                View::Table { words: data, .. } => {
                    tables += 1;
                    words += data.len();
                }
                View::Branch { low, high, .. } => {
                    todo.push(low);
                    todo.push(high);
                }
            }
        }
        (seen.len(), tables, words)
    }
...
mod tests {
    ...
    fn slabs_collide_grow_clone_and_preserve_raw_ids() {
        ...
        assert!(
            trial.nodes() > 100,
            "exercise slab growth and a long collision chain"
        );
        ...
        assert_eq!(
            (a.records, a.tables, a.logical_words),
            (b.records, b.tables, b.logical_words)
        );
        assert!(
            b.table_bytes < a.table_bytes,
            "one resident copy of table words"
        );
    }
