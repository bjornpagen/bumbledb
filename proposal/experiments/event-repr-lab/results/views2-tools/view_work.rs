...
fn run<const K: u32>(width: u32, layout: &str, fused: bool) {
    ...
    let mut sum = [0usize; 10];
    let mut max = [0usize; 10];
    for &(a,b) in &pairs {
        ...
        for i in 0..10 { sum[i] += values[i]; max[i] = max[i].max(values[i]); }
...
fn counters(s: ViewProductStatistics) -> [usize; 10] {
    [s.subproblems, s.memo_hits, s.local_cubes, s.borrowed_planes,
     ...
     s.branch_assignments, s.branch_word_merges]
}
fn main() {
