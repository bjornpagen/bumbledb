...
for name, digest in json.loads((LAB/'results/scratch-reading.json').read_text())['files'].items():
    assert hashlib.sha256((ROOT/name).read_bytes()).hexdigest() == digest, name
for reading in ['view-reading.json', 'view2-reading.json']:
...
for prefix, source_dir, checker in [
    ...
    ('view2', 'results/views2-src', 'view_check.py'),
]:
...
assert (LAB/'results'/view_sanity['log']).is_file()
...
for revision, expected_count in [('lean-v1',17), ('lean-v2',24), ('lean-v3',29), ('lean-v4',36), ('lean-v5',41)]:
...
assert sum(len(record['checked_axioms']) for record in lean_check['files']) == 44
...
for record_name, build_name, checker_name, collector in [
    ...
    rows, processes, binary, pairs, missing=collector(report['inputs'])
    assert report['cases']==len(rows) and report['matched_cases']==len(pairs)
    assert report['pairs']==pairs and report['processes']==processes and report['missing_baselines']==missing
...
assemblies = {}
for tag in ['relations', 'tail', 'dependency', 'transfer-words', 'essential', 'essential-derived', 'classify', 'slab', 'slab-storage', 'words', 'scratch', 'views-v1', 'views-v2']:
