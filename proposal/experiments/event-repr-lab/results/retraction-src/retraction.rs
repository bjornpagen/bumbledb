...
#[derive(Clone)]
pub struct Retraction<const K: u32> {
    raw: Arena<K>,
    order: Vec<u32>,
    n: usize,
    support: Ref,
...
impl<const K: u32> Retraction<K> {
    ...
    fn complete_fibres(&self, mask: u64) -> bool {
        ...
        (0..3).all(|k| {
            matches!((mask >> (k * width)) & face, 0) || ((mask >> (k * width)) & face) == face
        })
    }
...
impl<const K: u32> RegionOps for Retraction<K> {
    ...
    fn count(&self, a: Id) -> u64 {
        match a {
            0 => 0,
            1 => self.population,
            _ => super::contraction::count(&self.raw, &self.order, self.support, a as Ref),
        }
    }
...
impl<const K: u32> Carrier for Retraction<K> {
        ...
        for (e, &selection) in selections.iter().enumerate() {
            ...
            for face in 0..3 {
                ...
                for bit in 0..width {
                    ...
                    let repaired = if anchor >> bit & 1 == 0 {
