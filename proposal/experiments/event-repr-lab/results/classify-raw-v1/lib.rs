...
pub fn classify<const K: u32>(
    ...
    fn visit<const K: u32>(
        ...
        let signature = if variables.count_ones() <= K {
            // Bounded scalar local cube: no resident cofactor or Venn nodes.
            // Word-aligned contraction is a later matched algorithm, not assumed.
    ...
    result.signature = visit(
        arena,
        order,
        [support, a, b].map(Restricted::new),
        &mut memo,
        &mut result,
    );
...
mod tests {
    ...
    fn exhaustive<const K: u32>() {
        ...
        for order in [vec![0, 1], vec![1, 0]] {
            ...
            for support in 1..16usize {
                for a in 0..16usize {
                    for b in 0..16usize {
                        ...
                        let sig =
                            classify(&c, &order, roots[support], roots[a], roots[b]).signature;
                        assert_eq!(sig, expected);
                        seen |= 1 << sig;
                        let swapped = (sig & 9) | ((sig & 2) << 1) | ((sig & 4) >> 1);
                        assert_eq!(
                            classify(&c, &order, roots[support], roots[b], roots[a]).signature,
                            swapped
                        );
                        assert_eq!(
                            classify(&c, &order, roots[support], roots[a] ^ 1, roots[b]).signature,
                            ((sig << 2) | (sig >> 2)) & 15
                        );
                        assert_eq!(
                            classify(&c, &order, roots[support], roots[a], roots[b] ^ 1).signature,
                            ((sig & 5) << 1) | ((sig & 10) >> 1)
                        );
                    }
                }
            ...
            assert_eq!(
                classify(&c, &order, roots[1], roots[1], roots[1]).signature,
                8
            );
        }
        assert_eq!(
    ...
    fn scattered<const K: u32>() {
        ...
        for order in [(0..60).collect::<Vec<_>>(), (0..60).rev().collect()] {
            ...
            let result = classify(&c, &order, roots[0], roots[1], roots[2]);
