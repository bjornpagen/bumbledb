...
use event_unary_prototype::{Arena,Mode,Kernel,Ref};
use std::io::Write;
...
fn prepare_inputs(c:&mut Arena,roots:Vec<Ref>,compact:bool)->Vec<Ref> {
    println!("UNARY_PHASE {{\"phase\":\"inputs_constructed\",\"records\":{},\"bytes\":{}}}",c.nodes(),c.bytes());
...
fn main() {
    ...
    let kernel_mode=match kernel.as_str() {"steps"=>Kernel::Steps,"blocks"=>Kernel::Blocks,_=>panic!("kernel")};
    let compact_input=match policy.as_str() {"keep"=>false,"compact"=>true,_=>panic!("input policy")};
    let control=match mode.as_str() {"plain"=>Mode::Plain,"chains"=>Mode::Chains,_=>panic!("mode")};
    ...
    let (inputs,outputs,before,checksum,checked)=if shape=="coup" {
        ...
        println!("UNARY_PHASE {{\"phase\":\"inputs_verified\",\"records\":{},\"bytes\":{}}}",before.0,before.2);
        ...
        for g in 0..64 {
            if g%8==0 {
                println!("UNARY_PHASE {{\"phase\":\"group\",\"completed_groups\":{g},\"records\":{},\"bytes\":{}}}",c.nodes(),c.bytes());
                std::io::stdout().flush().unwrap();
            }
    ...
    println!("UNARY_SHAPE {{\"passed\":true,\"shape\":\"{shape}\",\"mode\":\"{mode}\",\"layout\":\"{layout}\",\"input_policy\":\"{policy}\",\"kernel\":\"{kernel}\",\"dimensions\":{n},\"inputs\":{},\"outputs\":{},\"input_records\":{},\"input_reachable\":{},\"input_bytes\":{},\"final_records\":{},\"output_reachable\":{},\"live_union\":{live_union},\"final_bytes\":{},\"query_apply_misses\":{},\"query_cofactor_misses\":{},\"query_projection_misses\":{},\"checked_world_answers\":{checked},\"checksum\":{checksum},\"compacted_records\":{compacted_records},\"compacted_bytes\":{compacted_bytes}}}",inputs.len(),outputs.len(),before.0,before.1,before.2,after.0,after.1,after.2,after.3-before.3,after.4-before.4,after.5-before.5);
}
