"""Matched original-support versus product-fibre counting; serial native jobs."""
...
ap.add_argument('--candidate',nargs='+',choices=['prefix64','prefix512','retraction64','retraction512','essential64','essential512','packed512','dense-dispatched'],default=['prefix512','retraction512','essential512','packed512','dense-dispatched'])
...
ap.add_argument('--factor',nargs='+',choices=['joint','faces'],default=['joint','faces'])
...
common=dict(EVENT_LAB_RETRACTION_FACTOR='faces',EVENT_LAB_RETRACTION_COUNT='words',EVENT_LAB_ESSENTIAL_KERNEL='derived',EVENT_LAB_OCCUPANCY_KERNEL='words',EVENT_LAB_VIEW_KERNEL='words',EVENT_LAB_PACKED_MAP='local',EVENT_LAB_TRANSFER_IMPORT='words',EVENT_LAB_IDENTITY='native')
def modes(selected):
...
for candidate in args.candidate:
 for storage in args.storage if candidate.startswith(('essential','retraction','prefix')) else ['enum']:
  for choice in modes(args.mode if candidate.startswith(('essential','retraction','prefix')) else ['materialized']):
   for layout in args.layout:
    for domain in args.domain:
     for count in args.count_kernel if candidate.startswith(('retraction','prefix')) else ['words']:
      ...
      job['EVENT_LAB_RETRACTION_COUNT']=count
      for factor in args.factor if candidate.startswith(('prefix','retraction')) else ['joint']:
       jobs.append(dict(job,EVENT_LAB_RETRACTION_FACTOR=factor))
