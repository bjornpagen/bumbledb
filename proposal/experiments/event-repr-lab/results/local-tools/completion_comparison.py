"""Compare normalization policies and controls for identical readout masks."""
...
KEYS = ['domain', 'width', 'bits', 'readout_family', 'readout_faces', 'layout', 'memo',
        'candidate', 'essential_layout', 'retraction_count', 'retraction_factor', 'retraction_normalize']
FIELDS = [('domain', 'EVENT_LAB_LEGAL_DOMAIN'), ('readout_family', 'EVENT_LAB_READOUT_FAMILY'),
          ...
          ('retraction_normalize', 'EVENT_LAB_RETRACTION_NORMALIZE')]
...
def collect(inputs):
        ...
        for run in data['runs']:
            ...
            for r in run['rows']:
                if r.get('kind') != 'readout_free_join':
                    continue
                assert r.get('retraction_project','joint') == 'joint', 'Use projection_comparison.py for changed support gates'
                ...
                if not r['candidate'].startswith(('prefix','retraction')):
                    assert r['retraction_normalize'] == 'staged'
    ...
    for key, (r, source) in rows.items():
        ...
        if candidate.startswith(('prefix', 'retraction')):
            ...
            choices.append(('anchor', dict(candidate='essential'+suffix, retraction_factor='joint', retraction_normalize='staged')))
        ...
        if candidate.startswith(('prefix', 'retraction', 'essential')):
            ...
            for control in ['packed512', 'dense-dispatched']:
                choices.append(('control', dict(candidate=control, essential_layout='enum', retraction_factor='joint', retraction_normalize='staged')))
...
def main():
    ...
    parser.add_argument('--output', default='COMPLETION-MEASUREMENTS.md')
    parser.add_argument('--record', default='completion-comparison.json')
    ...
    lines = ['# Same-binary completion normalization after Free Join', '',
    ...
    if incomplete:
        ...
        for p in incomplete:
            ...
            cells = [j.get('EVENT_LAB_CANDIDATE', 'verification'),
                     j.get('EVENT_LAB_RETRACTION_NORMALIZE', ''),
    ...
    for group in sorted({key[:7] for key in rows}):
        ...
        for key, (r, _) in sorted(rows.items()):
            if key[:7] != group:
                continue
            values = [r['candidate']+'/'+r['essential_layout']+'/'+r['retraction_normalize'], r['retraction_factor']]
