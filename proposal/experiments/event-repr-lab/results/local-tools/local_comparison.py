"""Compare independent completion locality and source-traversal strategies."""
...
KEYS = ['domain', 'width', 'bits', 'readout_family', 'readout_faces', 'layout', 'memo',
        'candidate', 'essential_layout', 'retraction_count', 'retraction_factor', 'retraction_normalize', 'retraction_project', 'retraction_complete']
FIELDS = [('domain', 'EVENT_LAB_LEGAL_DOMAIN'), ('readout_family', 'EVENT_LAB_READOUT_FAMILY'),
          ...
          ('retraction_normalize', 'EVENT_LAB_RETRACTION_NORMALIZE'), ('retraction_project', 'EVENT_LAB_RETRACTION_PROJECT'), ('retraction_complete', 'EVENT_LAB_RETRACTION_COMPLETE')]
...
def collect(inputs):
        ...
        for run in data['runs']:
            ...
            for r in run['rows']:
                ...
                if not r['candidate'].startswith(('prefix','retraction')):
                    assert r['retraction_normalize'] == 'staged' and r['retraction_project'] == 'joint' and r['retraction_complete'] == 'full'
    ...
    for key, (r, source) in rows.items():
        ...
        if candidate.startswith(('prefix', 'retraction')):
            ...
            choices.append(('anchor', dict(candidate='essential'+suffix, retraction_factor='joint', retraction_normalize='staged',retraction_project='joint',retraction_complete='full')))
        ...
        if candidate.startswith(('prefix', 'retraction', 'essential')):
            ...
            for control in ['packed512', 'dense-dispatched']:
                choices.append(('control', dict(candidate=control, essential_layout='enum', retraction_factor='joint', retraction_normalize='staged',retraction_project='joint',retraction_complete='full')))
        ...
        for comparison, changes in choices:
            ...
            if comparison in ['projection','projection-strength','completion','locality','traversal']:
...
def main():
    ...
    parser.add_argument('--output', default='LOCAL-COMPLETION-MEASUREMENTS.md')
    parser.add_argument('--record', default='local-comparison.json')
    ...
    lines = ['# Same-binary local completion after Free Join', '',
    ...
    if incomplete:
        ...
        for p in incomplete:
            ...
            cells = [j.get('EVENT_LAB_CANDIDATE', 'verification'),
                     j.get('EVENT_LAB_RETRACTION_NORMALIZE', '')+'/'+j.get('EVENT_LAB_RETRACTION_PROJECT','')+'/'+j.get('EVENT_LAB_RETRACTION_COMPLETE',''),
    ...
    for group in sorted({key[:7] for key in rows}):
        ...
        for key, (r, _) in sorted(rows.items()):
            if key[:7] != group:
                continue
            values = [r['candidate']+'/'+r['essential_layout']+'/'+r['retraction_normalize']+'/'+r['retraction_project']+'/'+r['retraction_complete'], r['retraction_factor']]
