"""Matched arbitrary-support contraction, serial native jobs."""
...
ap.add_argument('--seed',type=int,default=20261214)
...
ap.add_argument('--support',nargs='+',choices=['full','legal-product','copied-face','asymmetric'],default=['full','legal-product','copied-face','asymmetric'])
...
ap.add_argument('--reuse',nargs='+',choices=['off','bounded'],default=['off'])
...
runs=[]
def save():
 (RESULTS/args.output).write_text(json.dumps(dict(experiment='Exact staged scoped contraction: rotate bits within A faces, cycle B faces, eliminate Y, swap output Y/Z; compare complete grouped Events',build_metadata=metadata,trials=args.trials,timeout_s=args.timeout,seed=args.seed,runs=runs),indent=2)+'\n')
common=dict(EVENT_LAB_ESSENTIAL_KERNEL='derived',EVENT_LAB_OCCUPANCY_KERNEL='words',EVENT_LAB_VIEW_KERNEL='words',EVENT_LAB_PACKED_MAP='local',EVENT_LAB_TRANSFER_IMPORT='words',EVENT_LAB_IDENTITY='native')
def modes(selected):
  ...
  for choice in modes(args.mode if candidate.startswith('essential') else ['materialized']):
   for layout in args.layout:
    for support in args.support:
     jobs.append(dict(common,**choice,EVENT_LAB_CANDIDATE=candidate,EVENT_LAB_LANE='scoped-products',EVENT_LAB_ESSENTIAL_LAYOUT=storage,EVENT_LAB_LAYOUT=layout,EVENT_LAB_SCOPED_SUPPORT=support))
