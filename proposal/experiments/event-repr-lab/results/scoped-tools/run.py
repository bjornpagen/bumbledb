...
CANDIDATES=['essential64','essential512','block64','packed512','packed4096','packed64','packed256','dense','dense-dispatched','sparse','roaring','runs','bdd-root','bdd-pair','mdd4-pair','bdd-anchored']
DIRECT_SIGNATURE=['essential64','essential512','packed64','packed256','packed512','packed4096','dense','dense-dispatched']
...
def execute(binary,job,index,trials,timeout,log_dir):
    ...
    if verify or job.get('EVENT_LAB_CANDIDATE','').startswith('essential'): label+='-essential-'+job.get('EVENT_LAB_ESSENTIAL_KERNEL','words')+'-'+job.get('EVENT_LAB_ESSENTIAL_LAYOUT','enum')
    if verify or job.get('EVENT_LAB_CANDIDATE','').startswith('essential'): label+='-occupancy-'+job.get('EVENT_LAB_OCCUPANCY_KERNEL','scalar')
    ...
    for line in stdout.splitlines():
        if 'EVENT_LAB ' in line:
            ...
            if row.get('kind') in ['modal_free_join','relations_free_join','product_free_join','scoped_product_free_join','law_free_join','transfer_free_join','owned_free_join','elimination']:
            ...
            if row.get('kind') in ['relations_free_join','product_free_join','scoped_product_free_join']:
                expected_mode=job.get('EVENT_LAB_PRODUCT','materialized') if row['candidate'].startswith('essential') else 'materialized'
...
def main():
    ...
    ap.add_argument('--lane',choices=['join','classify','elimination','symbolic','modal','relations','products','scoped-products','laws','transport','owned','symbolic-relations','complement','all'],default='all');ap.add_argument('--trials',type=int,default=7)
    ...
    if not args.verify_only:
        ...
        for lane in ['modal','relations','products','scoped-products','laws','transport','owned','symbolic-relations','complement']:
    ...
    jobs=[dict(job,EVENT_LAB_ESSENTIAL_KERNEL=kernel) for job in jobs for kernel in (essential_modes if job.get('EVENT_LAB_CANDIDATE','').startswith('essential') else [essential_modes[0]])]
    jobs=[dict(job,EVENT_LAB_ESSENTIAL_LAYOUT=storage) for job in jobs for storage in (essential_layouts if job.get('EVENT_LAB_CANDIDATE','').startswith('essential') else [essential_layouts[0]])]
    jobs=[dict(job,EVENT_LAB_OCCUPANCY_KERNEL=mode) for job in jobs for mode in (occupancy_kernels if job.get('EVENT_LAB_CLASSIFY')=='direct' and job.get('EVENT_LAB_CANDIDATE','').startswith('essential') else [occupancy_kernels[0]])]
    jobs=[dict(job,EVENT_LAB_PRODUCT=mode) for job in jobs for mode in (product_modes if job.get('EVENT_LAB_CANDIDATE','').startswith('essential') and job.get('EVENT_LAB_LANE') in ['relations','products','scoped-products','owned','laws','symbolic-relations'] else ['materialized'])]
