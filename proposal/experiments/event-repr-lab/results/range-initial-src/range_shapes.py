...
ap=argparse.ArgumentParser();ap.add_argument('--output',default='range-initial-shapes.json');args=ap.parse_args()
...
binary=Path(meta['target_directory'])/'release/event-range-prototype'
jobs=list(itertools.product(['composition','coup','bilinear','shared-exit'],['plain','ranges'],['face-major','bit-major'],['keep','compact'],['steps','groups']))
...
for i,job in enumerate(jobs):
    ...
    try:
        ...
        output=p.stdout+'\n'+p.stderr
        rows=[json.loads(s.split('RANGE_SHAPE ',1)[1]) for s in p.stdout.splitlines() if s.startswith('RANGE_SHAPE {')]
    ...
    phases=[json.loads(s.split('RANGE_PHASE ',1)[1]) for s in output.splitlines() if s.startswith('RANGE_PHASE {')]
