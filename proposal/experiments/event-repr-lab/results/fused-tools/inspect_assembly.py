...
ap=argparse.ArgumentParser();ap.add_argument('--tag',required=True);ap.add_argument('--scope',choices=['regions','observation','transport','essential','classify','storage','views','scoped','admission','retraction','prefix','factor','ite'],default='regions');args=ap.parse_args()
...
if args.scope == 'retraction':
    selected = [(f'retraction{k}-count-{mode}', ['11contraction5count5visit', f'Km{cut}_Kb{flag}_'])
                for k,cut in [(64,6),(512,9)] for mode,flag in [('scalar',0),('words',1)]]
    selected += [
        ('retraction512-normalize', ['10retraction','10RetractionKm9_E','9normalize']),
        ('retraction512-preserving-product', ['10retraction','10RetractionKm9_E','18preserving_product']),
    ]
if args.scope == 'prefix':
    selected = [
        # The executable emits one normalization routine shared by the policies.
        ('shared512-normalize', ['10retraction','10RetractionKm9_E', '9normalize']),
        ('prefix512-preserving-product', ['10retraction','10RetractionKm9_Kb1_', '18preserving_product']),
        ('minimum512-preserving-product', ['10retraction','10RetractionKm9_E', '18preserving_product']),
        ('shared512-count-words', ['11contraction5count5visit', 'Km9_Kb1_']),
    ]
if args.scope == 'factor':
    # PREFIX does not affect this routine; LLVM emits a shared instantiation.
    selected = [
        ('shared512-observed-count', ['10retraction','10RetractionKm9_E','14observed_count']),
        ('shared512-count-words', ['11contraction5count5visit','Km9_Kb1_']),
    ]
if args.scope == 'ite':
    selected = [('essential-words-select', ['13essential_raw12word_kernels6select'])]
...
for label, parts in selected:
    ...
    instructions=[line.strip() for line in assembly.splitlines() if re.search(r'\b(?:and|bic|orr|eor|bsl|bit|bif)\.16b\b',line)]
    counts=[line.strip() for line in assembly.splitlines() if re.search(r'\b(?:cnt|uaddlv|addv|udot)(?:\.[a-z0-9]+)?\b',line)]
