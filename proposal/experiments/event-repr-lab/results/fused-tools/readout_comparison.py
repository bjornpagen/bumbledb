"""Compare carriers for identical readout masks, retaining every native sample."""
...
KEYS = ['domain', 'width', 'bits', 'readout_family', 'readout_faces', 'layout', 'memo',
        'candidate', 'essential_layout', 'retraction_count', 'retraction_factor']
...
def collect(inputs):
        ...
        for run in data['runs']:
            ...
            for r in run['rows']:
                if r.get('kind') != 'readout_free_join':
                    continue
                assert r.get('retraction_normalize', 'staged') == 'staged', 'Use completion_comparison.py for normalization-policy experiments'
    ...
    for key, (r, source) in rows.items():
        ...
        if candidate.startswith(('prefix', 'retraction')):
            ...
            choices.append(('anchor', dict(candidate='essential'+suffix, retraction_factor='joint')))
        ...
        if candidate.startswith(('prefix', 'retraction', 'essential')):
            ...
            for control in ['packed512', 'dense-dispatched']:
                choices.append(('control', dict(candidate=control, essential_layout='enum', retraction_factor='joint')))
...
def main():
    ...
    parser.add_argument('--output', default='READOUT-MEASUREMENTS.md')
    parser.add_argument('--record', default='readout-comparison.json')
    ...
    lines = ['# Native information readouts after Free Join', '',
    ...
    for group in sorted({key[:7] for key in rows}):
        ...
        for key, (r, _) in sorted(rows.items()):
            if key[:7] != group:
                continue
            values = [r['candidate']+'/'+r['essential_layout'], r['retraction_factor']]
