"""Serial same-binary comparison of projection completion strategies."""
...
parser.add_argument('--completion',nargs='+',choices=['full','needed','local','local-needed'],default=['full','needed','local','local-needed'])
...
def save():
    record = dict(
        experiment='Same-binary full/needed/local/local-needed projection completion; independent gate and conditional controls; actual three-input Free Join and grouped union, followed by original/Possible/Guaranteed/Ambiguous Events; 64 outputs',
...
for storage in args.storage:
    for mode in args.normalize:
        job = dict(common, verify=True, EVENT_LAB_ESSENTIAL_LAYOUT=storage, EVENT_LAB_RETRACTION_NORMALIZE=mode, EVENT_LAB_RETRACTION_PROJECT=args.gates[-1], EVENT_LAB_RETRACTION_COMPLETE=args.completion[-1])
                                ...
                                for gates in args.gates if candidate.startswith(('prefix','retraction')) else ['joint']:
                                    for completion in args.completion if candidate.startswith(('prefix','retraction')) else ['full']:
                                        jobs.append(dict(common, EVENT_LAB_LANE='readouts', EVENT_LAB_CANDIDATE=candidate,
                                                     ...
                                                     EVENT_LAB_RETRACTION_COMPLETE=completion))
