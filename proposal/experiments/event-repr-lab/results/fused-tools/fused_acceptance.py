"""Native ownership/symbolic acceptance for direct completed projection."""
...
ap.add_argument('--execute',nargs='+',choices=['staged','fused'],default=['fused'])
ap.add_argument('--completion',nargs='+',choices=['full','needed','local','local-needed'],default=['local-needed'])
ap.add_argument('--gates',nargs='+',choices=['joint','active','witness'],default=['witness'])
args = ap.parse_args()
...
jobs = [dict(common, EVENT_LAB_CANDIDATE=candidate, EVENT_LAB_LANE=lane,
             EVENT_LAB_PRODUCT=product, EVENT_LAB_RETRACTION_NORMALIZE=mode, EVENT_LAB_RETRACTION_PROJECT=gates, EVENT_LAB_RETRACTION_COMPLETE=completion, EVENT_LAB_RETRACTION_EXECUTE=execution)
        ...
        for mode in args.normalize for gates in args.gates for completion in args.completion for execution in args.execute]
