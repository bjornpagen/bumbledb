    ...
    for domains in &configurations {
        for layout in ["face-major", "bit-major"] {
            for gates in if C::VIEW_PRODUCT {
                ...
                assert!(clone.may(empty, target).is_err());
                assert!(clone.must(empty, target).is_err());
                let clone_target = clone.goal(goal).unwrap();
                assert!(clone.union_goals(clone_target, target).is_err());
                assert!(clone.may(rs[0], clone_target).is_err());
                for (i, r) in matrices.iter().enumerate() {
                    let (may, must) = p.modal(rs[i], target).unwrap();
                    for (result, universal) in [(may, false), (must, true)] {
                        let expected = bits(n, |w| {
                            let (x, e) = (w & 3, w >> 6);
                            domains.contains(w as u64)
                                && if universal {
                                    (0..4).any(|y| r[e][x * 4 + y])
                                        && (0..4).all(|y| !r[e][x * 4 + y] || y & 1 != 0)
                                } else {
                                    (0..4).any(|y| r[e][x * 4 + y] && y & 1 != 0)
                                }
                        });
                        assert_eq!(p.carrier().export(result), expected);
                    }
