...
ap=argparse.ArgumentParser();ap.add_argument('--output',default='build-range-native.json');args=ap.parse_args()
check_engine_sources()
engine=SCRATCH/'range-engine';engine.mkdir(exist_ok=True)
...
def sources():return {str(p.relative_to(LAB)):digest(p) for p in sorted((LAB/'range-native-src').glob('*.rs'))}
...
print('Building isolated range/native comparison.',flush=True)
...
out=dict(binary=binary,binary_sha256=digest(Path(binary)),source_sha256=before,
    ...
    boundary='Same-binary native range controls and baseline carriers. Scoped/anchored adapter; raw screen used completed Coup inputs. Timing requires separate serial runs.')
(LAB/'results'/args.output).write_text(json.dumps(out,indent=2)+'\n')
print('Native range comparison built.',flush=True)
