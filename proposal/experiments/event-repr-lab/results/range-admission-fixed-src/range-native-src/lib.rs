...
macro_rules! dispatch {($name:expr,$f:ident $(,$arg:expr)*)=>{match $name {
    ...
    "essential64"=>$f::<Essential<6>>($($arg),*),"essential512"=>$f::<Essential<9>>($($arg),*),
    "prefix64"=>$f::<Retraction<6,true>>($($arg),*),"prefix512"=>$f::<Retraction<9,true>>($($arg),*),
    "retraction64"=>$f::<Retraction<6>>($($arg),*),"retraction512"=>$f::<Retraction<9>>($($arg),*),
