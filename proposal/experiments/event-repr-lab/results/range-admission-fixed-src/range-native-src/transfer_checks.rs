...
pub(super) fn source<C: Carrier>(fixtures: &mut Vec<Fixture>) {
    for nbits in [3, 8] {
        let n = 1usize << nbits;
...
pub fn fixtures() -> Vec<Fixture> {
    ...
    add!(Finite<Dense>,Finite<Dense<true>>,Finite<Sparse>,Finite<Roaring>,Finite<Runs>,Diagram<2,false>,Diagram<2,true>,Diagram<4,true>,Anchored,Packed<1>,Packed<4>,Packed<8>,Packed<64>,Block64,Essential<6>,Essential<9>);
    f
}
pub(super) fn target<T: Carrier>(fixtures: &[Fixture]) {
    for f in fixtures {
...
pub(super) fn high_source<C: Carrier>(packets: &mut Vec<Packet>) {
...
pub(super) fn high_target<C: Carrier>(packets: &[Packet]) {
    for p in packets {
...
pub(super) fn table_modes<C: Carrier>() {
    let mut cases = 0;
...
pub(super) fn compute_owner<C: Carrier>() {
...
pub fn all() {
    ...
    targets!(Finite<Dense>,Finite<Dense<true>>,Finite<Sparse>,Finite<Roaring>,Finite<Runs>,Diagram<2,false>,Diagram<2,true>,Diagram<4,true>,Anchored,Packed<1>,Packed<4>,Packed<8>,Packed<64>,Block64,Essential<6>,Essential<9>);
    maps();
    wide_tables::<1>();
    wide_tables::<4>();
    wide_tables::<8>();
    ...
    sources!(Diagram<2,false>,Diagram<2,true>,Diagram<4,true>,Anchored,Packed<1>,Packed<4>,Packed<8>,Packed<64>,Block64,Essential<6>,Essential<9>);
    macro_rules! high {($($t:ty),*)=>{$(high_target::<$t>(&packets);)*};}
    high!(Diagram<2,false>,Diagram<2,true>,Diagram<4,true>,Anchored,Packed<1>,Packed<4>,Packed<8>,Packed<64>,Block64,Essential<6>,Essential<9>);
    println!(
        "EVENT_LAB {{\"kind\":\"transfer_symbolic\",\"bits\":40,\"source_target_pairs\":121,\"packet_bytes\":{:?},\"passed\":true}}",
...
fn retractions() {
    ...
    let mut f = fixtures();
    source::<Retraction<6>>(&mut f);
    source::<Retraction<9>>(&mut f);
    target::<Retraction<6>>(&f);
    target::<Retraction<9>>(&f);
    target::<Essential<9>>(&f);
    table_modes::<Retraction<6>>();
    table_modes::<Retraction<9>>();
    compute_owner::<Retraction<6>>();
    compute_owner::<Retraction<9>>();
    let mut packets = vec![];
    high_source::<Retraction<6>>(&mut packets);
    high_source::<Retraction<9>>(&mut packets);
    high_source::<Essential<9>>(&mut packets);
    high_target::<Retraction<6>>(&packets);
    high_target::<Retraction<9>>(&packets);
    high_target::<Essential<9>>(&packets);
    println!(
        "EVENT_LAB {{\"kind\":\"retraction_transfer_verification\",\"symbolic_pairs\":9,\"passed\":true}}"
    );
}
...
fn prefixes() {
    use super::retraction::Retraction;
    let mut f = fixtures();
    source::<Retraction<6, true>>(&mut f);
    source::<Retraction<9, true>>(&mut f);
    target::<Retraction<6, true>>(&f);
    target::<Retraction<9, true>>(&f);
    target::<Retraction<9>>(&f);
    table_modes::<Retraction<6, true>>();
    table_modes::<Retraction<9, true>>();
    compute_owner::<Retraction<6, true>>();
    compute_owner::<Retraction<9, true>>();
    let mut packets = vec![];
    high_source::<Retraction<6, true>>(&mut packets);
    high_source::<Retraction<9, true>>(&mut packets);
    high_source::<Retraction<9>>(&mut packets);
    high_target::<Retraction<6, true>>(&packets);
    high_target::<Retraction<9, true>>(&packets);
    high_target::<Retraction<9>>(&packets);
    println!(
        "EVENT_LAB {{\"kind\":\"prefix_transfer_verification\",\"symbolic_pairs\":9,\"passed\":true}}"
    );
}
