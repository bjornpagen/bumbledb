...
ap=argparse.ArgumentParser();ap.add_argument('--release',action='store_true');ap.add_argument('--product',choices=['materialized','views','views-inputs'],default='materialized');ap.add_argument('--essential-kernel',choices=['words','scalar','derived'],default='words');ap.add_argument('--occupancy-kernel',choices=['scalar','words','scratch'],default='scalar');ap.add_argument('--essential-layout',choices=['enum','slab'],default='enum');ap.add_argument('--output',default='range-admission-check.json');ap.add_argument('--packed-map',choices=['local','recursive'],default='local');ap.add_argument('--transfer-import',choices=['words','direct','recursive'],default='words');ap.add_argument('--view-kernel',choices=['assignments','words'],default='assignments');ap.add_argument('--view-reuse',choices=['off','bounded'],default='off');ap.add_argument('--view-normalize',choices=['deferred','source'],default='deferred');ap.add_argument('--retraction-normalize',choices=['staged','equal','ite'],default='staged');ap.add_argument('--retraction-project',choices=['joint','active','witness'],default='joint');ap.add_argument('--retraction-complete',choices=['full','needed','local','local-needed'],default='full');ap.add_argument('--retraction-execute',choices=['staged','fused'],default='staged');args=ap.parse_args()
folder=SCRATCH/'range-checker';(folder/'src').mkdir(parents=True,exist_ok=True)
(folder/'Cargo.toml').write_text('''[package]
name = "event-range-checker"
version = "0.0.0"
edition = "2024"
[workspace]
[dependencies]
...
names += ['range_raw','range_carrier','range_checks']
names += ['contraction', 'retraction', 'retraction_checks', 'ite_checks', 'projection_checks', 'local_checks', 'factor_checks', 'readout']
if (LAB/'src/blocks.rs').exists(): names.append('blocks')
code='#![allow(dead_code)]\n'+''.join('#[path = '+json.dumps(str(LAB/'range-native-src'/(name+'.rs')))+']\nmod '+name+';\n' for name in names)+'\n#[test]\nfn shared_semantics() { range_checks::all(); }\n'
(folder/'src/lib.rs').write_text(code)
digests={str(p.relative_to(LAB)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted((LAB/'range-native-src').glob('*.rs'))}
...
env['EVENT_LAB_RETRACTION_PROJECT']=args.retraction_project
env['EVENT_LAB_RETRACTION_COMPLETE']=args.retraction_complete
env['EVENT_LAB_RETRACTION_EXECUTE']=args.retraction_execute
...
assert digests=={str(p.relative_to(LAB)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted((LAB/'range-native-src').glob('*.rs'))},'Sources changed during check'
log=Path(args.output).stem+'.log';(LAB/'results'/log).write_text(p.stdout+'\n'+p.stderr)
result={'passed':p.returncode==0,'elapsed_s':time.time()-start,'profile':('release' if args.release else 'debug')+'; not a performance measurement','lab_sources':digests,'shared_modules':names,'packed_map':args.packed_map,'transfer_import':args.transfer_import,'essential_kernel':args.essential_kernel,'essential_layout':args.essential_layout,'occupancy_kernel':args.occupancy_kernel,'product_mode':args.product,'view_kernel':args.view_kernel,'view_reuse':args.view_reuse,'view_normalize':args.view_normalize,'log':log,'retraction_count':'words','retraction_factor':'faces','retraction_normalize':args.retraction_normalize,'retraction_project':args.retraction_project,'retraction_complete':args.retraction_complete,'retraction_execute':args.retraction_execute}
