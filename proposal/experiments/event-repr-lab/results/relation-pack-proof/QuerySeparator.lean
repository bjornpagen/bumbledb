...
theorem patch_right {V D : Type} (left right : V → Bool) (keys : V → Prop)
  ...
  intro v hr
  cases h : left v with
  | false => unfold patch; rw [h]
  | true =>
    have hk := overlap v h hr
    unfold patch; rw [h]; exact (ha v hk).trans (hb v hk).symm
...
theorem patch_keys {V D : Type} (left : V → Bool) (keys : V → Prop)
  ...
  intro v hk
  cases h : left v with
  | false => unfold patch; rw [h]; exact hb v hk
  | true => unfold patch; rw [h]; exact ha v hk
