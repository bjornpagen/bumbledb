...
impl Store {
    pub fn new(layout: &str) -> Self {
        match layout {
            "enum" => Self::Enum {
                nodes: vec![Node { variables: 0, body: Body::Constant }],
                unique: FxHashMap::default(),
            },
            "slab" => Self::Slab(Slab {
                records: vec![Record { variables: 0, payload: 0 }],
                words: vec![],
                links: vec![END],
    ...
    pub fn layout(&self) -> &'static str {
        match self { Self::Enum { .. } => "enum", Self::Slab(_) => "slab" }
    }
    ...
    pub fn node(&self, r: Ref) -> BorrowedNode<'_> {
        match self {
            Self::Enum { nodes, .. } => {
                ...
                BorrowedNode {
                    ...
                    body: match &n.body {
                        ...
                        Body::Branch { variable, low, high } => BorrowedBody::Branch {
            ...
            Self::Slab(s) => {
                ...
                BorrowedNode { variables: n.variables, body }
            }
        }
    }
    pub fn owned(&self, r: Ref) -> Node {
        let n = self.node(r);
        Node { variables: n.variables, body: match n.body {
            BorrowedBody::Constant => Body::Constant,
            BorrowedBody::Table(words) => Body::Table(words.to_vec()),
            BorrowedBody::Branch { variable, low, high } => Body::Branch { variable, low, high },
        }}
    }
    fn equal(&self, r: Ref, node: &Node) -> bool {
        let old = self.node(r);
        old.variables == node.variables && match (old.body, &node.body) {
            (BorrowedBody::Constant, Body::Constant) => true,
            (BorrowedBody::Table(a), Body::Table(b)) => a == b,
            (BorrowedBody::Branch { variable: av, low: al, high: ah },
             Body::Branch { variable: bv, low: bl, high: bh }) => av == *bv && al == *bl && ah == *bh,
            _ => false,
        }
    }
    ...
    pub fn find(&self, node: &Node) -> Option<Ref> {
        match self {
            ...
            Self::Slab(s) => {
                ...
                while at != END {
                    if self.equal(at, node) { return Some(at); }
            ...
            Self::Slab(s) => {
                ...
                let payload = match &node.body {
                    ...
                    Body::Table(data) => {
                        ...
                        assert_eq!(data.len(), (1usize << node.variables.count_ones()).div_ceil(64));
                        let offset = s.words.len();
                        assert!(offset.checked_add(data.len()).is_some_and(|n| n <= u32::MAX as usize),
                            "RESOURCE_CAP: essential table slab");
                        s.words.extend_from_slice(data);
                        TABLE | offset as u64
                    }
                    Body::Branch { variable, low, high } => {
                        assert!(*variable < 63 && (*low as u64) <= CHILD_MASK && (*high as u64) <= CHILD_MASK);
                        assert!(*low / 2 < r / 2 && *high / 2 < r / 2);
                        (*variable as u64) << 56 | (*high as u64) << 28 | *low as u64
                    }
                };
                let prior = s.index.insert(hash, r).unwrap_or(END);
                s.records.push(Record { variables: node.variables, payload });
                s.links.push(prior);
            }
        }
        r
    ...
    pub fn statistics(&self) -> Statistics {
        let mut out = Statistics { records: self.len(), ..Statistics::default() };
        for i in 0..self.len() {
        ...
        match self {
            Self::Enum { nodes, unique } => {
                ...
                out.table_bytes = nodes.iter().chain(unique.keys()).map(|n| match &n.body {
    ...
    pub fn collide_all(&mut self) {
        let Self::Slab(s) = self else { panic!("collision test requires slab store") };
