...
impl MemoryStats {
    pub fn bytes(self) -> usize {
        self.record_bytes + self.table_bytes + self.interner_bytes
            + self.metadata_bytes + self.cache_bytes
    }
}
pub fn memory_json(stats: Option<MemoryStats>) -> String {
    let Some(s) = stats else { return "null".into(); };
    format!(
        "{{\"layout\":\"{}\",\"record_bytes\":{},\"table_bytes\":{},\"interner_bytes\":{},\"metadata_bytes\":{},\"cache_bytes\":{},\"records\":{},\"tables\":{},\"logical_words\":{},\"total_bytes_est\":{}}}",
        s.layout, s.record_bytes, s.table_bytes, s.interner_bytes,
        s.metadata_bytes, s.cache_bytes, s.records, s.tables, s.logical_words, s.bytes()
    )
}
