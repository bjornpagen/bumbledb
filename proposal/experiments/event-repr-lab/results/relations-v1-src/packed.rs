...
impl<const W: usize> Packed<W> {
    ...
    fn permute_rec(&mut self, r: u32, literals: &[u32], memo: &mut FxHashMap<u32, u32>) -> u32 {
        ...
        let out = if r & 2 == 0 {
            ...
            let node = self.branches[(r >> 2) as usize];
            let low = self.permute_rec(node.low, literals, memo);
            let high = self.permute_rec(node.high, literals, memo);
            self.select(
...
impl<const W: usize> Carrier for Packed<W> {
    const NAME: &'static str = if W == 1 { "packed64" } else { "packed256" };
    ...
    fn permute(&mut self, a: Id, map: &Permutation) -> Id {
        ...
        let literals: Vec<_> = map
            ...
            .collect();
        let raw = self.permute_rec(root, &literals, &mut FxHashMap::default());
        self.seal(raw)
    }
