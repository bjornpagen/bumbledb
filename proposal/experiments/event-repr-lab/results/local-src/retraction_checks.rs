...
fn finite<const K: u32, const PREFIX: bool>() {
    let mut cases = 0;
    let mut normalization_inputs = 0;
    for domains in [
        vec![Domain::Below(3)],
        ...
        for layout in ["bit-major", "face-major"] {
            ...
            for seed in 0..16 {
                ...
                c.check_alternate_normalization(&data, event);
                normalization_inputs += 1;
    ...
    println!(
        "EVENT_LAB {{\"kind\":\"retraction_verification\",\"candidate\":\"{}\",\"projection_cases\":{},\"normalization_inputs\":{},\"passed\":true}}",
