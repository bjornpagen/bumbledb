...
pub struct Dense<const DISPATCHED: bool = false>(Vec<u64>);
impl<const DISPATCHED: bool> Set for Dense<DISPATCHED> {
    const DIRECT_SIGNATURE: bool = true;
    fn dense_words(&self) -> Option<&[u64]> {
        Some(&self.0)
    }
    const NAME: &'static str = if DISPATCHED {
        "dense-dispatched"
    } else {
        "dense"
    };
    ...
    fn raw_op(op: u8, a: &Self, b: &Self) -> Self {
        if DISPATCHED {
            #[inline]
            fn words<const OP: u8>(a: &[u64], b: &[u64]) -> Vec<u64> {
            ...
            return Self(match op {
                ...
                12 => words::<12>(&a.0, &b.0),
                14 => words::<14>(&a.0, &b.0),
                _ => unreachable!("finite raw operations are zero-preserving"),
            });
        }
        Self(
            a.0.iter()
...
impl<B: Set> RegionOps for Finite<B> {
    const DIRECT_SIGNATURE: bool = B::DIRECT_SIGNATURE;
    fn direct_signature(&self, a: Id, b: Id) -> Option<u8> {
        let left = self.sets[(a >> 1) as usize].dense_words()?;
        let right = self.sets[(b >> 1) as usize].dense_words()?;
        let raw = super::signature::word_occupancy(&self.support_bits, left, right);
    ...
    fn diagonal(&mut self, pairs: &[(u32, u32)]) -> Result<Id, &'static str> {
        validate_diagonal(self.dimensions(), pairs)?;
        Ok(self.import(&bits(self.n, |w| {
    ...
    fn variable(&mut self, coordinate: u32) -> Id {
        ...
        self.import(&bits(self.n, |w| w >> coordinate & 1 != 0))
    }
    ...
    fn spectrum(
        ...
        let set = &self.sets[(event / 2) as usize];
        let mut buffer = Vec::new();
        let words = if let Some(words) = set.dense_words() {
            words
        } else {
            buffer.resize(self.support_bits.len(), 0);
            set.write(&mut buffer);
            &buffer
        };
        let coords: Vec<_> = (0..plan.group_of.len() as u32).collect();
        plan.table(&coords).contract(
            plan,
            words,
            (event & 1 != 0).then_some(self.support_bits.as_slice()),
        )
    }
    ...
    fn import_table(&mut self, coordinates: &[u32], words: &[u64]) -> Option<Id> {
        if let Some(id) = import_full_table(self, coordinates, words) {
            return Some(id);
        }
        let mapped = bits(self.n, |w| {
            let index = coordinates
                .iter()
                .enumerate()
                .fold(0, |a, (i, &k)| a | (((w >> k) & 1) << i));
            at(words, index)
        });
        Some(self.import(&mapped))
    }
    #[inline]
