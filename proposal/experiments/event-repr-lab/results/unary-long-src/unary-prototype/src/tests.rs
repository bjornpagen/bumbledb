...
fn all_small_functions() {
    ...
    for order in orders {for mode in [Mode::Plain,Mode::Chains] {
        let mut c=Arena::new(order.to_vec(),mode);
    ...
    assert_eq!((binary,projection,maps,completed),(12582912,24576,89088,24576));
    println!("UNARY_CHECK {{\"passed\":true,\"binary\":{binary},\"projection\":{projection},\"maps\":{maps},\"completed_projection\":{completed},\"orders\":6,\"modes\":2}}");
}
...
fn long_chains_and_collection() {
    let mut checked=0;
    for n in [9usize,10,11,12,20,21,22,61] {for reverse in [false,true] {for mode in [Mode::Plain,Mode::Chains] {
        let mut order:Vec<_>=(0..n as u32).collect();if reverse {order.reverse();}
        let mut c=Arena::new(order.clone(),mode);
        ...
        if n==61 {
            println!("UNARY_LONG_SHAPE {{\"passed\":true,\"dimensions\":{n},\"reverse\":{reverse},\"mode\":\"{mode:?}\",\"constructed_records\":{},\"compacted_records\":{nodes},\"compacted_bytes\":{}}}",c.nodes(),compact.bytes());
        }
