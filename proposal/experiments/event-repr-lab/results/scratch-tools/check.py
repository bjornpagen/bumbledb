...
ap=argparse.ArgumentParser();ap.add_argument('--essential-kernel',choices=['words','scalar','derived'],default='words');ap.add_argument('--occupancy-kernel',choices=['scalar','words','scratch'],default='scalar');ap.add_argument('--essential-layout',choices=['enum','slab'],default='enum');ap.add_argument('--output',default='fast-check.json');ap.add_argument('--packed-map',choices=['local','recursive'],default='local');ap.add_argument('--transfer-import',choices=['words','direct','recursive'],default='words');args=ap.parse_args()
...
result={'passed':p.returncode==0,'elapsed_s':time.time()-start,'profile':'debug; not a performance measurement','lab_sources':digests,'shared_modules':names,'packed_map':args.packed_map,'transfer_import':args.transfer_import,'essential_kernel':args.essential_kernel,'essential_layout':args.essential_layout,'occupancy_kernel':args.occupancy_kernel,'log':log}
