...
ap=argparse.ArgumentParser();ap.add_argument('--tag',required=True);ap.add_argument('--scope',choices=['regions','observation','transport','essential','classify','storage','views','scoped','admission','retraction'],default='regions');args=ap.parse_args()
...
if args.scope == 'retraction':
    ...
    selected += [
        ('retraction512-normalize', ['10retraction','10RetractionKm9_','9normalize']),
        ('retraction512-preserving-product', ['10retraction','10RetractionKm9_','18preserving_product']),
    ]
