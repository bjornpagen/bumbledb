...
SCENARIOS=['random_4096','overlap_4096','structured_65536','sparse_65536','runs_65536','coup_4290','coup_product_65536']
RESULTS=LAB/'results'
...
def main():
    ...
    verification_jobs=[(mode,kernel,storage) for mode in transfer_modes for kernel in essential_modes for storage in essential_layouts]
    records=[execute(binary,{'verify':True,'EVENT_LAB_PACKED_MAP':args.packed_map,'EVENT_LAB_TRANSFER_IMPORT':mode,'EVENT_LAB_ESSENTIAL_KERNEL':kernel,'EVENT_LAB_ESSENTIAL_LAYOUT':storage,'EVENT_LAB_IDENTITY':args.identity if args.identity!='all' else 'native'},i,args.trials,args.timeout,log_dir) for i,(mode,kernel,storage) in enumerate(verification_jobs)]
