"""Serial same-binary comparison of staged and equal-cofactor completion."""
...
parser.add_argument('--normalize', nargs='+', choices=['staged', 'equal', 'ite'], default=['staged', 'equal'])
...
def save():
    record = dict(
        experiment='Same-binary staged/equal-cofactor completion; actual three-input Free Join and grouped union, followed by original/Possible/Guaranteed/Ambiguous Events; 64 outputs',
...
for storage in args.storage:
    for mode in args.normalize:
        job = dict(common, verify=True, EVENT_LAB_ESSENTIAL_LAYOUT=storage, EVENT_LAB_RETRACTION_NORMALIZE=mode)
                        ...
                        for factor in args.factor if candidate.startswith(('prefix', 'retraction')) else ['joint']:
                            for mode in args.normalize if candidate.startswith(('prefix', 'retraction')) else ['staged']:
                                jobs.append(dict(common, EVENT_LAB_LANE='readouts', EVENT_LAB_CANDIDATE=candidate,
                                                 ...
                                                 EVENT_LAB_RETRACTION_NORMALIZE=mode))
