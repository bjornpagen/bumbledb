"""Serial native comparisons of the same complete information-readout query."""
...
def save():
    record = dict(
        experiment='Actual three-input Free Join; grouped union of conjunctions, followed by original/Possible/Guaranteed/Ambiguous Events; 64 outputs',
...
for storage in args.storage:
    job = dict(common, verify=True, EVENT_LAB_ESSENTIAL_LAYOUT=storage)
    result = execute(str(binary), job, len(runs), args.trials, args.timeout, folder)
    runs.append(result)
    save()
    if result['status'] != 'passed':
        raise SystemExit('Readout verification failed: '+result['log'])
