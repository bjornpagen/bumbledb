"""Compare exact count modes using matched rows from one native executable."""
from pathlib import Path
import argparse,hashlib,json,statistics as st
from prepare import LAB
KEYS=['kind','domain','bits','layout','memo','candidate','essential_layout','product_mode','view_kernel','view_normalize','view_reuse','support_gates','retraction_count','retraction_factor']
FIELDS=[('domain','EVENT_LAB_LEGAL_DOMAIN'),('candidate','EVENT_LAB_CANDIDATE'),('layout','EVENT_LAB_LAYOUT'),('essential_layout','EVENT_LAB_ESSENTIAL_LAYOUT'),('product_mode','EVENT_LAB_PRODUCT'),('view_kernel','EVENT_LAB_VIEW_KERNEL'),('view_normalize','EVENT_LAB_VIEW_NORMALIZE'),('view_reuse','EVENT_LAB_VIEW_REUSE'),('retraction_count','EVENT_LAB_RETRACTION_COUNT'),('retraction_factor','EVENT_LAB_RETRACTION_FACTOR')]
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('inputs',nargs='+');ap.add_argument('--output',default='FACTOR-MEASUREMENTS.md');ap.add_argument('--record',default='factor-comparison.json');args=ap.parse_args()
 rows,processes,binary,within,missing=collect(args.inputs)
 pairs=[]
 for key,(r,source) in rows.items():
  if not r['candidate'].startswith(('prefix','retraction')):continue
  changes=[]
  if r['candidate'].startswith('prefix'):
   changes.append(dict(candidate=r['candidate'].replace('prefix','retraction'),retraction_count='words'))
  if r['candidate'].endswith('512'):
   changes.append(dict(candidate=r['candidate'][:-3]+'64'))
  changes.append(dict(candidate='essential'+r['candidate'].removeprefix('prefix').removeprefix('retraction'),retraction_count='words',retraction_factor='joint'))
  if r['retraction_factor']=='faces':changes.append(dict(retraction_factor='joint'))
  if r['retraction_count']=='words':changes.append(dict(retraction_count='scalar'))
  for control in ['packed512','dense-dispatched']:
   changes.append(dict(candidate=control,retraction_count='words',retraction_factor='joint',essential_layout='enum',product_mode='materialized',view_normalize='deferred',view_reuse='off',support_gates='materialized'))
  for change in changes:
   case=dict(zip(KEYS,key));case.update(change);bk=tuple(case[k] for k in KEYS)
   if bk not in rows:continue
   base,other=rows[bk]
   if change in [dict(retraction_count='scalar'),dict(retraction_factor='joint')]:
