"""Recompute checked legal relation programs and paired support-certificate comparisons."""
from pathlib import Path
import argparse,hashlib,json,statistics as st
from prepare import LAB

KEYS=['kind','domain','bits','layout','memo','candidate','essential_layout','product_mode','view_kernel','view_normalize','view_reuse','support_gates']
FIELDS=[('domain','EVENT_LAB_LEGAL_DOMAIN'),('candidate','EVENT_LAB_CANDIDATE'),('layout','EVENT_LAB_LAYOUT'),('essential_layout','EVENT_LAB_ESSENTIAL_LAYOUT'),('product_mode','EVENT_LAB_PRODUCT'),('view_kernel','EVENT_LAB_VIEW_KERNEL'),('view_normalize','EVENT_LAB_VIEW_NORMALIZE'),('view_reuse','EVENT_LAB_VIEW_REUSE')]
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('inputs',nargs='+');ap.add_argument('--output',default='LEGAL-MEASUREMENTS.md');ap.add_argument('--record',default='legal-comparison.json');args=ap.parse_args()
 ...
 lines=['# Checked legal relations through Free Join','',
 ...
 for group in sorted({k[1:5] for k in rows}):
  ...
  lines += [f'## {domain}, {bits} coordinates, {layout}, memo {memo}','',
  ...
  for key,(r,_) in sorted(rows.items()):
   ...
   values=[r['candidate']+'/'+r['essential_layout'],r['product_mode'],r['view_normalize'],r['view_reuse'],r['support_gates'],f'{1000*st.median(r["fresh_s"]):.3f}',f'{1000*min(r["fresh_s"]):.3f}–{1000*max(r["fresh_s"]):.3f}',f'{1000*st.median(r["warm_s"]):.3f}',f'{r["final_bytes_est"]/1000:.1f}',str(r['final_nodes']),str(len(r['fresh_s']))]
