...
def main():
    ...
    jobs=[dict(job,EVENT_LAB_PRODUCT=mode) for job in jobs for mode in (product_modes if job.get('EVENT_LAB_CANDIDATE','').startswith('essential') and job.get('EVENT_LAB_LANE') in ['relations','products','scoped-products','legal-relations','owned','laws','symbolic-relations'] else ['materialized'])]
