...
impl<const W: usize> Packed<W> {
    fn occupancy(
        ...
        a: u32,
        b: u32,
        memo: &mut FxHashMap<(u32, u32, u32), u8>,
    ) -> u8 {
        if support == 0 {
            return 0;
        }
        let key = (support, a, b);
        if let Some(&sig) = memo.get(&key) {
            return sig;
        }
        let level = self.level(support).min(self.level(a)).min(self.level(b));
        let result = if level == self.prefix {
            super::signature::word_occupancy(&self.words(support), &self.words(a), &self.words(b))
        } else {
            let low = self.occupancy(
                self.child(support, level, false),
    ...
    // Input axes already match this manager's physical order. At a terminal,
    // copy complete words; do not gather every truth-table cell independently.
    fn build_physical_words(&mut self, words: &[u64], level: usize) -> u32 {
        if level == self.prefix {
            let mut leaf = [0; W];
            leaf[..words.len()].copy_from_slice(words);
            return self.leaf(leaf);
        }
        let mid = words.len() / 2;
        let low = self.build_physical_words(&words[..mid], level + 1);
        let high = self.build_physical_words(&words[mid..], level + 1);
    ...
    fn transport_raw(
        ...
        let v = if r & 2 == 0 {
            out.table(
                self.tail.clone(),
                self.leaves[(r >> 2) as usize][..self.cells.div_ceil(64)].to_vec(),
            )
        } else {
...
impl<const W: usize> RegionOps for Packed<W> {
    ...
    fn packet(&self, names: &[u64], roots: &[Id]) -> super::transfer::Packet {
        ...
        let roots = roots
            .iter()
            .map(|&r| self.transport_raw((r >> 1) as u32, &mut out, &mut memo) ^ ((r & 1) as u32))
            .collect();
    ...
    fn import_table(&mut self, coordinates: &[u32], words: &[u64]) -> Option<Id> {
        let axes = coordinates
            .iter()
            .map(|k| self.tail.iter().position(|t| t == k))
        ...
        if let Some(axes) = axes {
            ...
            for cell in 0..self.cells {
                let index = axes
                    .iter()
                    .enumerate()
                    .fold(0, |v, (i, &k)| v | (((cell >> k) & 1) << i));
                if at(words, index) {
                    table[cell / 64] |= 1 << (cell % 64);
                }
            }
            let root = self.leaf(table);
            return Some(self.seal(root));
        }
        import_full_table(self, coordinates, words)
    }
    fn import_table_words(&mut self, coordinates: &[u32], words: &[u64]) -> Option<Id> {
        if coordinates.len() == self.order.len() {
            let axes = coordinates
                .iter()
                .map(|k| {
                    self.order
            ...
            Permutation::new(axes).ok()?.dense(&mut mapped);
            let raw = self.build_physical_words(&mapped, 0);
            return Some(self.seal(raw));
        }
        let mut axes = coordinates
            .iter()
            .map(|k| self.tail.iter().position(|c| c == k).map(|i| i as u32))
            .collect::<Option<Vec<_>>>()?;
        // Broadcast omitted axes, then permute the complete local axis roster.
        let mut mapped = [0; W];
        if coordinates.len() < 6 {
            let mut pattern = words[0];
            let mut cells = 1usize << coordinates.len();
            while cells < 64 {
            ...
            for (i, w) in mapped.iter_mut().enumerate() {
                *w = words[i % words.len()];
            }
        }
        for i in 0..self.tail.len() as u32 {
            if !axes.contains(&i) {
                axes.push(i);
            }
        }
        Permutation::new(axes)
            .ok()?
            .dense(&mut mapped[..self.cells.div_ceil(64)]);
        let raw = self.leaf(mapped);
        Some(self.seal(raw))
    }
...
impl<const W: usize> Carrier for Packed<W> {
    fn legal_space(domains: &super::legal::Domains, order: Vec<u32>) -> Result<Self, &'static str> {
        let mut out = Self::symbolic(domains.dimensions(), order);
        let event = domains.formula(&mut out);
        let population = out.count(event);
        assert_eq!(population, domains.population());
        let support = out.true_root(event);
        let anchor = domains.anchor() as usize;
        assert!(out.eval(support, anchor));
        out.support = support;
        out.anchor = anchor;
