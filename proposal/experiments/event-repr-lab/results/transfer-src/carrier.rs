...
pub trait Carrier: Clone {
    ...
    fn full_space(nbits: u32, order: Vec<u32>) -> Result<Self, &'static str> {
        if nbits > 20 { return Err("RESOURCE_CAP: explicit transfer proof space"); }
        let n = 1usize << nbits;
    ...
    fn variable(&mut self, coordinate: u32) -> Id {
        ...
        self.import(&bits(1usize << self.dimensions(), |w| w >> coordinate & 1 != 0))
    }
    fn packet(&self, names: &[u64], roots: &[Id]) -> super::transfer::Packet {
        ...
        let mut out = super::transfer::Builder::new(names.to_vec(), coords.iter().rev().copied().collect());
