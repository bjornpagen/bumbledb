...
impl<const N: usize, const P: bool> Diagram<N, P> {
    fn transport_raw(&self, r:u32, out:&mut super::transfer::Builder, memo:&mut FxHashMap<u32,u32>)->u32 {
        ...
        let semantic:Vec<_>=self.order[node.var as usize].iter().copied().enumerate()
            .take(N.trailing_zeros() as usize).filter(|&(_,c)|c<self.n.next_power_of_two().trailing_zeros()).collect();
        let children=(0..1usize<<semantic.len()).map(|i| {
            let old=semantic.iter().enumerate().fold(0,|v,(k,&(position,_))|v|(((i>>k)&1)<<position));
    ...
    fn raw_packet(&self,names:&[u64],roots:&[u32])->super::transfer::Packet {
        assert_eq!(names.len(),self.n.next_power_of_two().trailing_zeros() as usize);
        let order=self.order.iter().flat_map(|g|g[..N.trailing_zeros() as usize].iter().rev().copied())
            .filter(|&c|c<(names.len() as u32)).collect();
        ...
        let roots=roots.iter().map(|&r|self.transport_raw(r,&mut out,&mut memo)).collect();
...
impl<const N: usize, const P: bool> Carrier for Diagram<N, P> {
    ...
    fn full_space(nbits:u32,order:Vec<u32>)->Result<Self,&'static str> {Ok(Self::symbolic_order(nbits,order))}
    fn variable(&mut self,coordinate:u32)->Id {self.literal(coordinate)}
    fn packet(&self,names:&[u64],roots:&[Id])->super::transfer::Packet {
        self.raw_packet(names,&roots.iter().map(|&r|self.root(r)).collect::<Vec<_>>())
    }
...
impl Carrier for Anchored {
    fn dimensions(&self)->u32 {self.inner.n.next_power_of_two().trailing_zeros()}
    fn full_space(nbits:u32,order:Vec<u32>)->Result<Self,&'static str> {Ok(Self::symbolic(nbits,order))}
    fn variable(&mut self,coordinate:u32)->Id {self.literal(coordinate)}
    fn packet(&self,names:&[u64],roots:&[Id])->super::transfer::Packet {
        self.inner.raw_packet(names,&roots.iter().map(|&r|((r/2) as u32)^((r&1) as u32)).collect::<Vec<_>>())
    }
    fn prepare_spectrum(&self, plan: &mut super::observation::CountPlan) {
