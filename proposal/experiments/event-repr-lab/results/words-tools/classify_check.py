...
ap.add_argument('--occupancy-kernel', choices=['scalar','words'], default='scalar')
ap.add_argument('--output', default='classify-check.json')
args = ap.parse_args()
folder = LAB/'classify-prototype'
files = [folder/'Cargo.toml', folder/'src/lib.rs', LAB/'src/essential_raw.rs', LAB/'src/essential_words.rs', LAB/'src/occupancy.rs', LAB/'src/essential_store.rs', LAB/'src/occupancy_words.rs']
def digests():
...
result = dict(passed=run.returncode==0, command=command, profile='standalone release; correctness only, no native timing',
              ...
              scope='Read-only raw finite-binary classifier; Matched scalar/word classification, all small supported predicates and symmetries, scattered coordinates, pointwise axis insertion and pinned/broadcast views up to twelve local axes. Not a scoped/public API or native filter.')
