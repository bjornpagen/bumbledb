...
SCENARIOS=['random_4096','overlap_4096','structured_65536','sparse_65536','runs_65536','coup_4290','coup_product_65536','ordered_65536']
RESULTS=LAB/'results'
...
def execute(binary,job,index,trials,timeout,log_dir):
    ...
    for line in stdout.splitlines():
        if 'EVENT_LAB ' in line:
            ...
            if row.get('kind') in ['modal_free_join','relations_free_join','law_free_join','transfer_free_join','owned_free_join','elimination']:
...
def main():
    ...
    ap.add_argument('--lane',choices=['join','classify','elimination','symbolic','modal','relations','laws','transport','owned','symbolic-relations','complement','all'],default='all');ap.add_argument('--trials',type=int,default=7)
    ...
    ap.add_argument('--occupancy-kernel',choices=['scalar','words','both'],default='scalar')
    ...
    occupancy_kernels=['scalar','words'] if args.occupancy_kernel=='both' else [args.occupancy_kernel]
    verification_jobs=[(mode,kernel,storage,occupancy) for mode in transfer_modes for kernel in essential_modes for storage in essential_layouts for occupancy in occupancy_kernels]
    records=[execute(binary,{'verify':True,'EVENT_LAB_PACKED_MAP':args.packed_map,'EVENT_LAB_TRANSFER_IMPORT':mode,'EVENT_LAB_ESSENTIAL_KERNEL':kernel,'EVENT_LAB_ESSENTIAL_LAYOUT':storage,'EVENT_LAB_OCCUPANCY_KERNEL':occupancy,'EVENT_LAB_IDENTITY':args.identity if args.identity!='all' else 'native'},i,args.trials,args.timeout,log_dir) for i,(mode,kernel,storage,occupancy) in enumerate(verification_jobs)]
    ...
    if not args.verify_only:
        ...
        for lane in ['modal','relations','laws','transport','owned','symbolic-relations','complement']:
