...
/-- A total change of output names can be delayed until after contraction.
    The inverse laws are essential; arbitrary lossy maps cannot use this rule. -/
theorem output_bijection {W T S X Y : Type}
    (h : W → T) (u : W → X) (v : W → Y)
    (f : T → S) (g : S → T)
    (gf : ∀ t, g (f t) = t) (fg : ∀ s, f (g s) = s)
    (A : Region X) (B : Region Y) (s : S) :
    Fused (fun w => f (h w)) u v A B s ↔ Fused h u v A B (g s) := by
  constructor
  · rintro ⟨w, hs, ha, hb⟩
    exact ⟨w, by simpa only [gf] using congrArg g hs, ha, hb⟩
  · rintro ⟨w, ht, ha, hb⟩
    exact ⟨w, by change f (h w) = s; rw [ht, fg], ha, hb⟩

/-- Reindex admitted joint witnesses through a map with a section. Duplicate
    witnesses are harmless for existence; every old witness must still lift. -/
theorem joint_reindex_with_section {W V T X Y : Type}
    (h : W → T) (u : W → X) (v : W → Y)
    (f : W → V) (g : V → W) (gf : ∀ w, g (f w) = w)
    (A : Region X) (B : Region Y) (t : T) :
    Fused h u v A B t ↔
      Fused (fun z => h (g z)) (fun z => u (g z)) (fun z => v (g z)) A B t := by
  constructor
  · rintro ⟨w, ht, ha, hb⟩
    exact ⟨f w, by simpa [gf] using ht, by simpa [gf] using ha, by simpa [gf] using hb⟩
  · rintro ⟨z, ht, ha, hb⟩
    exact ⟨g z, ht, ha, hb⟩

/-- The local word merge is this Shannon identity at every meaningful bit.
    This is not a refinement proof of Rust packing, padding, or its map code. -/
theorem shannon_merge (selector low high : Bool) :
    ((low && !selector) || (high && selector)) =
      (if selector then high else low) := by
  cases selector <;> simp

#print axioms output_bijection
#print axioms joint_reindex_with_section
#print axioms shannon_merge
