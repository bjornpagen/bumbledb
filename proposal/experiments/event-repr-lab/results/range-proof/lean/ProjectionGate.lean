...
/-- The stronger FD is (X, observed Y) -> membership_A. It also covers faces
    that are entirely visible. Remove only the support gate, keeping every
    observation equality and the same joint existential witness. -/
theorem observable_gate_elimination {X Y O P : Type}
    (D : X → Prop) (J : Y → Prop) (A : X → Y → Prop)
    (fx : X → O) (fy : Y → P) (x₀ : X) (y₀ : Y) (legal : J y₀)
    (stable : ∀ x y z, fy y = fy z → (A x y ↔ A x z)) :
    (∃ x y, (D x ∧ J y) ∧ A x y ∧ fx x = fx x₀ ∧ fy y = fy y₀) ↔
    (∃ x y, D x ∧ A x y ∧ fx x = fx x₀ ∧ fy y = fy y₀) := by
  constructor
  · rintro ⟨x, y, hs, ha, hx, hy⟩
    exact ⟨x, y, hs.1, ha, hx, hy⟩
  · rintro ⟨x, y, hd, ha, hx, hy⟩
    exact ⟨x, y₀, ⟨hd, legal⟩, (stable x y y₀ hy).mp ha, hx, rfl⟩
...
#print axioms ProjectionGate.observable_gate_elimination
