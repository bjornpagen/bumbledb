...
pub struct Essential<const K: u32> {
    ...
    // This cache belongs to an immutable support in this carrier. Cloning
    // preserves that support; no certificate is accepted from another owner.
    support_maps: FxHashMap<Vec<u32>, bool>,
}
impl<const K: u32> Essential<K> {
    pub fn symbolic(dimensions: u32, order: Vec<u32>) -> Self {
        Self {
            ...
            support_maps: FxHashMap::default(),
        }
    }
    ...
    fn support_map(&mut self, map: &Permutation) -> bool {
        if self.support == 1
            || map
                .destinations
                .iter()
        ...
        if let Some(&preserves) = self.support_maps.get(map.destinations.as_slice()) {
            return preserves;
        }
        let mapped = self.raw.rename(self.support, &map.destinations);
        let preserves = mapped == self.support;
        self.support_maps
            .insert(map.destinations.clone(), preserves);
        preserves
    }
    fn support_map_bytes(&self) -> usize {
        self.support_maps.capacity() * (std::mem::size_of::<(Vec<u32>, bool)>() + 1)
            + self
                .support_maps
                .keys()
                .map(|m| m.capacity() * 4)
...
impl<const K: u32> RegionOps for Essential<K> {
    ...
    fn preserving_product(
        ...
        if !self.support_map(am) || !self.support_map(output) {
            return None;
        }
        let a = self.true_root(a);
        let b = self.true_root(b);
        let raw = self.raw.mapped_product(
            a,
            &am.destinations,
            b,
            &bm.destinations,
    ...
    fn bytes(&self) -> usize {
        self.raw.bytes() + self.order.capacity() * 4 + self.support_map_bytes()
    }
    fn memory_stats(&self) -> Option<MemoryStats> {
        ...
        Some(MemoryStats {
            ...
            cache_bytes: self.raw.cache_bytes() + self.support_map_bytes(),
            records: s.records,
            tables: s.tables,
...
impl<const K: u32> Carrier for Essential<K> {
    fn legal_space(domains: &super::legal::Domains, order: Vec<u32>) -> Result<Self, &'static str> {
        ...
        let population = out.raw.count(support);
        assert_eq!(population, domains.population());
        let anchor = domains.anchor();
        assert!(out.raw.evaluate(support, anchor));
        out.support = support;
