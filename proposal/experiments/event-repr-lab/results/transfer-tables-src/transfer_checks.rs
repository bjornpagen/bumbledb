                ...
                for reverse in [false, true] {
                    for hide in [false, true] {
                        ...
                        for flip in [0, 1] {
                            ...
                            let direct = evaluate_mode(&packet, root ^ flip, &map, &mut c, true);
                            let recursive =
                                evaluate_mode(&packet, root ^ flip, &map, &mut c, false);
...
pub fn all() {
    ...
    macro_rules! targets {($($t:ty),*)=>{$(target::<$t>(&f);table_modes::<$t>();)*};}
