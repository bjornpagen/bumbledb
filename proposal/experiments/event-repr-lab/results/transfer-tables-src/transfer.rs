    ...
    fn eval(&mut self, r: Ref) -> Id {
        ...
        let v = match node {
            ...
            Node::Table { coords, words } => {
                ...
                let direct = if self.direct_tables {
                    coords
                        .iter()
                        .map(|&k| self.map[k as usize])
                        .collect::<Option<Vec<_>>>()
                        .and_then(|axes| self.c.import_table(&axes, words))
                } else {
                    None
                };
...
fn evaluate<C: Carrier>(p: &Packet, r: Ref, map: &[Option<u32>], c: &mut C) -> Id {
    evaluate_mode(p, r, map, c, direct_tables())
}
...
fn direct_tables() -> bool {
    match std::env::var("EVENT_LAB_TRANSFER_IMPORT").as_deref() {
        Ok("recursive") => false,
        Ok("direct") | Err(std::env::VarError::NotPresent) => true,
...
impl<C: Carrier> Space<C> {
    ...
    pub fn publish(self: &Arc<Self>, ids: &[Id]) -> Batch<C> {
        ...
        state.published.extend(ids.iter().copied());
        Batch {
            owner: self.clone(),
            keys: ids
                .iter()
    ...
    pub fn resolve(&self, key: EventKey) -> Result<Id, Error> {
        ...
        if !self.state.lock().unwrap().published.contains(&key.region) {
