...
assert sum(len(record['checked_axioms']) for record in lean_check['files']) == 82
...
retraction_checks={}
for name in ['retraction-complete-enum-check.json','retraction-complete-slab-check.json']:
    record=json.loads((LAB/'results'/name).read_text())
    rows=[json.loads(line.split('EVENT_LAB ',1)[1]) for line in (LAB/'results'/record['log']).read_text().splitlines() if 'EVENT_LAB ' in line]
