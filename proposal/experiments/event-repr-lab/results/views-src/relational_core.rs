//! Full-product binary relation program. Includes real coordinate movement.
...
impl<C: RegionOps> Algebra<C> {
    ...
    pub(super) fn try_new(
        ...
        Ok(Self {
            ...
            views: product_mode() == "views" && C::VIEW_PRODUCT,
            width,
            identity,
        })
    }
    ...
    pub(super) fn product_strategy(&self) -> &'static str {
        if self.views { "views" } else { "materialized" }
    }
    pub(super) fn product(&mut self, a: Id, b: Id, mask: u64) -> Id {
        ...
        let out = if self.views {
            self.base
                .inner
                .view_product(a, &self.maps[4], b, &self.maps[4], mask, &self.maps[4])
                .expect("checked full-product view capability")
        } else {
    ...
    fn mapped_product(&mut self, a: Id, am: usize, b: Id, bm: usize, mask: u64, out: usize) -> Id {
        ...
        let value = self
            .base
            .inner
            .view_product(a, &self.maps[am], b, &self.maps[bm], mask, &self.maps[out])
            .expect("checked full-product view capability");
        if self.base.enabled {
