...
def execute(binary,job,index,trials,timeout,log_dir):
    ...
    for line in stdout.splitlines():
        if 'EVENT_LAB ' in line:
            ...
            if row.get('kind') in ['modal_free_join','relations_free_join','product_free_join','law_free_join','transfer_free_join','owned_free_join','elimination']:
            ...
            if row.get('kind') in ['relations_free_join','product_free_join']:
...
def main():
    ...
    ap.add_argument('--lane',choices=['join','classify','elimination','symbolic','modal','relations','products','laws','transport','owned','symbolic-relations','complement','all'],default='all');ap.add_argument('--trials',type=int,default=7)
    ap.add_argument('--product',choices=['materialized','views','both'],default='materialized')
    ...
    if not args.verify_only:
        ...
        for lane in ['modal','relations','products','laws','transport','owned','symbolic-relations','complement']:
    ...
    jobs=[dict(job,EVENT_LAB_PRODUCT=mode) for job in jobs for mode in (product_modes if job.get('EVENT_LAB_CANDIDATE','').startswith('essential') and job.get('EVENT_LAB_LANE') in ['relations','products','owned','laws','symbolic-relations'] else ['materialized'])]
