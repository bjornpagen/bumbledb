...
ap.add_argument('--mode',nargs='+',choices=['materialized','views'],default=['materialized','views'])
args=ap.parse_args()
...
for storage in args.storage:
    for mode in args.mode:
        job=dict(common,verify=True,EVENT_LAB_ESSENTIAL_LAYOUT=storage,EVENT_LAB_PRODUCT=mode)
        ...
        for mode in args.mode if candidate.startswith('essential') else ['materialized']:
            for lane in args.lane:
                ...
                for layout in ['bit-major'] if lane=='symbolic-relations' else args.layout:
                    jobs.append(dict(common,EVENT_LAB_CANDIDATE=candidate,EVENT_LAB_LANE=lane,
