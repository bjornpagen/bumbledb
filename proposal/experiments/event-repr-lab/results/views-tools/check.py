...
ap=argparse.ArgumentParser();ap.add_argument('--release',action='store_true');ap.add_argument('--product',choices=['materialized','views'],default='materialized');ap.add_argument('--essential-kernel',choices=['words','scalar','derived'],default='words');ap.add_argument('--occupancy-kernel',choices=['scalar','words','scratch'],default='scalar');ap.add_argument('--essential-layout',choices=['enum','slab'],default='enum');ap.add_argument('--output',default='fast-check.json');ap.add_argument('--packed-map',choices=['local','recursive'],default='local');ap.add_argument('--transfer-import',choices=['words','direct','recursive'],default='words');args=ap.parse_args()
...
num-traits = "=0.2.19"
''')
names=['carrier','essential','essential_raw','diagram','diagonal_checks','finite','packed','relational_core','semantic_checks','observation','observation_checks','dependencies','transfer','transfer_checks','occupancy','signature']
...
start=time.time();p=subprocess.run(['cargo','test','--offline']+(['--release'] if args.release else [])+['--','--nocapture','--test-threads=1'],cwd=folder,capture_output=True,text=True,env=env)
