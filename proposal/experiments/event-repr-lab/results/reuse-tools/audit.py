...
for name, digest in json.loads((LAB/'results/scratch-reading.json').read_text())['files'].items():
    assert hashlib.sha256((ROOT/name).read_bytes()).hexdigest() == digest, name
for reading in ['view-reading.json', 'view2-reading.json', 'reuse-reading.json']:
...
for revision, expected_count in [('lean-v1',17), ('lean-v2',24), ('lean-v3',29), ('lean-v4',36), ('lean-v5',41), ('lean-v6',44), ('lean-v7',46)]:
...
assert sum(len(record['checked_axioms']) for record in lean_check['files']) == 50
...
for run in reuse_work['runs']:
    assert run['exit_code']==0 and all(row['verified'] for row in run['rows'])
assemblies = {}
for tag in ['relations', 'tail', 'dependency', 'transfer-words', 'essential', 'essential-derived', 'classify', 'slab', 'slab-storage', 'words', 'scratch', 'views-v1', 'views-v2', 'reuse']:
