...
mod separator;
mod relation_pack;
mod relation_pack_checks;
mod relation_pack_bench;
mod pack;
mod pack_checks;
mod pack_bench;
mod range_raw;
...
mod relational_core;
mod retraction;
mod retraction_checks;
mod ite_checks;
mod projection_checks;
mod local_checks;
mod factor_checks;
mod readout;
mod readout_bench;
...
mod signature;
mod signature_bench;
mod symbolic_relations;
mod transfer;
mod transfer_bench;
mod transfer_checks;
mod view_bench;
use blocks::Block64;
use pack_bench::bench as bench_pack;
use relation_pack_bench::bench as bench_relation_pack;
use carrier::*;
use diagram::{Anchored, Diagram};
use essential::Essential;
use retraction::Retraction;
use finite::*;
use law_bench::bench_laws;
use legal_bench::bench as bench_legal_relations;
use readout_bench::bench as bench_readouts;
use modal::{bench_complement, bench_modal};
use owned_bench::bench_owned;
use packed::Packed;
use relations::bench_relations;
use scoped_bench::bench as bench_scoped_product;
use signature_bench::bench as bench_signature;
use std::hint::black_box;
use std::time::Instant;
use symbolic_relations::bench as bench_symbolic_relations;
use transfer_bench::bench_transfer;
use view_bench::bench as bench_view_product;
...
fn semantics() {
    separator::verify();
    relation_pack_checks::all();
    range_checks::all();
    pack_checks::all();
    semantic_checks::verify::<Packed<8>>();
    semantic_checks::verify::<Finite<Dense>>();
...
fn workload(name: &str) -> (usize, Vec<Vec<u64>>) {
    ...
    let bank = (0..48)
        .map(|i| {
            bits(n, |w| match name {
                // Nested cuts and their complements have at most three occupied
                // pair cells. Classification cannot finish by saturating to 15.
                "ordered_65536" => w < (i + 1) * n / 49,
                "axes_65536" => {
                    let axis = i % 16;
                    let a = w >> axis & 1 != 0;
                    match i / 16 {
...
fn bench<C: Carrier>(scenario: &str, shape: &str) {
    ...
    let construction_bytes = c.bytes();
    ...
    for cached in [false, true] {
        ...
        println!(
            "EVENT_LAB {{\"kind\":\"free_join\",\"candidate\":\"{}\",\"scenario\":\"{}\",\"shape\":\"{}\",\"memo\":{},\"worlds\":{},\"admissible_worlds\":{},\"rows\":{},\"groups\":{},\"plan_nodes\":{},\"input_nodes\":{},\"final_nodes\":{},\"input_bytes_est\":{},\"final_bytes_est\":{},\"colt_bytes\":{},\"construction_bytes_est\":{},\"input_storage\":{},\"final_storage\":{},\"build_s\":{:?},\"native_setup_s\":{},\"native_first_join_s\":{},\"join_only_s\":{:?},\"fresh_s\":{:?},\"warm_s\":{:?},\"warm_loops\":{},\"checksum\":{},\"verified\":true}}",
            C::NAME,
            scenario,
            shape,
            cached,
            ...
            memory_json(current.inner.memory_stats()),
            builds,
            setup,
            first_join,
            baselines,
...
fn symbolic_essential<const K: u32>(pairs: u32, interleaved: bool) {
    let order = if interleaved {
        (0..pairs).flat_map(|i| [i, pairs + i]).collect()
    } else {
        (0..pairs * 2).collect()
    ...
    let start = Instant::now();
    let mut c = Essential::<K>::symbolic(2 * pairs, order);
    let mut event = c.full();
    for i in 0..pairs {
        let a = c.literal(i);
        let b = c.literal(i + pairs);
...
macro_rules! dispatch {($name:expr,$f:ident $(,$arg:expr)*)=>{match $name {
    "range-shannon"=>$f::<range_carrier::RangeCarrier<0>>($($arg),*),
    "range-step"=>$f::<range_carrier::RangeCarrier<1>>($($arg),*),
    "range-group"=>$f::<range_carrier::RangeCarrier<2>>($($arg),*),
    "packed512"=>$f::<Packed<8>>($($arg),*),
    "dense"=>$f::<Finite<Dense>>($($arg),*),
    "essential512"=>$f::<Essential<9>>($($arg),*),
    "retraction512"=>$f::<Retraction<9>>($($arg),*),
    _=>panic!("candidate outside this native range round")
}};}
...
fn experiment() {
    ...
    if lane == "relation-pack" {
        dispatch!(name.as_str(),bench_relation_pack);
    } else if lane == "pack" {
        let scenario=std::env::var("EVENT_LAB_SCENARIO").unwrap();
        dispatch!(name.as_str(),bench_pack,&scenario);
    } else if lane == "join" {
        let scenario = std::env::var("EVENT_LAB_SCENARIO").unwrap();
        for shape in ["triangle", "clover"] {
        ...
        let scenario = std::env::var("EVENT_LAB_SCENARIO").unwrap();
        for shape in ["triangle", "clover"] {
            dispatch!(name.as_str(), bench_signature, &scenario, shape);
        }
    ...
    } else if lane == "products" {
        for nbits in [12, 18] {
            dispatch!(name.as_str(), bench_view_product, nbits);
        }
    } else if lane == "scoped-products" {
        for nbits in [12, 18] {
            dispatch!(name.as_str(), bench_scoped_product, nbits);
        }
    } else if lane == "legal-relations" {
        for width in [4, 6] {
            dispatch!(name.as_str(), bench_legal_relations, width);
        }
    } else if lane == "readouts" {
        for width in [4, 6] {
            dispatch!(name.as_str(), bench_readouts, width);
        }
    } else if lane == "laws" {
        for nbits in [12, 18] {
    ...
    } else if lane == "symbolic-relations" {
        for width in [6, 12, 20] {
            dispatch!(name.as_str(), bench_symbolic_relations, width);
        }
        ...
        match name.as_str() {
            ...
            "essential64" => symbolic_essential::<6>(pairs, interleaved),
            "essential512" => symbolic_essential::<9>(pairs, interleaved),
...
#[test]
fn factorized_pack_semantics() { pack_checks::all(); }
