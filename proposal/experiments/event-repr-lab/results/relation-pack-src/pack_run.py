...
def execute(binary,job,index,trials,timeout,log_dir):
    env=os.environ.copy();env.update({key:str(value) for key,value in job.items() if key.startswith(('EVENT_LAB_','EVENT_PACK_'))})
    ...
    env['EVENT_LAB_RETRACTION_PROJECT']=str(job.get('EVENT_LAB_RETRACTION_PROJECT','joint'))
    env['EVENT_LAB_RETRACTION_COMPLETE']=str(job.get('EVENT_LAB_RETRACTION_COMPLETE','full'))
    env['EVENT_LAB_RETRACTION_EXECUTE']=str(job.get('EVENT_LAB_RETRACTION_EXECUTE','staged'))
    ...
    if 'EVENT_LAB_RETRACTION_EXECUTE' in job: label+='-execute-'+job['EVENT_LAB_RETRACTION_EXECUTE']
    if 'EVENT_LAB_RETRACTION_COMPLETE' in job: label+='-complete-'+job['EVENT_LAB_RETRACTION_COMPLETE']
    if 'EVENT_LAB_RETRACTION_PROJECT' in job: label+='-project-'+job['EVENT_LAB_RETRACTION_PROJECT']
    ...
    for line in stdout.splitlines():
        if 'EVENT_LAB ' in line:
            ...
            row['retraction_execute']=job.get('EVENT_LAB_RETRACTION_EXECUTE','staged')
            row['retraction_complete']=job.get('EVENT_LAB_RETRACTION_COMPLETE','full')
            row['retraction_project']=job.get('EVENT_LAB_RETRACTION_PROJECT','joint')
...
def main():
    ...
    ap.add_argument('--retraction-execute',choices=['staged','fused'],default='staged')
    ap.add_argument('--retraction-complete',choices=['full','needed','local','local-needed'],default='full')
    ap.add_argument('--retraction-project',choices=['joint','active','witness'],default='joint')
    ...
    for i,job in enumerate(jobs,len(records)):
        ...
        job['EVENT_LAB_RETRACTION_PROJECT']=args.retraction_project
        job['EVENT_LAB_RETRACTION_COMPLETE']=args.retraction_complete
        job['EVENT_LAB_RETRACTION_EXECUTE']=args.retraction_execute
