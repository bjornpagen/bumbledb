...
pub fn count<const K: u32>(
    ...
    fn visit<const K: u32, const WORDS: bool>(
        ...
        if vars.count_ones() <= K {
            if WORDS {
                out = super::occupancy::intersection_count(a, views, vars);
            } else {
            ...
            for high in [false, true] {
                ...
                out += visit::<K, WORDS>(a, order, child, memo) << free;
            }
        }
    ...
    let count = if words {
        visit::<K, true>(a, order, views, &mut FxHashMap::default())
    } else {
        visit::<K, false>(a, order, views, &mut FxHashMap::default())
    };
    count << (order.len() as u32 - variables(a, views).count_ones())
}
