...
impl Block64 {
    fn transport_raw(
        ...
        let v = if r & 2 == 0 {
            out.table(
                self.groups[self.tail].clone(),
                vec![self.leaves[(r >> 2) as usize]],
            )
        } else {
            let node = self.branches[(r >> 2) as usize];
            let edges = &self.edges[node.start as usize..(node.start + node.len) as usize];
            let children = (0..self.cells[node.level as usize])
                .map(|i| {
                    let child = edges
                        .iter()
                        .find(|e| e.selector >> i & 1 != 0)
                        .unwrap()
                        .child;
