...
theorem sequential_is_not_simultaneous :
    let swap : Bool → Term Bool := fun v => .atom (!v)
    let first : Bool → Term Bool := fun v => if v then .atom true else .atom true
    let second : Bool → Term Bool := fun v => if v then .atom false else .atom false
    let t : Term Bool := .atom false
