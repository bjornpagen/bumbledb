...
pub struct Native {
    ...
    separator: Option<super::separator::Certificate>,
}
...
impl Native {
    pub fn pair(data:&[Vec<Row>;2])->Self {
        Self::with_mapping(data,&[[0,1,3,4],[0,2,3,5]],[0,3,4,5,5])
    }
    pub fn separator(&self)->Option<&super::separator::Certificate> {self.separator.as_ref()}
    pub fn new(data: &[Vec<Row>; 3], shape: &str) -> Self {
        ...
        Self::with_mapping(data,&mappings,vars)
    }
    /// One occurrence of the same typed row schema, executed by Free Join.
    /// Repeated output slots keep the callback shape unchanged.
    pub fn single(data:&[Row])->Self {
        Self::with_mapping(&[data.to_vec()],&[[0,1,2,3]],[0,2,3,3,3])
    }
    fn with_mapping(data:&[Vec<Row>],mappings:&[[u16;4]],vars:[u16;5])->Self {
        assert_eq!(data.len(),mappings.len());
        let schema: Schema = SchemaDescriptor {
            relations: (0..data.len())
        ...
        let separator=if data.len()==2 {
            super::separator::Certificate::derive(&normalized,
                [BTreeSet::from([OccId(0)]),BTreeSet::from([OccId(1)])],
                BTreeSet::from([VarId(vars[0]),VarId(vars[1])])).ok()
        } else {None};
        let order = JoinOrder {
            order: (0..data.len()).map(|i|OccId(i as u16)).collect(),
            estimates: vec![0; data.len()],
        };
        ...
        let images: Vec<_> = (0..data.len())
            .map(|i| source.image(&cache, RelationId(i as u32)))
            .collect();
        let colts = plan
            .occurrences()
            .iter()
