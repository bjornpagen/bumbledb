...
pub fn fixture(width:u32,fanout:usize,p:Program)->Fixture {
    ...
    let groups=16;
    let raw=std::array::from_fn(|family|(0..groups).flat_map(|g|(0..fanout).map(move |i|
        [g as u64,i as u64,1,((g*7+i*3+family*5)%bank_size+family*bank_size) as u64])).collect());
...
fn case<C:Carrier>(input:&Checked<C>,data:&[Vec<Row>;2],groups:usize,bank:&BTreeMap<Id,Vec<u64>>,d:&Domains)->usize {
    ...
    for p in [Program::Compose,Program::Residual] {let expected=oracle(data,groups,bank,d,p);
        for memo in [false,true] {for schedule in [Schedule::Complete,Schedule::Factorized] {
            ...
            for _ in 0..2 {
                match (&expected,engine.run(&mut c,groups,&published,p)) {
                    ...
                    (Ok((out,rows)),Ok(actual))=>{
                        ...
                        if schedule==Schedule::Factorized {assert_eq!(actual.summary_rows,2*actual.present());assert_eq!(actual.native_emissions,actual.branch_emissions+actual.present());}
...
fn carrier<C:Carrier>() {
    ...
    println!("EVENT_LAB {{\"kind\":\"relation_pack_verification\",\"candidate\":\"{}\",\"native_runs\":{checks},\"passed\":true}}",C::NAME);
}
pub fn all() {
