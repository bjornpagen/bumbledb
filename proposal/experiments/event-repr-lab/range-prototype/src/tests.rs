...
fn all_small_functions() {
    ...
    for kernel in [Kernel::Steps,Kernel::Groups] {for order in orders {for mode in [Mode::Plain,Mode::Ranges] {
    ...
    println!("RANGE_CHECK {{\"passed\":true,\"binary\":{binary},\"projection\":{projection},\"maps\":{maps},\"completed_projection\":{completed},\"orders\":6,\"modes\":2,\"kernels\":2}}");
}
...
#[test]
fn sparse_shared_exits() {
    let mut combinations=0;let mut projections=0;let mut cofactor_checks=0;
    for kernel in [Kernel::Steps,Kernel::Groups] {for reverse in [false,true] {for mode in [Mode::Plain,Mode::Ranges] {
        let mut order:Vec<_>=(0..8).collect();if reverse {order.reverse();}
        let mut c=Arena::with_kernel(order.clone(),mode,kernel);
        // Arbitrary overlapping child functions, after four selector coordinates.
        let lo_tab:Vec<_>=(0..256u64).map(|w| {
            let t=(4..8).fold(0,|m,i|m|((w>>order[i]&1)<<(i-4)));0x6b97u64>>t&1!=0
        }).collect();
        let hi_tab:Vec<_>=(0..256u64).map(|w| {
            let t=(4..8).fold(0,|m,i|m|((w>>order[i]&1)<<(i-4)));0xa639u64>>t&1!=0
        }).collect();
        let lo=c.import(&lo_tab);let hi=c.import(&hi_tab);
        let mut cases=Vec::new();
        for bits in 1..16u64 {for flip in 0..2 {
            let axes=(0..4).fold(0,|m,i|m|(((bits>>i)&1)<<order[i]));
            let r=c.block(axes,lo,hi)^flip;
            let table:Vec<_>=(0..256).map(|w| (if w as u64&axes==0 {lo_tab[w]} else {hi_tab[w]})^(flip!=0)).collect();
            assert_eq!(r,c.import(&table));cases.push((r,table));
        }}
        let roots:Vec<_>=cases.iter().map(|v|v.0).collect();
        let (mut collected,roots)=c.compact(&roots);c=collected.clone();
        for (i,&r) in roots.iter().enumerate() {
            let table=&cases[i].1;
            assert_eq!(c.raw_uniform_count(r),table.iter().filter(|&&v|v).count() as u64);
            for hidden in 0..256u64 {
                let expected:Vec<_>=(0..256u64).map(|w|(0..256u64).any(|s|s&!hidden==w&!hidden && table[s as usize])).collect();
                assert_eq!(c.exists(r,hidden),c.import(&expected));projections+=1;
            }
            for v in 0..8 {for high in [false,true] {
                let expected:Vec<_>=(0..256).map(|w|table[(w&!(1<<v))|((high as usize)<<v)]).collect();
                assert_eq!(collected.cofactor(r,v,high),collected.import(&expected));cofactor_checks+=1;
            }}
            for (j,&s) in roots.iter().enumerate() {for op in 0..16u8 {
                let expected:Vec<_>=(0..256).map(|w|(op>>((table[w] as u8)*2+cases[j].1[w] as u8))&1!=0).collect();
                assert_eq!(c.apply(op,r,s),c.import(&expected));combinations+=1;
            }}
        }
    }}}
    assert_eq!((combinations,projections,cofactor_checks),(115200,61440,3840));
    println!("RANGE_SPARSE_CHECK {{\"passed\":true,\"combinations\":{combinations},\"projections\":{projections},\"cofactors\":{cofactor_checks}}}");
}
...
fn long_chains_and_collection() {
    ...
    for kernel in [Kernel::Steps,Kernel::Groups] {for n in [9usize,10,11,12,20,21,22,61] {for reverse in [false,true] {for mode in [Mode::Plain,Mode::Ranges] {
        ...
        if n==61 {
            println!("RANGE_LONG_SHAPE {{\"passed\":true,\"dimensions\":{n},\"reverse\":{reverse},\"mode\":\"{mode:?}\",\"kernel\":\"{kernel:?}\",\"constructed_records\":{},\"compacted_records\":{nodes},\"compacted_bytes\":{}}}",c.nodes(),compact.bytes());
        }
    ...
    println!("RANGE_CHAIN_CHECK {{\"passed\":true,\"world_checks\":{checked},\"max_dimensions\":61}}");
    assert_eq!((multi_masks,multi_worlds),(128,16384));
    println!("RANGE_MULTI_CHECK {{\"passed\":true,\"four_axis_masks\":{multi_masks},\"visible_worlds\":{multi_worlds},\"hidden_assignments_per_world\":16}}");
}
