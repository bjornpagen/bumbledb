...
ap=argparse.ArgumentParser();ap.add_argument('--output',default='build-relation-pack.json');args=ap.parse_args()
check_engine_sources()
engine=SCRATCH/'relation-pack-engine';engine.mkdir(exist_ok=True)
...
def sources():return {str(p.relative_to(LAB)):digest(p) for p in sorted((LAB/'relation-pack-src').glob('*.rs'))}
...
print('Building isolated relational Pack comparison.',flush=True)
...
out=dict(binary=binary,binary_sha256=digest(Path(binary)),source_sha256=before,
    ...
    boundary='Same-binary native relational Pack schedules with checked legal relation roles. Independent branch scans and summary joining use actual Free Join. Timing requires separate serial runs.')
