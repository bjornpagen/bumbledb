"""Serial same-binary comparison of direct completed projection."""
...
parser.add_argument('--candidate', nargs='+', choices=[
    ...
    default=['prefix512', 'retraction512', 'packed512', 'dense-dispatched'])
...
parser.add_argument('--normalize', nargs='+', choices=['staged', 'equal', 'ite'], default=['ite'])
parser.add_argument('--execute',nargs='+',choices=['staged','fused'],default=['staged','fused'])
parser.add_argument('--completion',nargs='+',choices=['full','needed','local','local-needed'],default=['local-needed'])
parser.add_argument('--gates',nargs='+',choices=['joint','active','witness'],default=['witness'])
...
def save():
    record = dict(
        experiment='Same-binary staged versus direct completed projection; independent gate, completion and conditional controls; actual three-input Free Join and grouped union, followed by original/Possible/Guaranteed/Ambiguous Events; 64 outputs',
...
for storage in args.storage:
    for mode in args.normalize:
        job = dict(common, verify=True, EVENT_LAB_ESSENTIAL_LAYOUT=storage, EVENT_LAB_RETRACTION_NORMALIZE=mode, EVENT_LAB_RETRACTION_PROJECT=args.gates[-1], EVENT_LAB_RETRACTION_COMPLETE=args.completion[-1], EVENT_LAB_RETRACTION_EXECUTE=args.execute[-1])
                        ...
                        for factor in args.factor if candidate.startswith(('prefix', 'retraction')) else ['joint']:
                            for mode in args.normalize if candidate.startswith(('prefix', 'retraction')) else ['staged']:
                                for gates in args.gates if candidate.startswith(('prefix','retraction')) else ['joint']:
                                    for completion in args.completion if candidate.startswith(('prefix','retraction')) else ['full']:
                                        for execution in args.execute if candidate.startswith(('prefix','retraction')) else ['staged']:
                                            jobs.append(dict(common, EVENT_LAB_LANE='readouts', EVENT_LAB_CANDIDATE=candidate,
                                                         ...
                                                         EVENT_LAB_RETRACTION_COMPLETE=completion, EVENT_LAB_RETRACTION_EXECUTE=execution))
