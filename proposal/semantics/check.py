...
def main():
    ...
    ap.add_argument('--lean', required=True, type=Path,
                    help='Installed Lean 4.32.0 executable; no toolchain is downloaded')
    args = ap.parse_args()
    ...
    assert version.startswith('Lean (version 4.32.0,'), version
    ...
    for i, (path, names) in enumerate(expected.items()):
        ...
        passed = (p.returncode == 0 and len(axioms) == len(names)
                  and len(set(names)) == len(names)
                  and 'warning:' not in log and 'error:' not in log
                  and all(set(v) <= permitted for v in axioms.values()))
