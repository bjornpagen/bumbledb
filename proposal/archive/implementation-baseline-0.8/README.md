...
**Revision 0.8.** The proposal extends public BumbleDB **v1.3.1** with a first-class
`event` field and owned `Event` value. It is ready to implement against explicit
acceptance gates; it is not an implemented engine feature. The open-ended
...
| [Proof coverage](semantics/README.md) | Fresh Lean results, theorem-to-contract mapping and unproved obligations |
...
revision 0.8. Laboratory pages report experiments; they do not define additional
...
Native schema/admission, persistence, transaction ownership, source solvers and
planner integration remain the work specified by the implementation plan.
