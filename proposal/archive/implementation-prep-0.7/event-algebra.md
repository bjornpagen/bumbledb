...
**Revision 0.5; normative proposal, not implemented engine syntax.** This is the
...
| --- | --- | --- |
| `Event` | A region in one captured, nonempty world space | Existing two-word EventKey and shared complement-pair BDD arena |
...
| Complement | `S \ A` | `!a`; toggle pair orientation |
...
Use total transition relations, or include an explicit failure outcome, when
requiring that law. Revision 0.5 also includes exact Star and monotone fixed
points on a sealed finite stable state presentation, as specified in
...
field or an existing half-open interval. Empty evidence yields Impossible.
