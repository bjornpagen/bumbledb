...
**Representation update:** the [Rust laboratory](experiments/event-repr-lab/REPORT.md)
now compares complete finite canonical carriers and decision diagrams through real
Free Join. The BDD and dense-cofactor layouts below remain kernel candidates;
mandatory return-to-BDD normalization is no longer the universal resident design.
The earlier isolated instruction probes remain separate evidence.
...
collisions always compare complete keys. No native event cache is implemented yet.

## 4. Dense cofactor execution

A dense block is a truth bitplane over a declared, ordered list of assignments.
Bit k always refers to the same assignment in every operand and support plane.
For a bounded cofactor on up to eight Boolean variables, at most 256 truth bits
fit in two NEON vectors. A deliberately finite scenario can also use a longer
bitmap. Unused categorical codes and padding bits are masked by support.
...
truth values. It relies on B already lying in support. In a general BoolOp4
kernel, the result must be masked explicitly because functions such as NOR
produce true on zero/zero inputs outside support.
...
creation and return-to-BDD reduction are real costs. A local cofactor is valid
...
The [implementation experiments](implementation-plan.md) describe the next
...
Revision 0.5 adds a constructive relational core. A checked plan lifts
`R(theta,s,t)` and `Q(theta,t,u)` into one presentation, intersects their support-
masked roots, and existentially eliminates t. It then rebuilds the result in
the outer `(theta,s,u)` presentation. Shared theta is never freshly drawn or
independently existentially resolved on each side.
...
A fused recursive product avoids materializing the entire intermediate AND:
...
This pseudocode assumes both inputs already share the same order and admissible
triple presentation. Terminal handling, complemented references and output pair
interning use the canonical core. The memo retains the plan/quantified set and
...
same canonical constructors. No plugin can inject unchecked pair IDs or declare
an approximate result structurally equal.

Free Join plans and region plans remain inspectable separate objects. A fused
native path must preserve complete binding validation and the computed-stage
error boundary. This architecture permits optimization across stages without
