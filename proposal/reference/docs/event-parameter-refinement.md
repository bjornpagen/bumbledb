...
resolve a predicate roster on one source. `CommonSources` also imports exact
parameter distinctions from bound peer sources on that named domain. Each head
retains its primary source law and explicit identity. No public package version or release changes.
...
solving, parameter-changing maps,
