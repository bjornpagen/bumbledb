...
## Checked action arenas and persistent strategies

[`EventAction`](event-actions.md#sdk-arena-and-strategy-operations) now supplies
owned BEAC admission, description, inspection and replay; native enabled/good/
controlled-predecessor operations; ranked reach and continuing-safety solving;
and checked policy restriction. `fromMemory` explicitly exports a reconstructed
BEBA compilation into the same general action interface. Hidden predicates still
require the separate memory arena's interpretation. The SDK has no alternative
solver. Its ordinary Event and relation outputs work with existing stored/query
consumers; constructive action/memory query heads and general program transport
remain open. No release or version bump.
