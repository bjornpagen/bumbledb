...
**Derived; finite cases checked.** Fix an aligned event universe `W` and a
...
consequences additionally need valid normalized laws. Revision 0.5's
...
**Derived; finite cases checked.** Fix nonempty Boolean support S and a fixed
variable order. Let C be canonical reduced ordered BDD construction. For E,
form `(C(S & E), C(S & !E))`. The two roots are disjoint and their union is S.
They are unequal because S is nonempty. Ordering and interning this root pair,
with one orientation bit selecting the true root, gives an event handle.

E and F denote the same subset of support iff their selected canonical roots
are equal, hence iff their oriented pair handles are equal within the same
arena. Complement exchanges the pair's roots, so toggling its orientation bit
is exact relative complement. Empty and full are `(0,S)` in opposite orientations.
This proves why a support-masked root alone is insufficient for the desired
one-bit relative-complement implementation. Hash equality without full tuple
equality does not establish the canonical constructor's premise.
