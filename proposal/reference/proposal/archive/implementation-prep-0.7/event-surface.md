...
identity is a scoped canonical region handle. The experiments compare several
ways to represent it, including paired roots, an anchored root, and finite sets;
see [the proposal](proposal.md) and
[memory layout](representation.md). This page specifies the public meanings.
All new syntax remains proposed, not supported by the native engine yet.
