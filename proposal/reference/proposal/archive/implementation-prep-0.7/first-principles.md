...
is a sixteen-byte key. Its shared space owns a reduced decision diagram, which
shares repeated tests and identical continuations. The key selects one side of
a two-way partition of legal worlds. Toggling one bit selects the other side.
The owned value also retains its space so these references remain valid.
