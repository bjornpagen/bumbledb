...
**Revision 0.5; normative direction.** Event's unary Boolean operations are one
...
The [finite checks](algebra-checks.py) retain that counterexample.
...
The termination argument needs a finite, stable state quotient. Source parameters
can remain as a fixed shared environment with finitely many guard cells, provided
every relation respects that presentation. Arbitrary transformations of continuous
