...
**Revision 0.8; proposed extension to public BumbleDB v1.3.1.** This is the
...
and law certificates; those capabilities are distinct.
...
are unchanged. The lab validates unordered fault sets; native stable descriptors
and error integration remain acceptance work.
...
allocation order, decoder choice and execution layout. The exact codec and
schema-format version are a mandatory M2 gate, not already-qualified lab output.
The storage milestone cannot ship until cross-order/cross-carrier identity,
owner lifetimes, rollback, corruption and concurrent publication checks pass.
