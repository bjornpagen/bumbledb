...
**Revision 0.8.** Implement the full [Event contract](proposal.md) in ordered
...
The baseline is public BumbleDB v1.3.1. The current checkout additionally removes
scratch allocation-count tests; no production Event feature has been added.
Prefer isolated implementation commits with each gate's evidence attached.
...
**Native area:** [values](../crates/bumbledb/src/value.rs), new internal Event
manager/owner modules, binding storage and WorkContext integration.
...
identity argument tied to the implemented encoder/decoder. This remains an
open implementation gate, not a proven part of the lab.
...
schema_partition_mass. The source constructor separately validates the law.
...
**Proof anchors:** InformationReadout, ReadoutMaps, ModalContract. General game
strategy synthesis and native fixed-point termination are not claimed proved.
