...
- Total accessors `Query.interiors` / `Query.arity` / `Query.rules` by match are fine. There is NO `Query.rec` accessor.
...
- Restate `evalQuery_sound` (`Reach.lean:977-1048`) by constructor cases; its `recLinear` premise dies with lean-002.
- Update `Bridge.lean` rows citing restated theorems (`@Query.evalQuery_sound`, `@Query.evalQuery_empty_rules`, …).
...
- Must land as one change with lean-002; lean-009's `allRules` deletion and lean-012 (duplicate of this issue) ride along.
