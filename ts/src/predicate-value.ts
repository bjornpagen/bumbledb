...
export function encodedPredicate(input: unknown): ObservationPredicate {
	...
	if (bytes.length < 5 || bytes[0] !== 66 || bytes[1] !== 69 || bytes[2] !== 78 || bytes[3] !== 80 || bytes[4] !== 1)
		throw new AuthoringError({ message: "ObservationPredicate: expected a BENP v1 envelope" })
