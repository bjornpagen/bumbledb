...
describe("the ψ-on-closed golden: manifest spelling, engine folding, violation paste-back", function psiSuite() {
	...
	test("the ENGINE folds ψ at validate: a member commit lands, a non-member commit pastes the manifest spelling back", function psiCommit() {
		...
		const landed = native.dbWrite(handle, function write(passing) {
			assert.deepEqual(native.txInsert(passing, certificate.id, 1n, [1n, 1n]), { submitted: 1n, changed: 1n })
			return true
		})
		...
		const rejected = native.dbWrite(handle, function write(violating) {
			assert.deepEqual(native.txInsert(violating, certificate.id, 1n, [2n, 0n]), { submitted: 1n, changed: 1n })
			return true
		})
